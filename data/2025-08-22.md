...
Method: 采用统一的协同训练框架，在单一SFT阶段同时训练三种安全行为模式，通过系统级指令或魔法令牌动态激活不同行为，形成安全对齐边界。

Result: 8B模型在安全性能上超越DeepSeek-R1（671B），匹配SFT+DPO的安全对齐质量，同时显著降低训练复杂度和部署成本。

Conclusion: 该方法提供了可扩展、高效且高度可控的LLM内容安全解决方案，实现了细粒度的行为控制和部署灵活性。
...
<details>
  <summary>Details</summary>
Motivation: 向任务参与者分享初步结果，帮助他们在准备系统提交论文时参考，但强调这不是最终结果

Method: 使用自动指标对机器翻译系统进行评估和排名

Result: 生成了基于自动评估的初步排名，但承认可能存在偏向重排序技术（如质量估计重排序和最小贝叶斯风险解码）的系统偏差

Conclusion: 自动排名是初步结果，最终官方排名将基于更可靠的人工评估，人工评估结果将取代自动排名
...
<details>
  <summary>Details</summary>
Motivation: 现有多语言数学推理基准主要通过翻译生成，保留了英语中心的实体，缺乏反映准确本土实体（如人名、组织名、货币）的社会文化任务数据集，导致低资源语言的数学推理能力落后于英语。

Method: 开发了一个LLM驱动的文化本地化框架，能够从现有资源自动构建包含本土名称、组织和货币的数学应用题数据集。

Result: 实验表明，翻译的基准测试在适当的社会文化背景下可能掩盖真实的多语言数学能力，该框架有助于减轻英语中心实体偏见，并在引入本土实体时提高各种语言的鲁棒性。

Conclusion: 该框架成功解决了多语言数学推理中文化本地化数据稀缺的问题，通过自动生成本土化数据集，提升了低资源语言数学推理的性能和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何利用少量易于生成的数据资源来改进通用指令调优大语言模型的机器翻译能力，特别是针对低资源语言如斯洛文尼亚语

Method: 使用Direct Preference Optimization (DPO)方法，基于程序化筛选和增强的公共数据集构建质量排序的翻译对，通过两个LLM生成翻译并基于启发式规则和COMET等自动评估指标进行排序

Result: 微调后的模型在翻译维基百科文章时，相比基线模型分别获得了约0.04和0.02的COMET分数提升，并且更一致地避免了语言和格式错误

Conclusion: DPO训练方法能够有效利用相对较少的程序化生成数据来显著提升大语言模型的机器翻译性能，特别是在低资源语言场景下
...
<details>
  <summary>Details</summary>
Motivation: 当前基于意图识别的ConvXAI系统在英语上表现良好，但面临多语言训练数据稀缺和自定义输入支持有限的问题，阻碍了多语言泛化能力。

Method: 首先扩展了MultiCoXQL多语言数据集覆盖5种语言，提出新的解析方法提升多语言解析性能；然后创建Compass数据集支持自定义输入提取，并在多语言、跨语言和单语言设置下评估多种LLM模型。

Result: 在MultiCoXQL上评估了三种LLM的不同解析策略，在Compass上对三种不同规模的LLM和BERT类模型进行了全面评估，验证了所提方法的有效性。

Conclusion: 通过构建多语言数据集和提出新的解析方法，有效解决了ConvXAI系统在多语言环境和自定义输入处理方面的挑战，为多语言对话式可解释AI的发展提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 测试时对齐技术虽然能提升大语言模型的安全性和推理能力，但往往带来巨大的推理成本，限制了实际应用。受推测采样加速技术的启发，希望解决测试时对齐的效率瓶颈问题。

Method: 提出奖励偏移推测采样算法，让草稿模型与人类偏好对齐，而目标模型保持不变。通过修改接受准则和奖励代币分布，利用对齐草稿模型与未对齐目标模型之间的分布偏移来恢复RLHF最优解。

Result: 在测试时弱到强对齐实验中，算法以显著降低的推理成本获得了优越的黄金奖励分数，验证了其有效性和效率。

Conclusion: 该方法成功解决了测试时对齐的高成本问题，通过创新的算法设计实现了高效的人类偏好对齐，为大语言模型的实际部署提供了可行的解决方案。
...
Method: 三阶段框架：1)分解答案为自包含事实 2)通过词法和语义过滤缩小候选匹配 3)结构化蕴含检查验证对齐

Result: 在三个长格式QA基准测试中，相比强基线方法在召回准确性上有显著提升

Conclusion: LongRecall提供了一个系统化召回评估的基础构建模块，能减少误报和漏报，适应多样化表达和上下文变化
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然在各种语言任务中表现出色，但存在幻觉问题和复杂推理困难，特别是在结构化预测任务中，需要结合推理方法来提高结构一致性

Method: 结合LLM与组合推理，研究不同提示策略来估计LLM置信度值用于符号推理，并进行校准和基于结构化预测目标的微调

Result: 实验表明，无论使用何种提示策略，在提示基础上增加符号推理都能产生更一致和准确的预测，结构化学习在LLM时代仍然具有重要价值

Conclusion: 将大语言模型与符号推理相结合能够有效解决LLM的结构一致性问题，结构化学习方法的集成显著提升了挑战性任务的性能
...
<details>
  <summary>Details</summary>
Motivation: 现有基于Common Crawl的数学数据集由于脆弱的提取启发式方法、有损的HTML到文本转换以及无法可靠保留数学结构而导致质量下降，需要更好的数据提取方法。

Method: 使用布局感知渲染（lynx）和基于LLM的针对性清理阶段的新管道，能够恢复各种格式的数学内容（MathJax、KaTeX、MathML），保持方程和代码块的结构完整性，同时去除样板文件并将符号标准化为LaTeX表示。
...
Conclusion: 这是第一个能够从嘈杂的网络规模数据中可靠提取科学内容（包括数学）的管道，在数学、代码和通用推理方面带来可衡量的收益，并在开放数学预训练语料库中设立了新的技术标准。
...
<details>
  <summary>Details</summary>
Motivation: 人们经常提出包含错误假设的问题，这类问题没有常规答案。大型语言模型(LLM)由于幻觉问题，往往会生成误导性回答。论文旨在解决识别和回答这类错误假设问题。

Method: 将问题简化为事实验证任务，提出利用外部证据来减轻幻觉的方法。具体包括检索相关证据，生成并验证原子假设，通过指定错误假设来提供可解释的答案。

Result: 在五个LLM上的实验表明：(1)整合检索证据是有益的；(2)生成和验证原子假设能带来更多改进，并通过指定错误假设提供可解释的答案。

Conclusion: 通过外部证据验证和原子假设分析的方法，能够有效识别和回答包含错误假设的问题，减少LLM的幻觉问题，提高回答的准确性和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型和视觉语言模型在处理复杂多轮、需要深度推理和视觉基础的任务时仍面临挑战，现有基准无法充分捕捉真实世界多模态交互的动态性和复杂性。

Method: 构建包含300个复杂多轮对话场景的MMDR-Bench数据集，并提出CoLVLM Agent框架，采用无需重新训练底层模型的迭代"记忆-感知-规划-执行"循环机制。

Result: CoLVLM Agent在MMDR-Bench上获得平均4.03的人类评估分数，显著超越GPT-4o(3.92)和Gemini 1.5 Pro(3.85)，在推理深度、指令遵循和错误抑制方面表现优异。

Conclusion: 该框架的模块化设计和迭代方法在复杂多模态交互中表现出有效性，能够维持长时间对话的稳定性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有分词方法如BPE和WordPiece仅基于频率统计，忽略了文本的语义结构，导致语义冗余区域的过度分词和上下文连贯性利用不足

Method: 使用轻量级编码器提取上下文语义嵌入，进行局部语义聚类合并语义等效token，基于语义密度分配异构token粒度，在内容丰富区域使用细粒度分词，在重复或低熵区域使用粗粒度压缩

Result: 在WikiText-103和LongBench等长上下文语言建模基准测试中，实现了最多2.4倍的token减少和1.9倍的加速，困惑度和下游任务准确率几乎没有下降

Conclusion: 语义结构为优化大语言模型中的分词和计算提供了新的有前景的方向
...
Conclusion: 领域专业化的奖励建模对于使LLMs与专家级金融推理对齐具有重要价值，Fin-PRM为金融领域的推理监督提供了有效解决方案。
...
Method: 采用无结构稀疏性，在通道级别剪枝KV缓存，并在注意力分数计算时动态恢复被剪枝的条目，与现有压缩技术正交兼容。

Result: 在相同内存预算下可处理更长序列，相同序列长度下KV缓存存储减少30%以上，即使80%剪枝率性能下降也小于5%。

Conclusion: SPARK通过通道级剪枝有效解决了KV缓存瓶颈问题，在保持模型精度的同时显著提升了长序列处理效率。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用问答中表现良好，但在特定领域场景中表现不佳。检索增强生成存在幻觉和延迟问题，而持续预训练成本高且缺乏跨领域灵活性。领域知识的长尾分布导致部分有用内部知识未被充分利用。

Method: 提出Selct2Know框架，采用内部-外部知识自选择策略和选择性监督微调来内化领域知识。引入结构化推理数据生成流程，并集成GRPO来增强推理能力。

Result: 在医学、法律和金融问答基准测试中，S2K持续优于现有方法，并以显著更低的成本达到领域预训练LLM的水平。

Conclusion: S2K框架通过渐进式知识获取策略，有效解决了领域特定场景中的知识利用问题，提供了一种成本效益高的解决方案。
...
Method: Stepwise Experience Recall (SEER)方法，从持续更新的经验池中进行细粒度逐步检索，增量式添加过去成功的轨迹，实现经验池的持续扩展

Result: 在ToolQA基准测试中，简单问题平均提升6.1%，困难问题提升4.7%；在τ-bench真实场景中，Qwen2.5-7B和72B模型分别获得7.44%和23.38%的准确率提升

Conclusion: SEER方法通过自引导的经验检索机制，有效解决了LLM在多步工具调用中的挑战，实现了持续性能改进，无需大量人工干预
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成任务自动评估中面临标准模糊的挑战，自动检查表生成方法的有用性尚未充分探索。

Method: 使用六种方法生成检查表，在八种模型规模上进行评估，通过成对比较和直接评分实验分析检查表效果，并识别与人工评估相关的检查表项目。

Result: 选择性使用检查表在成对比较设置中能提升评估性能，但在直接评分中效果不稳定。低相关性检查表项目仍反映人工编写标准，表明人工评估存在不一致性。
...
<details>
  <summary>Details</summary>
Motivation: 小型语言模型在资源受限环境中具有计算优势，但词汇相关组件（嵌入和语言建模头）由于词汇量大导致内存占用过高，现有静态词汇剪枝方法存在信息损失和灵活性不足的问题

Method: 基于词汇局部性原理和组件计算特性不对称的洞察，提出VocabTailor框架：1）通过卸载嵌入减少内存使用；2）为LM Head实现混合静态-动态词汇选择策略，按需加载词汇组件

Result: 在多样化下游任务上的综合实验表明，VocabTailor将词汇相关组件内存使用减少高达99%，任务性能仅有最小或没有退化，显著优于现有静态词汇剪枝方法

Conclusion: VocabTailor通过动态词汇选择有效解决了SLM部署中的内存瓶颈问题，为边缘设备部署提供了高效的内存优化解决方案
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在英语指令跟随方面表现出色，但在泰语等低资源语言中的性能仍未充分探索。现有基准测试通常依赖翻译，缺乏现实世界应用所需的文化和领域特定细微差别。

Method: 通过多阶段质量控制流程创建人工编写的泰语数据集WangchanThaiInstruct，包含四个专业领域和七种任务类型。进行两项研究：(1)零样本评估显示在文化和专业特定任务上的性能差距；(2)指令调优研究，通过消融实验分离本土监督的效果。

Result: 在WangchanThaiInstruct上微调的模型在领域内和领域外基准测试中均优于使用翻译数据的模型。零样本评估揭示了在文化和专业特定任务上的显著性能差距。

Conclusion: 研究结果强调了需要基于文化和专业背景的指令数据，以改善低资源、语言多样化环境中大型语言模型的对齐性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决代码切换语音技术中数据集稀缺的问题，促进多语言语音系统的发展

Method: 使用SWORDS算法（同义词替换单词），在保持句子语义不变的前提下，通过替换选定单词的翻译来生成代码切换语音

Result: CS-FLEURS语料库在可理解性和自然度方面表现优异，在客观和主观指标上与现有数据集相当

Conclusion: UniCoM方法能够推动代码切换语音技术的发展，使多语言系统更具包容性
...
<details>
  <summary>Details</summary>
Motivation: 现有的模拟职业角色的大语言模型缺乏全面的心理和伦理评估，特别是在教育领域需要评估教师角色的道德发展和伦理风险。
...
<details>
  <summary>Details</summary>
Motivation: 解决检索增强生成(RAG)中当检索到的外部上下文与LLM参数知识冲突时，模型无法正确处理这种冲突的问题

Method: 引入Conflict-Aware REtrieval-Augmented Generation (CARE)，包含上下文评估器和基础LLM。上下文评估器通过压缩记忆token嵌入和软提示技术来识别不可靠上下文并提供指导信号
...
Conclusion: 该方法能够有效挖掘时间常识，构建高质量数据集，提升模型在时间推理任务上的性能，为解决时间常识缺失问题提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 许多语音处理任务需要不受上下文影响的纯净音素表示，但传统方法需要分析长语音段和语言特定模式，因此需要开发能够快速捕获关键音素特征的轻量模型

Method: CUPE处理短固定宽度窗口（约一个音素长度），独立分析每个窗口，通过监督和自监督训练在多种语言上学习所有语言共有的基础声学模式

Result: 模型参数量少于当前方法但实现了竞争性的跨语言性能，在UCLA语音语料库的零样本测试中表现出强大的跨语言泛化能力

Conclusion: 通过在音素长度窗口内建模基础声学模式，可以实现有效的通用语音处理，证明了这种方法的可行性
...
<details>
  <summary>Details</summary>
Motivation: 知识图谱补全模型评估存在挑战，不同数据集和评估指标（如MRR、Hit@k）之间的性能比较不一致，导致模型选择困难，需要统一的元度量方法进行综合评估。

Method: 提出基于平均解距离的评估方法（EDAS），将模型在多个数据集和多样评估标准上的性能综合为单一标准化分数（Mi ∈ [0,1]），提供全局性能视角。

Result: 在FB15k-237和WN18RR等基准数据集上的实验表明，EDAS能有效整合多指标、多数据集性能，提供一致、稳健且可推广的评估框架。

Conclusion: EDAS为知识图谱补全模型评估提供了统一的元度量方法，支持更明智的模型选择，促进跨数据集评估的公平性，解决了传统评估方法的不一致性问题。
...
Method: 系统梳理当前大语言模型评测基准现状与发展，将283个代表性基准分为三类：通用能力基准（涵盖核心语言学、知识、推理等方面）、领域特定基准（聚焦自然科学、人文社科、工程技术等领域）、目标特定基准（关注风险、可靠性、智能体等）。

Result: 识别出现有基准存在数据污染导致分数膨胀、文化语言偏见造成评估不公、缺乏过程可信度和动态环境评估等问题。

Conclusion: 为未来基准创新提供了可参考的设计范式，强调需要解决当前基准存在的各种问题，以更全面、公平、可靠地评估大语言模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的可信度问题日益受到关注，现有评估方法往往只关注狭窄方面，忽视了多模态带来的风险，需要建立全面的评估框架。

Method: 提出了三维框架：五个可信度维度（真实性、鲁棒性、安全性、公平性、隐私性）；两种新型风险类型（多模态风险和跨模态影响）；多种缓解策略（数据、模型架构、训练、推理算法）。构建了包含32个任务和28个数据集的基准。

Result: 实验发现当前模型存在显著漏洞，可信度与通用能力之间存在差距，多模态训练和推理会放大基础LLM的潜在风险。现有缓解策略效果有限，多数方法在改善特定方面的同时会带来意外的性能权衡。

Conclusion: 提出了推理增强的安全对齐方法RESA，通过思维链推理能力发现潜在风险，取得了最先进的结果，为未来改进提供了实用见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法使用静态推测长度和刚性验证标准，无法适应不同模型不确定性和输入复杂性，限制了加速效果和适应性

Method: 利用起草模型输出分布的熵和边界不确定性度量，动态调整每轮推测生成的令牌数量，并使用相同置信度信号调制验证过程实现灵活令牌接受
...
<details>
  <summary>Details</summary>
Motivation: 研究大型词汇表对语言模型性能提升的真正原因，当前实践倾向于使用越来越大的词汇表但好处来源不明确

Method: 进行控制实验，将词汇表从24K扩展到196K，保持数据、计算和优化不变，量化token化文本复杂度，分析词级损失分解

Result: 更大的词汇表主要通过降低前2500个高频词的不确定性来减少交叉熵，尽管低频词的损失上升；约束嵌入范数会逆转收益

Conclusion: 词汇表大小的好处实质是降低token化文本复杂度，这为tokenizer-模型协同设计提供了简单原则性杠杆
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型的广泛采用，对其可靠性和可信度的担忧日益增长。目前基于证据的文本生成研究领域存在术语不一致、评估实践孤立和缺乏统一基准的问题。

Method: 系统分析134篇相关论文，引入基于证据的LLM文本生成的统一分类法，从七个关键维度研究300个评估指标，重点关注使用引用、归属或引文的证据生成方法。

Result: 建立了该领域的统一分类体系，识别了代表性方法，并深入分析了评估指标的多维度特征。

Conclusion: 该研究为基于证据的LLM文本生成领域提供了系统性的框架，指出了当前存在的挑战，并为未来研究方向提供了指导。
...
<details>
  <summary>Details</summary>
Motivation: 大型音频-语言模型在处理多模态输入方面表现出色，但其在音频与文本信息冲突时的表现尚未得到充分研究，需要系统评估其信息优先级选择能力。

Method: 构建MCR-BENCH基准，包含不一致的音频-文本对，通过广泛的音频理解任务评估模型表现，分析文本偏向性的影响因素，并探索通过监督微调等缓解策略。

Result: 发现LALMs存在显著的文本偏向性，在模态冲突时性能大幅下降，模型即使面对矛盾输入也表现出持续的过度自信。

Conclusion: 需要在训练过程中改进模态平衡，开发更复杂的融合机制来增强处理冲突多模态输入的鲁棒性，以提高实际应用的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 大规模语音语言模型发展受限于碎片化架构和缺乏透明度，模型权重发布时往往缺少对应的训练数据和配置，阻碍了系统比较和研究的可复现性。

Method: 提供三个核心资源：LLaSO-Align（1200万语音文本对齐语料）、LLaSO-Instruct（1350万多任务指令调优数据集）、LLaSO-Eval（可复现评估基准），并基于这些公开数据训练了38亿参数的LLaSO-Base参考模型。

Result: LLaSO-Base模型取得了0.72的标准化分数，超越了同类可比模型，建立了强大的可复现基线。分析显示更广泛的训练覆盖能提升性能，但在未见任务上仍存在显著泛化差距。

Conclusion: 通过完整发布数据、基准和模型，LLaSO建立了基础性开放标准，旨在统一研究努力并加速社区驱动的大规模语音语言模型发展。
...
<details>
  <summary>Details</summary>
Motivation: 语言模型通常基于敏感数据训练，可能记忆并泄露隐私信息，而当前对语言模型隐私风险及其缓解方法的理解仍然有限

Method: 对隐私保护语言建模方法进行系统性综述和分析，深入评估各种方法的优缺点

Result: 提供了隐私保护语言建模方法的全面概述，识别了现有方法的优势与局限性
...
<details>
  <summary>Details</summary>
Motivation: 心理健康障碍是全球危机，现有数据集在识别主动求助个体方面存在关键空白，需要专门针对求助行为的数据集。
...
<details>
  <summary>Details</summary>
Motivation: 消除语言理解中的误解，特别是在食物、服装、文化传统等领域的非对等词汇翻译问题，这些词汇在目标语言中往往没有直接对应词。

Method: 采用基于文献的研究方法，分析并提出了从源语言到目标语言翻译非对等词汇的不同方法和规则。

Result: 成功提出了翻译非对等词汇的专业方法，并将25个达里语和乌兹别克语的非对等词汇翻译成了英语和俄语。

Conclusion: 通过系统的方法和规则，可以有效解决非对等词汇的翻译问题，促进语言间的准确理解和交流，但仍有大量词汇需要进一步研究和翻译。
...
Result: 在具有挑战性的SGD基准测试中，PyTOD实现了最先进的对话状态跟踪性能，在准确性和鲁棒性用户目标估计方面均超越了强基线方法。

Conclusion: PyTOD展示了执行感知状态跟踪的有效性，通过代码生成和执行反馈机制显著提升了任务导向对话系统的性能。
...
TL;DR: RadReason是一个新颖的放射学报告评估框架，提供细粒度错误类型评分和人类可读的解释，超越了现有方法并达到GPT-4级别的评估性能。


<details>
  <summary>Details</summary>
Motivation: 当前放射学报告自动评估缺乏临床基础、可解释性和细粒度指标，现有方法要么评分粗糙，要么依赖黑盒模型，限制了在临床工作流程中的实用性。

Method: 基于Group Relative Policy Optimization，创新性地引入子分数动态加权（根据实时F1统计自适应优先处理临床挑战性错误类型）和多数引导优势缩放（基于子分数一致性调整策略梯度更新）。

Result: 在ReXVal基准测试中，RadReason超越了所有现有离线指标，达到与GPT-4评估相当的性能，同时保持可解释性、成本效益和临床部署适用性。

Conclusion: RadReason提供了一个临床基础、可解释且细粒度的放射学报告评估解决方案，解决了现有方法的局限性，具有重要的临床应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 个性化营销能显著提升客户参与度和业务增长，研究表明良好的个性化策略可使收入提升40%，但现有研究主要集中在推荐系统和个性化广告，个性化优惠生成具有巨大潜力

Method: 使用对比学习方法微调Google的T5-Small编码器-解码器语言模型，采用InfoNCE损失在共享嵌入空间中对齐客户画像和相关优惠，引入自适应学习行为重塑潜在空间

Result: 在模拟客户行为和优惠接受模式的合成数据集上评估，相比监督微调基线提高了17%的优惠接受率

Conclusion: 对比学习目标在推进个性化营销方面非常有效，SLM4Offer模型通过对比学习方法显著提升了个性化优惠生成的效果
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理用户主观行为和偏好时存在局限性，用户浏览行为形成的"浏览语言"具有高度异质性和主观性，需要专门的方法来有效捕捉这种多样性。

Method: 提出HeTLM（异质性感知语言模型训练）方法，使用页面级分词器训练小型语言模型，并采用基于聚类的异质性参数集来捕捉不同用户群体的行为特征。

Result: 实验表明：1）使用页面级分词器的小型模型优于大型预训练或微调模型；2）异质性聚类特定参数集的HeTLM优于同族单一模型；3）实现了更高的平均性能和更低的方差，表明对齐效果改善。

Conclusion: 针对主观浏览行为，小型专业化模型和异质性感知训练方法比通用大型语言模型更有效，能够更好地捕捉用户多样性和实现个性化对齐。
...
<details>
  <summary>Details</summary>
Motivation: 传统的课程学习在语言模型预训练中效果有限，主要是因为使用人类中心的难度度量可能无法准确反映模型训练过程中的实际难度。

Method: 提出使用训练数据影响力作为难度度量，根据训练样本对模型输出的影响程度来排序训练数据，从影响力小的样本开始训练。

Result: 使用该方法的模型在基准测试中比随机顺序训练的模型性能提升超过10个百分点。

Conclusion: 课程学习对语言模型预训练确实有益，但需要使用更符合模型视角的难度度量标准，而非传统的人类中心难度标准。
...
Result: 评估结果显示了SLM之间的多样化权衡关系，有些模型在准确性方面表现优异，而其他模型在能源效率方面更胜一筹。
...
<details>
  <summary>Details</summary>
Motivation: 现有身份检测数据集主要是英语中心、单标签且关注粗粒度身份类别，而社会身份通常由特定文化语境塑造并通过特定语言使用表达，需要针对希伯来语等非英语政治语境开发多标签身份检测资源。

Method: 构建包含5536个句子的希伯来语多标签语料库，手动标注12种基于调查数据的细粒度社会身份，对比多标签和单标签编码器以及2B-9B参数生成式LLM的性能。

Result: 希伯来语调优的LLM获得最佳性能（宏观F1=0.74），应用分类器分析政客Facebook帖子和议会演讲，发现身份表达的流行度、时间趋势、聚类模式和性别相关差异。

Conclusion: HebID为研究希伯来语社会身份提供了全面基础，可作为其他非英语政治语境类似研究的模型，实现了精英话语与公众身份优先级的比较分析。
...
Method: 采用离散扩散建模方法，通过简单的训练技术实现，包括基于自回归LLM的初始化和上下文自适应的token级噪声重调度。

Result: 在通用、数学和编程任务上一致优于现有扩散语言模型，展示了卓越的规划能力和推理灵活性，包括任意顺序生成、填充能力和可调节的质量-速度权衡。

Conclusion: Dream 7B是目前最强大的开放扩散大语言模型，其简单有效的训练技术为基于扩散的语言建模研究提供了新的方向，发布了Dream-Base和Dream-Instruct两个版本以促进进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 研究政治去合法化话语（PDD）的动机是为了系统分析对政治实体规范性有效性的象征性攻击，理解这种话语在民主话语中的模式和影响。

Method: 方法包括构建包含10,410个句子的希伯来语语料库，手动标注1,812个PDD实例，开发结合微调编码器模型和解码器LLM的两阶段分类管道（最佳模型DictaLM 2.0）。

Result: 最佳模型在二元PDD检测中达到F1分数0.74，在去合法化特征分类中达到宏观F1分数0.67。分析显示PDD在过去三十年中显著增加，社交媒体比议会辩论更普遍，男性政治家比女性使用更多，右倾行为者倾向更强。

Conclusion: 结论表明自动化PDD分析对于理解民主话语具有可行性和价值，能够揭示政治话语中的去合法化模式及其随时间、平台和政治倾向的变化。
...
Method: 通过协调7个专门代理构建自动化流水线，集成人类专业知识，确保过程可控性和成本可控性，构建低冗余、强判别力的安全基准。

Result: 构建的SafetyFlowBench数据集包含23,446个查询，评估了49个先进LLM的安全性，验证了系统的有效性和效率。

Conclusion: SafetyFlow提供了首个完全自动化的基准构建流水线，显著提升了LLM安全评估的效率和覆盖面，为模型漏洞发现提供了可靠工具。
...
TL;DR: 提出了"训练微型模型"概念，通过针对特定高价值应用微调小型语言模型，以极低成本生成领域特定的类似响应


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然擅长文本生成，但计算成本高昂，特别是在销售和营销等针对性应用中成本不可行

Method: 使用小型语言模型针对特定高价值应用进行微调，创建专门的"训练微型模型"

Result: 能够以极低成本生成与大型模型相似的领域特定响应

Conclusion: 训练微型模型为特定高价值应用提供了一种成本效益高的替代方案，解决了大型语言模型成本过高的问题
...
<details>
  <summary>Details</summary>
Motivation: 发现LLM在识别有害请求作为判别器时比作为生成器防御时更有效，存在安全不一致性，希望通过对齐模型的判别和生成能力来提升安全性。

Method: 提出SDGO框架，使用模型自身的判别能力作为强化学习的奖励信号，通过迭代自改进来优化生成安全性。

Result: SDGO显著提升了模型安全性，在保持通用基准上有用性的同时，对分布外越狱攻击表现出鲁棒性能。

Conclusion: 通过对齐判别和生成能力，SDGO实现了更紧密的能力耦合，仅需少量判别样本就能进一步增强生成能力，为LLM安全提供有效解决方案。
...
TL;DR: 提出了SurGE基准测试，用于评估科学综述生成任务，包含测试实例集和百万级学术语料库，并设计了自动化评估框架衡量四个维度，发现当前LLM在此任务上仍面临巨大挑战。


<details>
  <summary>Details</summary>
Motivation: 科学综述文章对总结研究进展至关重要，但由于学术文献快速增长，手动编写变得不可行。虽然大语言模型有自动化潜力，但缺乏标准化基准和评估协议阻碍了进展。

Method: 引入SurGE基准，包括：(1)测试实例集（主题描述、专家撰写综述及引用文献）(2)百万级学术语料库作为检索池。提出自动化评估框架，从信息覆盖度、引用准确性、结构组织和内容质量四个维度评估生成综述。

Result: 对多种基于LLM的方法评估表明，即使使用先进的自反思框架，综述生成任务仍然极具挑战性，凸显了该任务的复杂性和继续研究的必要性。

Conclusion: SurGE基准填补了科学综述生成评估的空白，展示了当前LLM在此任务上的局限性，为未来研究提供了标准化评估工具和方向。
...
<details>
  <summary>Details</summary>
Motivation: 位置偏差（PB）表现为不同上下文位置的非均匀敏感性，严重损害长上下文理解能力。现有方法通过修改架构来缓解PB，但显著的PB仍然存在，需要更有效的解决方案。

Method: 提出Pos2Distill框架，利用位置诱导的差异来对抗PB本身。针对检索和推理范式设计了两种专门实现：Pos2Distill-R¹和Pos2Distill-R²，通过知识蒸馏将优势位置的能力转移到劣势位置。
...
TL;DR: 本文对孟加拉语词干提取方法进行全面调查，指出该领域存在研究断层、实现稀缺和评估方法不足的问题，并针对孟加拉语丰富的形态变化和方言多样性提出了发展建议。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语作为全球第七大语言拥有3亿母语者，但在数字领域代表性不足，缺乏标注数据集资源。词干提取作为语言分析的关键预处理步骤，对低资源、高屈折性语言尤为重要，能显著降低算法复杂度。

Method: 通过文献综述方法，系统调查现有的孟加拉语词干提取方法，分析现有研究的局限性和实现的可复现性问题，并批判性地评估现有评估方法学。

Result: 发现孟加拉语词干提取研究存在显著空白，现有文献存在断层，可复现的实现稀缺，评估指标不够相关，难以应对孟加拉语丰富的形态变化和方言多样性挑战。

Conclusion: 建议开发更强大的孟加拉语词干提取器，推动该领域持续研究，以提升孟加拉语的语言分析和处理能力，并呼吁采用更相关的评估指标。
...
TL;DR: 提出了EcomMMMU电商多模态数据集和SUMEI方法，发现产品图像不一定提升性能，有时反而会降低效果，需要智能选择有用图像
...
Result: 分析发现产品图像并不总是提升性能，有时会降低性能，表明MLLMs难以有效利用丰富视觉内容；SUMEI方法在实验中表现出有效性和鲁棒性

Conclusion: 电商多模态任务中需要智能的图像选择策略，盲目使用所有图像可能适得其反；SUMEI方法为解决这一问题提供了有效方案
...
TL;DR: Deep-DxSearch是一个基于强化学习的医疗诊断RAG系统，通过端到端训练提升医疗大语言模型的诊断准确性和可追溯性


<details>
  <summary>Details</summary>
Motivation: 解决医疗大语言模型在诊断中存在知识鸿沟和幻觉问题，传统检索增强方法对外部知识利用不足且缺乏反馈推理可追溯性

Method: 构建大规模医疗检索语料库，将LLM作为核心智能体，检索语料库作为环境，通过强化学习训练智能体RAG策略，使用格式、检索、推理结构和诊断准确性等多维度奖励

Result: 在多个数据中心超越提示工程和无训练RAG方法，诊断准确性显著优于GPT-4o、DeepSeek-R1等强基线模型，在常见和罕见疾病诊断中均表现优异

Conclusion: 端到端的智能体强化学习训练框架有效提升医疗诊断性能，奖励设计和检索语料库组件对性能提升至关重要，为临床提供更可靠精确的初步诊断支持
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在推理任务上取得进展，但在精确计算方面仍有不足。工具集成推理(TIR)通过整合外部工具来增强推理，但其泛化效果和实际改进程度尚不明确。

Method: 提出了ReasonZoo基准测试，涵盖9个不同推理类别，并设计了两个新指标PAC和AUC-PCC来评估推理效率。

Result: TIR模型在数学和非数学任务中均优于非TIR模型，推理效率显著提升，表现为PAC和AUC-PCC指标的改善。

Conclusion: TIR具有领域通用的优势，能够提升大语言模型在复杂推理任务中的能力，实现更精简高效的推理过程。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估AI代理在真实动态场景中使用多样化MCP工具解决多步骤任务能力的基准测试

Method: 通过迭代LLM重写和人工审核精心策划101个真实查询，引入基于真实执行计划而非原始API输出的新颖评估方法

Result: 实验显示前沿LLM的成功率低于60%，详细消融和错误分析揭示了工具编排中的主要挑战和不同失败模式
...
TL;DR: 本文提出了一种替代Soft-argmax的新训练目标，在三个面部关键点检测基准上实现了最先进性能，训练收敛速度快2.2倍，同时保持更好/竞争性精度


<details>
  <summary>Details</summary>
Motivation: 热图回归方法在面部关键点检测中广泛使用，但argmax不可微分，传统使用Soft-argmax近似。作者重新审视这一长期选择，证明它不是实现强性能的唯一方法

Method: 基于经典结构化预测框架提出替代训练目标，不使用Soft-argmax近似，而是采用不同的可微分训练策略

Result: 在WFLW、COFW和300W三个面部关键点基准测试中达到最先进性能，训练收敛速度快2.2倍，精度保持更好或具有竞争力

Conclusion: Soft-argmax不是实现高性能面部关键点检测的唯一方法，提出的结构化预测框架提供了有效的替代方案，具有更快的训练速度和竞争性精度
...
<details>
  <summary>Details</summary>
Motivation: 传统卷积神经网络在处理复杂数据结构方面存在局限，而图卷积网络能够更好地建模关系数据。研究旨在通过结合Voronoi图和GCNs来提升图像分类性能，特别是在复杂场景和细粒度分类任务中。
...
TL;DR: YOPO是一个单阶段、基于查询的RGB-only框架，将9自由度姿态估计统一到2D检测中，无需额外数据即可在三个基准测试中达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 需要一种更简单的RGB-only替代方案，直接从类别级别学习，统一物体检测和9自由度姿态估计，无需伪深度、CAD模型或多阶段级联
...
Conclusion: YOPO证明了单阶段RGB-only框架能够以高性能统一物体检测和9自由度姿态估计，为机器人和自动化应用提供了更简洁有效的解决方案
...
TL;DR: 提出了一种统一的训练框架Paired-Sampling Contrastive Framework，能够同时检测物理呈现攻击和数字伪造攻击，在Face Anti-Spoofing挑战赛上达到2.10%的错误率，模型轻量且训练快速。


<details>
  <summary>Details</summary>
Motivation: 传统人脸识别系统使用分离的模型处理物理攻击和数字伪造攻击，增加了系统复杂度、推理延迟，且无法应对组合攻击向量。需要一种统一的解决方案来同时处理这两种攻击类型。

Method: 提出了配对采样对比框架，利用自动匹配的真实和攻击自拍对来学习模态无关的活体检测线索，实现统一的训练方法。

Result: 在第六届Face Anti-Spoofing挑战赛统一物理-数字攻击检测基准上，平均分类错误率(ACER)达到2.10%，优于之前的解决方案。模型计算量仅为4.46 GFLOPs，训练时间不到一小时。

Conclusion: 该框架提供了一种轻量级、高效的统一解决方案，能够同时应对物理呈现攻击和数字伪造攻击，具有实际部署的可行性。
...
TL;DR: TAIGen是一种无需训练的黑盒对抗图像生成方法，仅需3-20步采样即可从无条件扩散模型生成高质量对抗样本，比现有方法快10倍且攻击效果显著


<details>
  <summary>Details</summary>
Motivation: 现有生成模型的对抗攻击通常产生低质量图像且计算资源需求大，扩散模型虽然能生成高质量图像但需要数百步采样步骤，因此需要开发高效的对抗生成方法

Method: 在混合步骤区间注入扰动，采用选择性RGB通道策略：对红色通道应用注意力图，对绿色和蓝色通道使用GradCAM引导的扰动，保持图像结构的同时最大化目标模型的误分类

Result: 在ImageNet数据集上，TAIGen对VGGNet源模型的攻击成功率达到：ResNet 70.6%、MNASNet 80.8%、ShuffleNet 97.8%；PSNR超过30dB保持视觉质量；生成速度比现有扩散攻击快10倍

Conclusion: TAIGen实现了最低的鲁棒准确率，表明这是最具影响力的攻击方法，防御机制对其生成的图像净化效果最差
...
TL;DR: RUN++是一个可逆展开网络，通过生成式细化来解决隐蔽视觉感知任务，结合掩码域和RGB域的可逆建模，并使用针对性扩散模型优化不确定区域
...
Result: 该方法能够有效减少假阳性和假阴性，在真实世界退化条件下保持鲁棒性，计算效率高

Conclusion: RUN++提供了一个新的CVP系统构建范式，通过双级优化框架实现了掩码域和RGB域的可逆建模协同，显著提升了隐蔽目标感知性能
...
TL;DR: GasTwinFormer是一个混合视觉变换器，用于实时甲烷排放分割和饮食分类，在牛肉牛甲烷监测中达到74.47% mIoU和100%饮食分类准确率。
...
Result: 在11,694个标注帧的数据集上，达到74.47% mIoU和83.63% mF1的分割性能，仅3.348M参数、3.428G FLOPs和114.9 FPS推理速度，饮食分类准确率100%。
...
<details>
  <summary>Details</summary>
Motivation: 现有整流流模型使用线性轨迹，可能迫使图像生成过程通过数据流形的低概率区域，影响语义对齐和指令遵循能力

Method: 提出CurveFlow框架，将曲率指导直接融入流路径，采用曲率正则化技术惩罚轨迹内在动力学的突变

Result: 在MS COCO数据集上达到SOTA性能，在BLEU、METEOR、ROUGE和CLAIR等语义一致性指标上显著优于标准整流流变体和其他非线性基线

Conclusion: 曲率感知建模显著增强了模型忠实遵循复杂指令的能力，同时保持高质量的图像生成
...
TL;DR: HiRQA是一种自监督的无参考图像质量评估框架，通过分层排序和对齐学习实现质量感知嵌入，无需参考图像或主观标签，在合成和真实失真上均表现出色


<details>
  <summary>Details</summary>
Motivation: 解决无参考图像质量评估中数据集偏差和依赖主观标签导致的泛化性能问题

Method: 结合排序学习和对比学习的自监督框架，使用高阶排序损失、嵌入距离损失和对比对齐损失，仅需输入图像即可预测质量分数

Result: 在合成和真实失真基准测试中达到最先进性能，具有良好的泛化能力和实时部署能力（HiRQA-S变体每张图像仅需3.5ms）

Conclusion: HiRQA框架通过自监督学习有效解决了无参考图像质量评估的泛化问题，无需参考图像或主观标签，在多种失真条件下均表现优异
...
TL;DR: 本文提出了一种基于投资组合理论的边缘资源管理策略，用于在动态边缘环境中实现可靠的多视角3D重建，能够应对时空相关的系统中断问题。


<details>
  <summary>Details</summary>
Motivation: 多视角3D重建应用在紧急响应、战术场景等关键用例中需要快速态势感知，但边缘环境的动态性和操作逆境会导致时空相关的系统中断，影响相机操作和重建质量。

Method: 采用投资组合理论启发的边缘资源管理策略，通过遗传算法解决优化问题，选择最佳的相机组合来保证重建质量。

Result: 使用公开和定制3D数据集验证，该方法相比传统基线策略能够在时空中断下保证可靠的3D重建质量。

Conclusion: 提出的相机选择策略能够有效应对边缘环境中的时空相关中断，为关键应用提供可靠的多视角3D重建服务。
...
TL;DR: 提出XDR-LVLM框架，利用视觉语言大模型实现高精度糖尿病视网膜病变诊断，并提供自然语言解释，在准确性和可解释性方面都达到先进水平。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型在糖尿病视网膜病变诊断中的黑盒问题，缺乏透明度和可解释性阻碍了临床采用，需要既能准确诊断又能提供解释的系统。

Method: 整合专用医学视觉编码器和LVLM核心，采用多任务提示工程和多阶段微调，深入理解眼底图像病理特征并生成包含DR分级、关键病理概念识别和详细解释的综合诊断报告。

Result: 在DDR数据集上达到84.55%平衡准确率和79.92% F1分数，概念检测获得77.95% BACC和66.88% F1分数，人工评估确认生成解释具有高流畅性、准确性和临床实用性。

Conclusion: XDR-LVLM成功弥合了自动化诊断与临床需求之间的差距，通过提供稳健且可解释的洞察，展示了在医疗AI领域的重要应用价值。
...
TL;DR: MeSS方法利用城市网格模型作为几何先验，通过改进的图像扩散模型生成高质量、风格一致的室外场景纹理，解决了3D城市模型缺乏真实纹理的问题
...
Conclusion: MeSS方法成功解决了城市网格模型的纹理生成问题，为虚拟城市应用提供了高质量的3D场景合成解决方案
...
TL;DR: 本文提出了首个开源手术伤口数据集SurgWound，包含697张标注图像和8个临床属性，建立了手术伤口诊断基准，并开发了三阶段学习框架WoundQwen用于自动伤口分析和报告生成。
...
Method: 1) 创建SurgWound数据集，包含697张手术伤口图像，由3名专业外科医生标注8个细粒度临床属性；2) 建立包含视觉问答(VQA)和报告生成任务的基准；3) 提出三阶段学习框架WoundQwen：第一阶段用5个MLLM预测伤口特征，第二阶段用2个MLLM进行诊断评估，第三阶段整合结果生成综合报告。

Result: 开发了首个开源手术伤口数据集和诊断基准，提出了能够分析详细伤口特征并提供个性化护理指导的三阶段框架。

Conclusion: 该研究为个性化伤口护理、及时干预和改善患者预后奠定了基础，推动了手术伤口自动筛查工具的发展。
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶等安全关键应用中，现有强化学习方法使用基于规则的周围智能体来训练最优行为，但这些规则智能体的建模准确性至关重要

Method: 开发学习型对抗智能体，针对各种基于规则的行为建模策略和IDM模型，生成导致故障场景的对抗行为

Result: 评估显示对抗智能体能够有效降低所有基于规则智能体的累积奖励，证明其制造故障场景的能力

Conclusion: 该方法为安全关键系统中的规则智能体建模提供了有效的对抗测试手段，有助于识别和防范潜在的安全风险
...
<details>
  <summary>Details</summary>
Motivation: 现有肾脏病理数据集和自动化方法主要提供二元语义分割掩码，限制了形态学定量分析的精度，需要更有效的实例分割方法来处理肾脏组织中多样的形态和复杂连接性。

Method: 提出动态形态学引导的二元到实例分割管道，整合分水岭、骨架化和形态学操作，采用自适应几何细化和可定制超参数调优，通过系统参数优化分离粘连和异质结构。

Result: 实验结果表明该方法优于单个经典方法和简单组合，实现了更优的实例分离，支持更准确的肾脏病理形态学分析。

Conclusion: DyMorph-B2I为肾脏病理工作流程提供了强大的实例分割解决方案，能够处理复杂的组织结构，提升下游分析的准确性。
...
Result: 在三个公开数据集上实验表明，STAGNet在平均精度和平均碰撞时间指标上均优于现有方法，跨数据集测试也表现良好。

Conclusion: STAGNet模型证明了仅使用行车记录仪视频就能有效预测交通事故，为低成本、易部署的ADAS系统提供了可行方案。
...
Conclusion: TriMM通过有效利用多模态数据，在3D生成任务中实现了优异的性能，证明了多模态方法在3D内容建模中的巨大潜力
...
TL;DR: 提出一种面向中心的原型对比聚类框架，通过软原型对比模块和双重一致性学习模块解决类间冲突问题，提高聚类性能
...
Method: 软原型对比模块使用样本属于聚类中心的概率作为权重计算类别原型；双重一致性学习模块对齐同一样本的不同变换和不同样本的邻域，确保特征具有变换不变的语义信息和紧凑的类内分布
...
<details>
  <summary>Details</summary>
Motivation: 传统无人机视觉语言导航存在轨迹过长、机动性复杂的问题，需要人工干预或过于详细的指令。利用无人机高机动性提供多粒度视角的同时，保持可管理的运动空间进行学习。

Method: 提出双无人机协作框架AeroDuo：高空无人机使用多模态大语言模型(Pilot-LLM)进行目标推理，低空无人机采用轻量级多阶段策略进行导航和目标定位。两机仅交换最小坐标信息确保效率。

Result: 构建了HaL-13k数据集，包含13,838条协作高低空无人机演示轨迹，每对轨迹配有目标导向的语言指令，包含未见地图和未见对象验证集以评估泛化能力。

Conclusion: DuAl-VLN任务和AeroDuo框架通过高低空无人机协作，有效解决了无人机视觉语言导航的挑战，提供了系统性的评估方法和解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然在各生成任务中达到最先进效果，但其顺序生成过程需要长序列逐步生成，效率较低。现有方法如DDIM通过非马尔可夫过程减少采样步骤，但缺乏对原始扩散过程能否以马尔可夫方式实现相同效率的理解。

Method: 提出跳过步采样机制，在迭代生成过程中绕过多个中间去噪步骤，而不是传统的逐步细化。该方法从与标准扩散模型相同的训练目标推导而来，表明通过马尔可夫方式的加速采样是预训练扩散模型的内在属性。

Result: 在OpenAI ADM、Stable Diffusion和Open Sora等流行预训练扩散模型上的大量实验表明，该方法能够以显著减少的采样步骤实现高质量生成。

Conclusion: 跳过步采样是一种有效的加速扩散模型生成的方法，它证明了通过马尔可夫方式实现高效采样是扩散模型的内在能力，并且可以与DDIM等现有方法结合进一步提升性能。
...
Method: 提出三方面创新：多功能编码框架统一不同编码模式；交互式编码代理通过增强上下文学习方法训练LLM；建立首个专用基准测试IIC-bench

Result: Comp-X能高效理解编码请求并展现出色的文本交互能力，在单一编码框架下仍保持可比的压缩性能

Conclusion: 该方法为图像压缩中的人工通用智能提供了有前景的发展方向，推动了图像编码范式的演进
...
TL;DR: Ano-NAViLa是一种基于预训练视觉语言模型的病理图像异常检测方法，通过融入正常和异常病理知识，在计算效率和可解释性方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法主要针对工业场景设计，在病理图像中面临计算限制、组织结构多样性和缺乏可解释性等挑战，特别是在疾病相关数据有限的情况下。

Method: 基于预训练视觉语言模型构建，加入轻量级可训练MLP，融合正常和异常病理知识，通过图像-文本关联提供可解释性。

Result: 在两个不同器官的淋巴结数据集上评估，Ano-NAViLa在异常检测和定位方面达到最先进性能，优于竞争模型。

Conclusion: Ano-NAViLa通过知识增强的视觉语言模型有效解决了病理图像异常检测的挑战，在准确性和鲁棒性方面表现优异，同时提供良好的可解释性。
...
TL;DR: RATopo提出了一种冗余分配策略，通过改进Transformer解码器结构和一对多分配机制，增强车道拓扑推理的监督效果。


<details>
  <summary>Details</summary>
Motivation: 现有车道拓扑推理方法采用先检测后推理的范式，基于一对一分配的监督策略导致监督范围有限，拓扑推理性能次优。

Method: 交换Transformer解码器的交叉注意力和自注意力层，保留冗余车道预测；实例化多个并行交叉注意力块以增强车道检测多样性；实现有效的一对多分配。

Result: 在OpenLane-V2数据集上的大量实验表明，RATopo策略具有模型无关性，可无缝集成到现有拓扑推理框架中，持续提升车道-车道和车道-交通拓扑性能。

Conclusion: RATopo通过冗余分配策略实现了数量丰富和几何多样的拓扑监督，显著改善了车道拓扑推理的性能。
...
TL;DR: 利用CLIP模型开发DesignCLIP框架，通过多模态方法提升设计专利分析的准确性和可靠性，在专利分类和检索任务上优于现有方法


<details>
  <summary>Details</summary>
Motivation: 传统设计专利分析依赖图像数据，但专利草图往往缺乏充分的视觉上下文和语义信息，导致在先技术搜索中存在模糊性。视觉语言模型的发展为更可靠的AI驱动专利分析提供了机会

Method: 开发DesignCLIP统一框架，采用类感知分类和对比学习，利用生成的详细图像标题和多视图图像学习来处理专利数据的独特特征

Result: DesignCLIP在专利分类和检索等下游任务中始终优于基线和最先进模型，多模态专利检索为设计创新提供了更多样化的灵感来源

Conclusion: 多模态方法在推进专利分析方面具有巨大潜力，DesignCLIP框架展示了视觉语言模型在设计专利领域的应用价值
...
TL;DR: TPA是一种利用基础图像-文本模型和提示感知对比学习的胎儿先天性心脏病超声视频分类方法，整合了时间建模和不确定性量化，在CHD检测和心脏功能评估中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前超声视频中的先天性心脏病检测面临图像噪声和探头位置变异性的挑战，现有机器学习方法往往忽略时间信息、仅限于二元分类且缺乏预测校准，需要更可靠的自动化诊断方法。

Method: 提出时间提示对齐(TPA)方法：使用图像编码器提取视频子片段每帧特征，通过可训练时间提取器聚合特征捕捉心脏运动，利用边界铰链对比损失将视频表示与类别特定文本提示对齐；引入条件变分自编码器风格调制(CVAESM)模块学习潜在风格向量来调制嵌入并量化分类不确定性。

Result: 在CHD检测私有数据集和EchoNet-Dynamic公共数据集上，TPA获得85.40%的宏F1分数用于CHD诊断，同时将预期校准误差降低5.38%，自适应ECE降低6.8%；在三分类任务中宏F1提升4.73%（从53.89%到58.62%）。
...
TL;DR: 本文提出了BasketLiDAR数据集和新型MOT框架，结合LiDAR点云和多视角相机数据，解决了篮球运动中实时3D轨迹追踪的挑战，实现了更高的追踪精度和更低的计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统多相机系统受限于2D视频数据和复杂3D重建处理，难以实现实时分析。篮球运动中的快速移动、复杂轨迹和频繁遮挡使得实时3D追踪特别困难。

Method: 构建BasketLiDAR多模态数据集（LiDAR点云+多视角相机），开发基于LiDAR高精度3D空间信息的MOT算法，包括纯LiDAR实时追踪管道和LiDAR-相机融合的多模态追踪管道。

Result: 方法实现了实时操作（传统相机方法难以达到），在遮挡条件下仍保持优越的追踪性能，包含4,445帧和3,105个球员ID的完整同步数据。

Conclusion: LiDAR与相机融合的多模态方法有效解决了篮球运动中的实时3D追踪难题，为体育分析提供了新的技术路径，数据集可供研究使用。
...
TL;DR: 提出RAG-SEG训练免费范式，通过检索增强生成粗掩码作为提示，再用SAM进行精分割，无需传统训练即可在个人笔记本上实现竞争性伪装目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 解决伪装目标检测中目标与背景高度相似、现有方法需要大量训练和计算资源、基础模型需要高质量提示但手动生成成本高的问题。

Method: 两阶段方法：第一阶段通过无监督聚类构建检索数据库进行特征检索生成粗掩码作为提示；第二阶段使用SAM2进行精确掩码生成和精炼。

Result: 在基准COD数据集上表现与或超越最先进方法，所有实验均在个人笔记本上完成，证明了计算效率和实用性。

Conclusion: RAG-SEG提供了一种无需训练的高效伪装目标检测解决方案，在保持竞争性能的同时显著降低了计算资源需求。
...
TL;DR: VideoEraser是一个无需训练的框架，通过选择性提示嵌入调整和抗干扰噪声引导，有效防止文本到视频扩散模型生成不良内容，在多个擦除任务中平均减少46%的不良内容生成。


<details>
  <summary>Details</summary>
Motivation: 文本到视频扩散模型的快速发展引发了隐私、版权和安全担忧，这些模型可能被滥用来生成有害或误导性内容，且训练数据中可能包含未经授权的个人身份、艺术创作和有害材料。
...
TL;DR: 本研究开发了一个端到端的深度学习框架，用于预测行人过马路意图，通过姿态检测和序列建模技术，发现1D CNN在速度上表现最佳，GRU在预测准确性上优于LSTM。
...
Result: GRU在预测行人过马路意图方面优于LSTM模型，而1D CNN在速度方面表现最佳。

Conclusion: 研究成功开发了一个有效的端到端框架，能够准确预测行人过马路意图，为自动驾驶系统提供了重要的安全保障。
...
TL;DR: RCDINO是一个基于transformer的多模态3D目标检测模型，通过融合DINOv2预训练模型的语义丰富表示来增强视觉特征，在nuScenes数据集上取得了雷达-相机模型中最先进的性能
...
Method: 提出RCDINO模型，使用基于transformer的架构，将预训练的DINOv2基础模型的语义丰富表示与视觉骨干网络特征进行融合，增强视觉表示能力

Result: 在nuScenes数据集上达到56.4 NDS和48.1 mAP的state-of-the-art性能，在雷达-相机模型中表现最佳

Conclusion: 通过融合DINOv2的语义表示能有效提升3D目标检测性能，同时保持与基线架构的兼容性，为多模态融合提供了有效解决方案
...
TL;DR: 本文通过注意力敲除技术系统分析Video-LLMs的内部机制，发现视频信息处理呈现两阶段模式，中间层是关键异常点，且时空建模更多依赖语言引导而非视频token自注意力。
...
Result: 发现视频信息提取主要发生在早期层，形成感知编码和抽象推理的两阶段过程；某些中间层对视频问答有异常大的影响；时空建模更依赖语言引导检索而非计算成本高的视频token自注意力。
...
<details>
  <summary>Details</summary>
Motivation: 传统微调方法需要更新所有模型参数，可能导致过拟合和较高计算成本，需要一种更高效的迁移学习策略。

Method: 使用进化算法识别出需要微调的特定层集合，只对这些层进行参数更新，减少可训练参数数量。

Result: 在9个不同领域的图像分类数据集上评估，BioTune在准确性和效率方面都优于或与AutoRGN和LoRA等现有方法相当。

Conclusion: 通过聚焦于相关层的子集进行微调，BioTune降低了计算成本，促进了跨不同数据特征和分布的更高效迁移学习。
...
TL;DR: 提出ICGS-Quantizer方法，通过联合利用高斯间和属性间相关性，使用跨场景共享码本，将3DGS压缩到千字节级别，并支持基于图像的场景更新解码。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS压缩方法只能将中等规模场景压缩到兆字节级别，不适用于大规模场景或场景集合的长期归档，且缺乏对归档后场景变化的适应机制。

Method: 提出图像条件高斯溅射量化器，联合利用高斯间和属性间相关性，使用跨训练场景的共享码本，编码、量化和解码过程联合训练，支持基于捕获图像的场景条件解码。

Result: ICGS-Quantizer在压缩效率和场景变化适应性方面持续优于最先进方法，将3DGS存储需求降低到千字节范围同时保持视觉保真度。

Conclusion: 该方法有效解决了3DGS长期归档的存储限制和场景变化适应问题，为大规模场景集合的压缩和更新提供了实用解决方案。
...
TL;DR: DriveSplat提出了一种基于神经高斯表示的动态-静态解耦方法，用于驾驶场景的高质量3D重建，通过区域化体素初始化、可变形神经高斯和深度/法线先验监督，在Waymo和KITTI数据集上实现了最先进的性能。
...
Method: 采用区域化体素初始化方案将场景分为近、中、远区域；引入可变形神经高斯建模非刚性动态物体，通过可学习变形网络进行时间参数调整；使用预训练模型的深度和法线先验监督整个框架。
...
TL;DR: 本文针对深度学习模型在抽象推理方面的瓶颈，通过Raven渐进矩阵问题研究如何提升机器智能的抽象推理能力。基于因果链建模视角分析RPM任务，设计了DIO基线模型，但发现互信息最大化目标存在局限，进而提出了三种改进方法。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型在抽象推理方面存在根本性瓶颈，Raven渐进矩阵问题被作为评估抽象推理能力的权威基准。本文旨在通过解决RPM问题来提升机器智能的抽象推理能力。

Method: 采用因果链建模视角分析RPM任务的完整因果链，设计DIO基线模型架构。针对互信息最大化目标的局限性，提出了三种渐进式改进方法。

Result: 实验发现基于互信息最大化的优化目标无法让模型真正掌握预设的人类推理逻辑，主要原因是下界紧致性影响优化效果，且互信息无法捕捉因果关系。

Conclusion: 需要超越统计相关的互信息最大化方法，采用更能捕捉因果关系的改进方法来提升模型的抽象推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管MAE在视觉基础模型预训练中表现强大，但在应用于新数据集时需要大量超参数调优，且MAE超参数与下游任务性能之间的关系尚未充分探索。

Method: 通过分析推导线性MAE学习的特征，并将分析扩展到非线性MAE，研究MAE表示如何适应数据集中的空间相关性。

Result: 研究发现掩码比例和补丁大小可以控制MAE学习短程和长程空间相关性的能力，MAE表示能够适应超出二阶统计量的数据集空间相关性。
...
TL;DR: 提出BIRD方法，一种双向时序信息传播的红外小目标检测算法，通过局部和全局时序信息融合，在保持快速推理的同时达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的多帧方法采用滑动窗口方式聚合相邻帧信息，但忽略了滑动窗口外的全局时序信息，导致计算冗余和次优性能

Method: 设计双向传播策略，包含局部时序运动融合模块(LTMF)建模相邻帧时空依赖，全局时序运动融合模块(GTMF)聚合全局传播特征，并通过时空融合损失联合优化整个视频片段

Result: 大量实验证明BIRD方法不仅达到了最先进的性能，而且显示出快速的推理速度

Conclusion: BIRD方法通过双向时序信息传播有效解决了红外小目标检测中的全局时序信息利用问题，在性能和效率方面都有显著提升
...
<details>
  <summary>Details</summary>
Motivation: 传统汽车损伤检测方法劳动密集、手动操作且经常忽略微小表面缺陷，需要更快速精确的自动检测技术来满足汽车保险评估和检测需求。

Method: 使用YOLOv8目标检测框架，创建包含不同光照条件、角度和纹理的定制数据集，采用实时数据增强技术训练YOLOv8m模型及其定制变体YOLOv8m-t4和YOLOv8m-t42。

Result: YOLOv8m-t42模型表现最佳，精度0.86、召回率0.84、F1分数0.85，mAP@0.5达到0.60，PR曲线面积0.88，显示出优异的检测性能和实时应用潜力。

Conclusion: 基于YOLOv8的深度学习模型能够有效检测汽车表面微小凹陷缺陷，YOLOv8m-t42变体在精度和一致性方面表现最优，适合实际汽车检测应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 视频到视频时刻检索任务面临语义帧级对齐和复杂依赖关系建模的挑战，需要开发能够精确捕捉语义上下文和时间细节的新方法。
...
Conclusion: MATR通过有效的序列对齐和自监督预训练，在视频到视频时刻检索任务上取得了显著的性能提升，证明了其在捕捉语义匹配和时间定位方面的有效性。
...
TL;DR: 提出了一种无需SfM的3D高斯泼溅方法，能够在极稀疏视图（仅2张训练视图）下联合估计相机位姿并重建3D场景，显著提升了渲染质量。


<details>
  <summary>Details</summary>
Motivation: 传统3DGS方法严重依赖密集多视图输入和精确的相机位姿，但在现实场景中很难获得。当输入视图极其稀疏时，基于SfM的初始化方法无法准确重建3D几何结构，导致渲染质量下降。

Method: 1）提出密集立体模块替代SfM，渐进估计相机位姿并重建全局密集点云进行初始化；2）设计一致性视图插值模块，基于训练视图对插值相机位姿并生成视角一致的内容作为额外监督信号；3）引入多尺度拉普拉斯一致性正则化和自适应空间感知多尺度几何正则化来提升几何结构和渲染内容质量。

Result: 在极稀疏视图条件下（仅2张训练视图），PSNR指标相比现有最优3DGS方法提升了2.75dB，合成的图像失真最小且保留了丰富的高频细节，视觉质量显著优于现有技术。

Conclusion: 该方法成功解决了3DGS在极稀疏视图下的性能瓶颈问题，无需SfM初始化即可实现高质量的3D场景重建和新视角合成，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像分割中轻量级模型性能与效率的平衡问题，现有模型往往为了效率牺牲性能，且缺乏全局上下文感知能力，同时忽略了相同卷积核下的通道冗余问题。

Method: 提出LGMSNet框架，采用异构层内核提取局部高频信息并缓解通道冗余，集成稀疏Transformer-卷积混合分支捕获低频全局信息。
...
TL;DR: MExECON是一个从稀疏多视角RGB图像进行3D穿衣人体重建的新方法，通过多视角联合优化和法线图集成，显著提升了单视角方法的几何精度和细节表现。


<details>
  <summary>Details</summary>
Motivation: 现有的单视角3D人体重建方法（如ECON）在几何精度和姿态估计方面存在局限，需要利用多视角信息来提升重建质量，同时保持无需重新训练的优势。

Method: 提出了联合多视角身体优化（JMBO）算法，跨所有输入视图拟合单个SMPL-X身体模型，确保多视角一致性；使用优化后的身体模型作为低频先验，通过前后视角法线图集成添加几何细节。

Result: 实验结果表明，MExECON相比单视角基线 consistently 提升保真度，与现代少样本3D重建方法相比具有竞争力。

Conclusion: MExECON成功扩展了单视角方法到多视角场景，无需重新训练即可实现高质量的多视角3D人体重建，在几何细节和姿态估计方面均有显著改进。
...
TL;DR: AdaSFFuse是一个用于多模态图像融合的新型框架，通过自适应跨域协同融合学习解决模态不对齐、高频细节破坏等问题，在多个融合任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法使用生成先验作为伪真值，但往往导致几何一致性差和渲染过度平滑的问题，需要一种更全面的解决方案来改善外推视图的质量。
...
TL;DR: D3FNet是一种用于高分辨率遥感影像中细粒度窄路分割的深度学习网络，通过差分注意力扩张提取和双流解码融合机制，显著提升了窄路的提取精度和连续性
...
Conclusion: D3FNet为复杂遥感和协同感知场景中的细粒度窄路提取提供了一个鲁棒的解决方案，证明了其在处理细粒度、遮挡和低对比度道路段方面的有效性
...
Method: 将测试时自适应重新构建为高斯概率推理任务，使用逐步更新的类别均值和共享协方差矩阵建模类别条件似然。引入基于CLIP先验和历史知识库的轻量级正则化来纠正可能的似然偏差。无需源数据、梯度更新或完整访问目标数据。

Result: 在多种基准测试上的广泛实验表明，该方法在各种分布偏移下实现了最先进的性能，具有优越的可扩展性和鲁棒性。

Conclusion: ADAPT提供了一种高效、可扩展的测试时自适应解决方案，通过概率推理框架和轻量级正则化机制，在保持高性能的同时解决了现有方法的局限性。
...
TL;DR: 提出两阶段训练策略解决INR频谱偏差问题，通过邻域感知软掩码对高频细节像素赋予更高权重，改善图像重建质量
...
Method: 采用两阶段训练策略：第一阶段使用邻域感知软掩码自适应地为局部变化强烈的像素分配更高权重，鼓励模型早期关注细节；第二阶段进行全图像训练
...
<details>
  <summary>Details</summary>
Motivation: 现有的半定规划松弛方法求解点云配准问题需要数百秒，计算效率低下，需要开发更快速的全局最优求解方法。

Method: 提出线性时间凸松弛方法和contractor方法加速分支定界算法，在给定旋转轴的情况下求解截断最小二乘问题。

Result: 算法能在0.5秒内求解100个点的3D点云配准问题，比STRIDE SDP求解器快两个数量级，并能处理高达95%的异常值。
...
Method: 在尼泊尔奇特旺国家公园及周边地区，使用可见光/近红外相机陷阱和热红外无人机采集图像，构建深度学习模型（YOLOv11s和增强版Faster RCNN）进行自动识别，并进行空间模式分析。

Result: YOLOv11s模型在相机陷阱图像中表现最佳（精度96.2%，召回率92.3%），无人机热成像提供了补充的空中视角。空间分析识别出明显的野生动物和人类活动热点区域及其重叠模式。
...
TL;DR: Grounded VideoDiT是一个视频大语言模型，通过扩散时序潜在编码器、对象接地表示和混合token方案，解决了现有视频LLM在时间感知方面的局限性，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频LLM在时间感知方面存在不足：时间戳仅隐式编码、帧级特征连续性差、语言视觉对齐容易漂移，需要更精确的时间定位和实体交互理解能力。

Method: 1) 扩散时序潜在编码器增强边界敏感性和时间一致性；2) 对象接地表示将查询实体显式绑定到局部视觉证据；3) 混合token方案使用离散时间token进行显式时间戳建模。

Result: 在Charades STA、NExT GQA和多个VideoQA基准测试中取得了最先进的性能，验证了模型的强大接地能力。

Conclusion: Grounded VideoDiT通过三个关键创新解决了视频LLM的时间感知问题，为细粒度时间推理提供了有效解决方案，在多个任务上表现出色。
...
TL;DR: 提出弱监督方法改进机载激光扫描数据的树木实例分割，通过人工质量评估训练评分模型，再反馈优化分割模型，提升34%的正确识别率


<details>
  <summary>Details</summary>
Motivation: 机载激光扫描数据中的树木实例分割对森林监测至关重要，但面临数据变化大和精确标注成本高的问题

Method: 使用非微调模型或闭式算法获得初始分割结果，人工进行质量评级，训练评分模型分类分割质量，然后用评分模型反馈微调分割模型

Result: 方法将正确识别的树木实例数量提升34%，同时显著减少非树木实例的误判

Conclusion: 该方法有效解决了树木分割问题，但在稀疏林区小树木和复杂环境中的性能仍有挑战
...
Method: 基于不同DNN架构使用一致关键特征的观察，提出关键特征引导(CFG)方法，通过计算特征重要性来正则化对抗点云搜索，优先破坏跨架构通用的关键特征，并在损失函数中约束最大偏差以保证不可感知性

Result: 在ModelNet40和ScanObjectNN基准数据集上的大量实验表明，CFG方法大幅优于现有最先进的攻击方法

Conclusion: CFG方法通过关键特征引导有效提升了3D点云对抗样本的迁移攻击能力，为黑盒场景下的3D模型安全评估提供了有效工具
...
TL;DR: MapKD是一个新颖的多层次跨模态知识蒸馏框架，通过Teacher-Coach-Student范式将多模态教师模型的知识转移到轻量级视觉学生模型，显著提升在线高精地图构建性能。


<details>
  <summary>Details</summary>
Motivation: 现有在线高精地图构建方法依赖离线地图先验和多模态传感器，导致推理时计算开销大且受限于过时的离线地图。需要开发更高效、低成本的视觉中心解决方案。
...
Conclusion: MapKD通过知识蒸馏成功实现了从多模态模型到轻量级视觉模型的有效知识转移，为自动驾驶系统提供了更高效的在线高精地图构建解决方案。
...
TL;DR: 提出CM2LoD3方法，利用冲突映射从射线到模型先验分析中重建LoD3建筑模型，通过语义分割和纹理融合提高3D重建精度
...
Method: 使用语义冲突映射生成器(SCMG)生成合成冲突映射，进行语义分割，并将纹理模型的分割结果与冲突映射通过置信度分数融合

Result: 实验结果显示方法能有效分割和重建建筑开口，通过不确定性感知的纹理融合达到61%的性能
...
TL;DR: 提出了MDPR框架来解决预训练视觉语言模型在类别不平衡场景下微调时的偏差问题，通过多维动态提示路由机制提升下游任务的性能


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型微调方法在类别不平衡场景下存在偏差，且往往忽略了预训练阶段固有的类别不平衡问题，导致下游任务中的偏差累积
...
Method: 提出StreamMem机制，以流式方式编码新视频帧，使用视觉token和通用查询token之间的注意力分数压缩KV缓存，并维护固定大小的KV内存。
...
TL;DR: WorldWeaver是一个用于生成长视频的框架，通过联合建模RGB帧和感知条件来解决长序列中的结构一致性问题，利用深度线索和分段噪声调度来减少时间漂移。


<details>
  <summary>Details</summary>
Motivation: 当前生成视频建模方法主要依赖RGB信号，在长序列中会导致对象结构和运动的累积错误，需要解决时间一致性问题。

Method: 提出WorldWeaver框架，联合预测感知条件和颜色信息；利用深度线索构建记忆库；采用分段噪声调度训练预测组。

Result: 在扩散模型和整流流模型上的大量实验证明，WorldWeaver能有效减少时间漂移，提高生成视频的保真度。

Conclusion: 该框架通过统一的长时域建模方案显著增强了时间一致性和运动动态，同时降低了计算成本。
...
TL;DR: 提出了分子赋能的All-in-SAM模型，通过结合分子信息和视觉基础模型，在计算病理学中实现更精确的细胞分割和分类，同时减少标注工作量。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉基础模型（如SAM）在细粒度语义分割方面存在局限，特别是在识别特定细胞亚型时表现不佳，需要开发更精确且标注效率更高的方法。

Method: 采用全栈方法：1）分子赋能的标注减少像素级标注需求；2）使用SAM适配器增强模型语义理解能力；3）整合分子导向校正学习（MOCL）提升分割精度。

Result: 在内部和公共数据集上的实验表明，该模型显著提高了细胞分类性能，即使在标注质量不同的情况下也能保持良好表现。
...
TL;DR: Waver是一个高性能的多模态生成基础模型，能够统一进行图像和视频生成，支持文本到视频、图像到视频和文本到图像生成，在多个基准测试中排名前三。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有视频生成模型在运动捕捉、时间一致性和分辨率方面的局限性，开发一个统一的框架来同时处理图像和视频生成任务。

Method: 采用混合流DiT架构增强模态对齐和加速训练收敛，建立全面的数据筛选流程，使用MLLM视频质量模型过滤高质量样本，并提供详细的训练和推理方案。

Result: 能够直接生成5-10秒的720p原生分辨率视频（可上采样至1080p），在Artificial Analysis的T2V和I2V排行榜中均排名前三，超越现有开源模型并匹配或超越商业解决方案。
...
TL;DR: ATLAS是一个从60万高分辨率扫描数据学习的高保真人体模型，通过显式解耦形状和骨骼基础，解决了现有参数化人体模型在多样姿态和形状细节捕捉方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有参数化人体模型由于训练数据多样性不足和建模假设限制，难以捕捉不同身体姿态和形状的细节变化，且内部骨骼与外部软组织之间存在问题依赖关系，限制了直接控制身体高度和骨骼长度的能力。

Method: 使用240个同步相机捕获的60万高分辨率扫描数据学习，显式解耦形状和骨骼基础，将网格表示基于人体骨骼，实现形状和骨骼的分离建模。
...
Method: 提出SceneGen框架，包含新颖的特征聚合模块整合视觉和几何编码器的局部和全局场景信息，通过位置头在单次前向传播中生成3D资产及其相对空间位置。

Result: 广泛的定量和定性评估证实了方法的效率和鲁棒生成能力，在单图像训练基础上可扩展到多图像输入场景。
...
TL;DR: VAREdit是一个基于视觉自回归模型的图像编辑框架，通过序列化多尺度特征生成实现精确编辑，解决了扩散模型全局去噪导致的编辑偏差问题


<details>
  <summary>Details</summary>
Motivation: 扩散模型在指令引导图像编辑中存在全局去噪过程导致编辑区域与整个图像上下文纠缠，产生意外修改和编辑指令遵循度不足的问题

Method: 提出VAREdit框架，将图像编辑重构为下一尺度预测问题，使用尺度对齐参考(SAR)模块注入尺度匹配的源图像条件信息到自注意力层

Result: 在标准基准测试中，GPT-Balance分数比领先的扩散方法高出30%+，512×512图像编辑仅需1.2秒，比同类方法快2.2倍

Conclusion: 自回归模型通过序列化和组合机制为图像编辑提供了更精确和高效的解决方案，VAREdit在编辑准确性和速度方面都取得了显著进步
...
<details>
  <summary>Details</summary>
Motivation: 传统生成模型独立采样导致输出冗余，限制了用户选择和创新探索。实际应用中用户通常需要一组多样化的输出结果。
...
TL;DR: CineScale是一种新颖的推理范式，无需微调即可实现高分辨率视觉生成，支持8K图像和4K视频生成


<details>
  <summary>Details</summary>
Motivation: 现有视觉扩散模型受限于训练分辨率和计算资源，无法生成高质量高分辨率内容，容易出现重复模式问题

Method: 提出针对不同视频生成架构的专用变体，基于开源视频生成框架实现高分辨率I2V和V2V合成

Result: 实验验证了该范式在扩展高分辨率视觉生成能力方面的优越性，实现8K图像生成和4K视频生成

Conclusion: CineScale成功解决了高分辨率视觉生成中的重复模式问题，扩展了预训练模型的高分辨率生成能力
