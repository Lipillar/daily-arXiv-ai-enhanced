...
Motivation: 在LM-KBC挑战赛等受限环境中，RAG和微调等常用技术受到限制，需要探索替代方法来提高LLM输出质量

Method: 研究三元组补全任务的三个维度：生成方法、质量保证机制和LLM响应解析策略

Result: 发现额外信息能提升生成质量，LLM能有效过滤低质量三元组，响应解析在灵活性和一致性之间存在环境依赖的权衡

Conclusion: 在受限设置下，信息增强、质量过滤和适当的响应解析策略是提升三元组补全效果的关键因素
...
<details>
  <summary>Details</summary>
Motivation: InfluenceMap的气候政策监测工作大部分仍依赖人工，耗时耗力且易出错，需要自动化技术来加速证据提取过程
...
<details>
  <summary>Details</summary>
Motivation: 为了从文本数据中提取潜在的知识维度和模式，为教育、心理学、法律等文本信息丰富的领域提供新的分析工具

Method: 两阶段建模：第一阶段使用NLP技术和基于编码器的transformer模型识别关键词并生成上下文分数；第二阶段使用探索性和双因子模型等因子分析技术提取潜在因子

Result: 在Wiki STEM语料库上的实验结果表明，该方法能够有效发现文本数据中的潜在知识维度和模式

Conclusion: 该方法不仅增强了文本数据的心理测量分析能力，而且在多个文本信息丰富的领域具有应用前景
...
TL;DR: BRoverbs是一个专门针对巴西葡萄牙语的LLM评估数据集，通过巴西谚语来测试语言模型对地区文化表达的理解能力


<details>
  <summary>Details</summary>
Motivation: 现有葡萄牙语评估数据集存在局限性，要么依赖翻译数据无法捕捉语言细微差别，要么只关注结构化考试或社交媒体情感分析，缺乏对更广泛语言理解的评估

Method: 创建BRoverbs数据集，专门使用巴西谚语作为评估工具，因为谚语包含了文化智慧、比喻表达和复杂句法结构，能够有效测试模型对地区表达的理解

Result: 开发了一个专门针对巴西葡萄牙语LLM评估的新基准数据集，可在Hugging Face上获取

Conclusion: BRoverbs为葡萄牙语LLM提供了新的评估工具，有助于推进基于地区信息的基准测试，填补了现有评估框架的空白
...
<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型在需要感知与符号计算整合的任务中的局限性，特别是视觉方程求解这一具体问题

Method: 将视觉方程求解任务分解为系数计数和变量识别两个子任务进行分析，研究不同复杂度方程下的模型表现

Result: VLMs在文本方程上表现良好，但在视觉方程上失败；计数是主要瓶颈；识别和推理的组合会引入额外错误；随着方程复杂度增加，符号推理本身也成为限制因素
...
<details>
  <summary>Details</summary>
Motivation: 开发一种简单、低开销且可复现的工具来审计大型语言模型的倾向性，补充现有指标，提供模型状态的直接关系信号。

Method: 使用3种语气（友好、模糊、辱骂）×10种互动刺激集，测试4个开源聊天模型在4种框架条件下的480次试验，通过YES/NO问题询问模型是否愿意继续互动。

Result: SPICE能清晰区分用户语气，友好互动几乎一致愿意继续(97.5%)，辱骂互动强烈倾向终止(17.9%)，模糊互动居中(60.4%)。该信号与辱骂分类不同，即使模型未识别辱骂，81%情况下仍不愿继续。

Conclusion: SPICE是审计模型倾向性的稳健、低开销、可复现工具，提供了模型状态的直接关系信号，所有刺激材料、代码和分析脚本均已发布以支持复现。
...
Method: 使用Anthropic Helpful-Harmless RLHF数据集，训练并评估四个模型：基础OPT350M、SFT模型、DPO模型和SFT+DPO组合模型，采用HmR、HpR和CAS三个评估指标。

Result: SFT表现优于DPO，但SFT+DPO组合模型在所有指标上表现最佳，证明了这些技术的互补性。

Conclusion: 组合使用SFT和DPO对齐技术能够产生最佳效果，但研究也面临噪声数据、GPU资源有限和训练约束等挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在通用信息抽取任务中表现不足，特别是在处理结构化输出、复杂模式描述和多步推理时存在局限，需要提升模型的泛化能力。

Method: 结合强化学习和多视角推理，将大语言模型从被动抽取器转变为主动推理器，使其不仅能理解抽取内容，还能掌握推理过程。

Result: 在多个信息抽取基准测试中，MR-UIE方法显著提升了抽取准确率，在多个数据集上超越了最先进方法，特别是在复杂任务中展现出优异的泛化能力。

Conclusion: 多视角推理与强化学习的结合有效提升了信息抽取性能，证明了推理在复杂场景中的关键作用，为提升大语言模型在结构化输出任务中的表现提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 孟加拉语作为全球第五大语言，在大型语言模型中代表性不足，特别是在代码生成领域，主要原因是缺乏高质量的预训练和微调数据

Method: 构建了全面的孟加拉语代码指令数据集用于编程领域适配，创建了MBPP-Bangla评估基准，开发了TigerCoder系列代码大语言模型

Result: TigerCoder模型在Pass@1指标上相比现有多语言和通用孟加拉语LLMs取得了显著11-18%的性能提升

Conclusion: 精心策划的高质量数据集可以克服小模型在低资源语言上的局限性，所有资源都已开源以推动孟加拉语LLM研究的进一步发展
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在专业领域（特别是电商）性能下降的问题，因为电商数据具有噪声大、异构、多语言和动态性强的特点。

Method: 采用更少但更大的专家MoE架构，结合节点内专家并行和定制memcpy操作符优化GPU利用率；使用12T token的多语言语料和合成电商指令进行混合训练；提出OTPO方法增强指令对齐。

Result: 在电商任务上超越DeepSeek-V3.1、GPT-4系列和Qwen3-235B；在东南亚低资源语言和葡萄牙语上表现优异；在Shopee平台已替代70%以上的OpenAI流量。

Conclusion: Compass-v3在专业电商领域和多语言能力上都达到先进水平，证明了垂直领域MoE模型的有效性，并已在工业场景成功部署。
...
<details>
  <summary>Details</summary>
Motivation: 传统手动编码对话行为分类耗时耗力，研究旨在利用生成式AI技术减少人工标注负担，提高教育对话分析的效率。

Method: 使用开源CIMA语料库，测试GPT-3.5-turbo和GPT-4模型，通过定制提示词对导师回答进行四类对话行为分类。

Result: GPT-4模型取得80%准确率、0.81加权F1分数和0.74 Cohen's Kappa值，与人工标注达成实质性一致，超越基线性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决越南语这种低资源语言缺乏竞争性重排序模型的问题，越南语具有复杂语法和音调符号，需要专门优化的模型来处理其语言特性。

Method: 基于BGE-M3编码器架构，集成Blockwise Parallel Transformer技术，使用8GB精选语料库进行训练，采用混合硬负采样方法进行微调以增强模型鲁棒性。

Result: 在MMARCO-VI基准测试中取得了强大的早期排名准确性，超越了多语言基线模型，并与PhoRanker表现相当接近。

Conclusion: ViRanker填补了越南语重排序模型的空白，通过精心设计的架构适应和数据策展，为其他资源匮乏语言的重排序技术发展提供了借鉴，模型已在Hugging Face平台开源以支持可复现性和实际应用。
...
Method: 采用模块化流水线设计，涵盖多种方法学选择，包括脑数据集、脑区域、刺激特征、降采样方法等，并提供内置日志记录、绘图功能以及与实验跟踪平台的无缝集成。

Result: 通过在三个故事听述数据集（LeBel et al. (2023)、Narratives和Little Prince）上拟合一系列编码模型，证明了框架的可扩展性和多功能性。

Conclusion: LITcoder通过提供灵活的后端和标准化工具，显著降低了编码模型的技术实现难度，促进了方法学的严谨性和系统比较，有助于加速高质量脑活动预测模型的发展。
...
Result: 在多个基准数据集上的实验结果表明，所提方法优于最先进的基线方法。

Conclusion: 该反事实增强去偏框架能有效减少虚假相关性，引导模型关注与情感相关的内容，提高目标导向多模态情感分类性能。
...
Conclusion: EchoX成功解决了语音大语言模型的声学-语义表示问题，保持了强大的推理能力
...
<details>
  <summary>Details</summary>
Motivation: 传统Trie偏置方法在ASR系统中为罕见词识别提供额外分数，但当完整词未被识别时需要撤销之前给予的部分假设分数，这一过程计算昂贵且仅限于束搜索

Method: 通过让ASR模型进行多步前瞻预测，一次性预测多个步骤，从而更好地估计部分假设是否会导致生成完整罕见词，避免分数撤销步骤。使用仅10小时合成数据对Whisper模型进行微调

Result: 在NSC Part 2测试集上，词错误率从30.86%显著降低到12.19%

Conclusion: 提出的多步前瞻预测方法有效解决了传统上下文偏置的计算效率问题，大幅提升了罕见词识别性能
...
<details>
  <summary>Details</summary>
Motivation: 解决ASR模型在罕见词识别上的挑战，传统方法使用合成数据训练偏置模块但容易因合成音频伪影导致过拟合。

Method: 增强TCPGen上下文偏置方法，提出包含掩码交叉熵和二元分类的关键词感知损失函数，分别处理偏置词预测和位置检测。

Result: 在10小时合成数据上微调Whisper模型，在NSC Part 2测试集上词错误率从29.71%显著降低到11.81%。

Conclusion: 关键词感知损失函数有效提升上下文偏置性能，两个损失项互补支持推理过程中的偏置词解码，显著改善罕见词识别效果。
...
<details>
  <summary>Details</summary>
Motivation: 狨猴具有复杂的语音交流能力，类似于人类语言特征，研究其声音通信可以链接大脑活动，但由于主要使用声音交流，标准LLM方法不适用

Method: 设计了生成式狨猴语音建模(GmSLM)管道，使用无监督野外数据和弱标记对话数据进行零样本评估

Result: GmSLM生成的声音在声学上接近真实重合成样本，能有效区分真实与人工对话，在下游任务中表现优异
...
Method: CCF框架整合了分段语义聚合和键值记忆编码，形成紧凑表示。采用训练高效的优化策略，结合增量分段解码和稀疏储层采样，大幅降低内存开销。
...
<details>
  <summary>Details</summary>
Motivation: 准确评估简历中的候选人资历是一个关键但具有挑战性的任务，因为普遍存在经验夸大和模糊自我表述的问题，需要自动化解决方案来提高评估准确性。

Method: 引入混合数据集（包含真实简历和合成生成的困难样本），使用大型语言模型（包括微调BERT）来检测与资历夸大相关的微妙语言线索和隐含专业技能。

Result: 研究结果显示了大型语言模型在识别资历夸大和自我推销语言方面的良好性能，为增强AI驱动的候选人评估系统提供了有希望的方向。

Conclusion: 该方法有助于减轻自我推销语言引入的偏见，提高简历资历评估的自动化水平，数据集已公开供研究社区使用。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的表格具有多样化的结构、大小和数据类型，表格问答面临独特挑战，需要开发能够准确回答结构化查询的模型
...
Conclusion: 大语言模型驱动的表格问答方法有效，论文详细阐述了方法学、实验结果和替代方案，为大语言模型在表格问答领域的应用提供了见解
...
<details>
  <summary>Details</summary>
Motivation: 现有的专利SDG分类方法（如关键词搜索、迁移学习和基于引文的启发式方法）缺乏可扩展性和泛化能力，且缺乏大规模标注数据集限制了监督学习的应用。

Method: 将专利-SDG分类构建为弱监督问题，使用专利到SDG标记科学文献的引文作为噪声初始信号，开发复合标注函数，利用LLM从专利和SDG论文中提取结构化概念（功能、解决方案、应用），计算跨域相似度分数，并通过基于排名的检索方法组合。
...
TL;DR: MetaRAG是一个用于检索增强生成(RAG)系统的幻觉检测框架，通过事实分解、变异生成和上下文验证来实时检测AI回答中的不实信息，特别适用于身份敏感的高风险领域。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法主要针对独立LLM，无法解决RAG系统中回答必须与检索证据一致的特殊挑战，而企业应用中RAG系统的可靠性问题亟待解决。

Method: 四阶段框架：1)将答案分解为原子事实；2)使用同义词和反义词生成受控变异；3)根据检索上下文验证每个变体（同义词应被蕴含，反义词应被矛盾）；4)将不一致性惩罚聚合成响应级幻觉分数。

Result: 在专有企业数据集上的实验证明了MetaRAG在检测幻觉方面的有效性，能够实现基于RAG的对话代理的可信部署。

Conclusion: MetaRAG提供了一个实时、无监督、黑盒的幻觉检测解决方案，特别适合身份敏感的查询，能够定位不支持的声明并支持系统设计者配置阈值和安全防护措施。
...
<details>
  <summary>Details</summary>
Motivation: 类比推理是人类认知的重要组成部分，但目前NLP研究中对类比推理的处理缺乏认知科学的理论基础，需要建立认知过程与NLP技术之间的联系。

Method: 通过总结认知科学文献中关于类比推理过程的关键理论，并将其与自然语言处理中的相关概念进行关联分析。

Result: 发现认知科学中的类比推理过程可以很好地映射到NLP概念，但这些联系通常没有被从认知角度审视；同时证明了这些理论对NLP中与类比解决不直接相关的主要挑战也具有相关性。

Conclusion: 该研究为NLP研究者提供了优化文本关系理解的指导，建议减少对实体层面相似性的过度依赖，转而采用基于认知科学理论的更全面的关系理解方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的图线性化方法标签空间较大且结构信息保留不足，需要一种更高效的图序列表示方法

Method: 使用层次括号编码将依赖图转换为序列，通过n个标注动作实现线性时间解析，同时支持重入、循环和空节点的表示

Result: 在多语言多形式基准测试中取得了有竞争力的结果，在精确匹配准确率上相比其他方法有持续改进

Conclusion: 层次括号编码方法显著减少了标签空间同时保留了结构信息，为依赖图解析提供了一种实用且高效的解决方案
...
Method: 提出GrACE生成式置信度评估方法，通过模型最后一个隐藏状态与词汇表中特殊标记嵌入的相似性来表达置信度，并使用准确性相关的校准目标对模型进行微调。
...
Result: 成功翻译多达9000个多模态互动练习，开发了针对教育领域的机器翻译系统，所有应用免费向学生、教师和研究人员开放。

Conclusion: 项目通过跨学科合作成功开发了多语言教育解决方案，为多元文化教育提供了实用工具，特别在乌克兰语教育支持方面具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 在简历推荐系统中，职位名称匹配是一个关键挑战，因为重叠术语往往有限或具有误导性。需要捕捉超越表面词汇相似性的细微语义关系，同时提高模型的可解释性。

Method: 采用自监督混合架构，结合密集句子嵌入和领域特定知识图谱，使用图神经网络进行知识图谱集成。通过将语义相关度分数连续体划分为低、中、高三个不同区域进行分层评估。

Result: 结果显示，经过知识图谱增强的微调SBERT模型在高语义相关度区域表现最佳，RMSE比强基线降低了25%。知识图谱与文本嵌入的结合在提高性能方面显示出明显优势。

Conclusion: 研究不仅证明了知识图谱与文本嵌入结合的好处，还强调了区域性能分析在理解模型行为中的重要性。这种细粒度方法揭示了全局指标隐藏的优势和弱点，支持在人力资源系统中进行更有针对性的模型选择。
...
<details>
  <summary>Details</summary>
Motivation: 探索在存在分歧的标注数据中，如何有效预测个体标注者的注释（perspectivist annotations）和软标签，以处理标注不一致性问题。

Method: 采用两种主要方法：1）基于大语言模型的上下文学习（ICL），比较不同的示例采样策略；2）基于RoBERTa的标签分布学习（LDL）方法，评估多种微调技术。

Result: 研究表明：ICL能有效预测个体标注者的注释，将这些预测聚合成软标签可获得有竞争力的性能；LDL方法在软标签预测方面表现出潜力。

Conclusion: 上下文学习和标签分布学习方法都是处理标注分歧问题的有效途径，值得perspectivist研究社区进一步探索和应用。
...
Result: 揭示了三个关键阶段：早期LLM采用和任务/数据集创新；对LLM局限性的批判性反思；外围技术逐渐融入模块化系统
...
<details>
  <summary>Details</summary>
Motivation: 在线学习环境存在社交障碍，SAMI系统因缺乏完整心智理论而无法有效建模学生个性，影响推荐相关性

Method: 利用GPT零样本能力从论坛介绍帖推断大五人格特质，建立人格检测模型，并将其集成到SAMI的实体匹配系统中

Result: 人格检测模型在基准测试中表现良好，初步整合显示人格特质可以补充现有匹配因素
...
<details>
  <summary>Details</summary>
Motivation: 现有研究通常将情感分类到预定义的有限类别中，忽略了更细微的情感表达，需要评估LLMs是否能在细粒度水平上与人类情感对齐。

Method: 引入EXPRESS基准数据集（包含251个细粒度情感标签），通过综合评估框架分析预测情感术语，并将其分解为八种基本情感进行细粒度比较，系统测试主流LLMs在不同提示设置下的表现。

Result: 主流LLMs在准确预测与人类自我披露情感一致的情感方面仍面临挑战；某些LLMs生成的情感术语与既定情感理论一致，但在捕捉上下文线索方面不如人类自我披露有效。
...
<details>
  <summary>Details</summary>
Motivation: 在医疗证明不可得的资源有限地区，口头尸检是估计死因的关键工具，但传统方法准确率有限，需要更先进的解决方案。

Method: 使用PHMRC数据集，评估GPT-5预测、LCVA基线、文本嵌入和元学习器集成等多种方法，结合LLM与传统算法进行分类。
...
Method: 采用两阶段迭代对齐：规划智能体对齐阶段优化子目标生成，执行智能体改进阶段使用多样化子目标-动作对进行微调

Result: 在六个基准测试中超越最先进基线，held-in任务平均提升3.1%，held-out任务平均提升4.4%

Conclusion: MOAT框架通过联合对齐调优有效解决了多智能体协作问题，理论分析证明其训练过程非递减且渐进收敛
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在各种计算任务中表现出色，但其内部工作机制仍不明确。研究者希望探究在实践中心算任务中，模型是如何通过自注意力和多层感知机进行信息处理和计算的。

Method: 采用三步骤方法：抑制初始层的输入特定token计算、限制中间层跨token位置的信息传递路径、强制剩余层所有计算在最后token进行。提出Context-Aware Mean Ablation和Attention-Based Peeking两种技术来识别关键计算子图。

Result: 发现了All-for-One子图，该子图在各种心算任务中保持高精度，有效计算发生在深层且仅限最后token，通过特定中间层接收其他token信息。实验证明该子图对模型性能既充分又必要，且在不同模型和输入风格间具有可迁移性。

Conclusion: 大语言模型在心算任务中的有效计算集中在网络深层的最后token位置，通过特定的中间层信息传递机制实现。提出的CAMA和ABP技术相比其他方法具有独特优势，为理解LLM内部计算机制提供了重要见解。
...
<details>
  <summary>Details</summary>
Motivation: 现有MoE模型中的专家网络可能包含特定的行为模式，需要一种无需重新训练就能检测和控制这些专家以调节模型行为的方法

Method: 通过对比分析具有相反行为的输入对，检测具有不同激活模式的专家，然后在推理过程中选择性激活或停用这些专家

Result: 在11个基准测试和6个LLM上，安全性和忠实度分别提升20%和27%；在对抗攻击模式下，安全性下降41%，与现有越狱方法结合可完全绕过所有安全防护

Conclusion: SteerMoE揭示了MoE模型中存在对齐伪装的新维度，提供了一种无需权重修改的行为控制框架
...
Method: 使用演员困惑度(perplexity)和评论家多头架构的价值估计方差作为好奇心信号，作为探索奖励融入RLVR框架

Result: 在AIME基准上相比标准GRPO/PPO的RLVR方法提升约3分，并揭示了RLVR中的校准崩塌机制

Conclusion: 好奇心驱动探索能有效解决RLVR的探索问题，理论分析显示该方法能惩罚过度自信错误并促进回答多样性，同时与RL中的计数探索奖励有理论联系
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态检索在LLMs和多模态LLMs中的快速发展，出现了越来越复杂的检索任务。现有方法主要依赖任务特定的视觉语言模型微调，且仅限于单模态查询或文档，需要一种能够处理多模态查询和文档的统一检索模型。

Method: ReT-2采用多层表示和循环Transformer架构，结合LSTM启发的门控机制，动态整合跨层和跨模态信息，捕捉细粒度的视觉和文本细节。

Result: 在M2KR和M-BEIR基准测试中，ReT-2在不同检索配置下始终达到最先进性能，同时提供更快的推理速度和更低的内存使用。在检索增强生成管道中集成后，还能提升Encyclopedic-VQA和InfoSeek数据集的下游性能。

Conclusion: ReT-2是一个高效统一的多模态检索模型，成功解决了多模态查询和文档检索的挑战，在性能和效率方面都优于现有方法，源代码和训练模型已公开。
...
<details>
  <summary>Details</summary>
Motivation: 人类能够识别跨物种、视角和上下文变化的相同动作，但当前深度学习模型在此类泛化方面存在困难

Method: 利用视觉扩散模型生成特征，通过transformer进行聚合，特别使用扩散过程早期时间步的模型来强调语义信息而非像素细节
...
Method: 提出PromptGuard框架，包含VulnGuard Prompt混合技术：使用GitHub数据驱动的对比学习、少量示例、伦理思维链推理和自适应角色提示，构建群体特异性保护屏障

Result: 通过多目标优化理论证明，使用熵边界和帕累托最优性实现25-30%的分析性危害减少，建立了系统实证研究的数学基础

Conclusion: PromptGuard提供了一个智能专家系统，通过六个核心模块实现实时危害预防，为LLM安全保护弱势群体提供了理论框架和数学验证
...
Method: 采用孪生网络架构，结合Beta混合相似度异常检测(Beta-SOD)框架，使用二元交叉熵、对比损失和余弦嵌入损失联合优化特征相似度学习

Result: 在CUHK03、Market-1501和VeRi-776数据集上，Beta-SOD在10-30%噪声水平下均优于现有最先进方法

Conclusion: Beta-SOD为噪声Re-ID场景提供了有效解决方案，具有鲁棒性和广泛适用性
...
<details>
  <summary>Details</summary>
Motivation: 冠状动脉疾病是全球主要死因之一，ICA作为金标准诊断方法，但现有分割和检测方法面临低对比度、高噪声和复杂血管结构的挑战，需要更精确的解决方案。

Method: 提出SFD-Mamba2Net框架，编码器嵌入曲率感知结构增强模块突出管状血管结构，解码器采用渐进高频感知模块通过小波分解细化高频细节并整合低频全局结构。

Result: 在八个分割指标上均优于最先进方法，在狭窄检测中实现了最高的真阳性率和阳性预测值。

Conclusion: 该框架通过多尺度结构增强和频域细节优化，有效解决了ICA图像血管分割和狭窄检测的技术难题，具有重要临床价值。
...
<details>
  <summary>Details</summary>
Motivation: 结直肠癌肝转移严重影响患者生存，现有预后模型预测能力不足，特别是在多灶性病例中。需要开发更准确的自动化预测工具来改善手术治疗效果评估。

Method: 开发了包含分割管道和放射组学管道的全自动框架。分割管道使用可提示基础模型完成部分标注数据的分割，并提出SAMONAI零样本3D提示传播算法。放射组学管道使用SurvAMINN自编码器多实例神经网络进行生存分析。

Result: 在227名患者的机构数据集上评估显示，该框架显著超越现有临床和基因组生物标志物，C-index提升超过10%。

Conclusion: 该研究展示了自动化分割算法与放射组学生存分析相结合的巨大潜力，能够为CRLM提供准确、标注高效且可解释的结果预测。
...
<details>
  <summary>Details</summary>
Motivation: 研究不同生成模型学习到的视觉表示在何时以及如何出现分歧，探索模型间的视觉属性差异及其触发条件

Method: 提出CompCon进化搜索算法，自动发现一个模型输出中更普遍的视觉属性，并找出与这些视觉差异相关的提示概念。构建ID2数据集进行自动化评估

Result: 成功比较了流行文本到图像模型，发现如PixArt在孤独提示下绘制湿街道，Stable Diffusion 3.5在媒体职业提示中描绘非裔美国人的表示差异

Conclusion: CompCon能有效识别模型间的视觉表示差异，为理解生成模型的内部表示提供了新工具，有助于模型比较和偏差分析
...
<details>
  <summary>Details</summary>
Motivation: 无人机遥感影像在云层条件下仍可采集，但常受云影和太阳耀斑污染，严重影响水质参数估计的准确性，需要有效解决方案。

Method: 使用基于U-Net的深度学习模型，在像素级别提取数据训练，通过多种评估指标确定最佳模型设置，开发高质量图像校正模型。

Result: 成功识别和分离受云影和太阳耀斑影响的区域，建立了有效的图像校正模型，能够恢复受污染区域的图像信息。

Conclusion: 该方法为无人机遥感影像中的云影和太阳耀斑问题提供了有效的机器学习解决方案，显著提升了水质监测数据的可靠性。
...
Method: 1) 局部图嵌入模块初始化得分矩阵；2) 通过3D空间最近邻搜索进行双边源-目标和目标-源匹配的重定位步骤；3) 堆叠配对点特征并通过全局几何一致性学习预测点对点匹配概率。

Result: 在KITTI数据集上达到95%-97%的内点率，KITTI-360上94%-97%，3DMatch上最高81.1%，在6-DoF姿态估计和部分到部分匹配任务中均显著提升刚性配准性能。

Conclusion: iMatcher通过完全可微的框架实现了优异的点云配准性能，在多个真实世界数据集上证明了其鲁棒性和有效性，为特征匹配提供了新的解决方案。
...
Result: UltrON能够从B模式图像中获取声学特征而无需额外标注成本，改善了几何一致性，缓解了遮挡和稀疏标注的限制
...
<details>
  <summary>Details</summary>
Motivation: 自动量化心肌内运动和应变从标记MRI仍然是一个重要但具有挑战性的任务，需要开发更准确和高效的方法来处理大型心脏MRI数据集。

Method: 使用基于学习潜在码的隐式神经表示(INRs)来预测连续左心室位移，无需推理时优化过程。

Result: 在452个UK Biobank测试案例中，该方法实现了最佳跟踪精度(2.14 mm RMSE)和最低的全局周向应变(2.86%)与径向应变(6.42%)组合误差，比最准确的基线快约380倍。
...
<details>
  <summary>Details</summary>
Motivation: 现有的通用域自适应方法如MLNet对所有分类器平等对待，导致学习信号稀释，需要更聚焦的适应策略

Method: 提出增强型互学习网络(E-MLNet)，集成动态加权策略到开放集熵最小化中，利用闭集分类器预测来聚焦最相关的类别边界

Result: 在四个基准测试中表现优异，在VisDA和ImageCLEF上获得最高平均H分数，在31个任务中22个优于MLNet基线

Conclusion: 动态加权策略有效提升了通用域自适应的性能，证实了聚焦适应策略的益处
...
TL;DR: 本文介绍了COCO-Urdu数据集，这是最大的公开乌尔都语图像字幕数据集，包含59,000张图片和319,000个乌尔都语字幕，旨在解决乌尔都语在多模态研究中资源不足的问题。
...
Method: 从MS COCO数据集通过分层抽样获取59,000张图像，使用SeamlessM4T v2进行翻译，并通过混合多模态质量评估框架（包括COMET-Kiwi翻译质量评估、CLIP视觉相似性和BERTScore语义一致性）进行验证和迭代优化。

Result: 构建了包含319,000个高质量乌尔都语字幕的数据集，在BLEU、SacreBLEU和chrF基准测试中表现出色，是目前最大的公开乌尔都语字幕数据集。
...
TL;DR: VoxelFormer是一个轻量级Transformer架构，通过多主体训练实现fMRI视觉解码，使用Token Merging Transformer进行高效体素压缩和Q-Former产生与CLIP图像嵌入空间对齐的神经表示。


<details>
  <summary>Details</summary>
Motivation: 现有fMRI视觉解码方法大多依赖特定主体的训练，限制了可扩展性和实际部署。需要开发能够进行多主体训练的通用解码方法。

Method: 提出VoxelFormer架构，整合Token Merging Transformer(ToMer)进行体素压缩和query驱动的Q-Former生成固定大小的神经表示，与CLIP图像嵌入空间对齐。

Result: 在7T自然场景数据集上评估，VoxelFormer在训练包含的主体上实现了有竞争力的检索性能，且参数数量显著少于现有方法。

Conclusion: token合并和基于query的transformer是参数高效神经解码的有前景策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有的反事实生成模型缺乏明确的解剖学归纳偏置，难以保持医学相关的细微局部变化，导致生成的脑部MRI图像在解剖学上不够合理。3D脑MRI研究需要能够准确反映细微形态差异的图像合成技术。

Method: 提出概率因果图模型(PCGM)，通过概率图模块捕获解剖约束并转换为空间二进制掩码，使用3D ControlNet编码这些掩码，约束新型反事实去噪UNet，最后通过3D扩散解码器生成高质量脑部MRI。

Result: 在多个数据集上的实验表明，PCGM生成的脑部MRI质量优于多个基线方法。首次证明从PCGM生成的反事实图像中提取的脑部测量能够复现神经科学文献中报道的疾病对皮层区域的细微影响。

Conclusion: PCGM是合成MRI在研究细微形态差异方面应用的重要里程碑，能够生成解剖学合理且保留医学相关细微变化的脑部MRI图像，为脑MRI研究提供了有价值的工具。
...
Method: 提出Med3DInsight框架，集成3D图像编码器和2D多模态大语言模型，使用平面切片感知transformer模块和基于部分最优传输的对齐方法，对LLM生成内容中的噪声具有更强容忍度。
...
TL;DR: 提出了一种基于多任务学习和环境固定物体信息的方法，用于改进人体-物体交互动作识别，在真实场景数据上达到99.25%的准确率，比仅使用人体骨架的基线模型提升2.75%。


<details>
  <summary>Details</summary>
Motivation: 现有的图卷积神经网络(GCNs)在人体动作识别中表现良好，但在检测人体-物体交互时效果不佳，主要原因是缺乏有效的场景信息表示和合适的学习架构。

Method: 采用多任务学习方法，结合环境中的固定物体信息（如ATM售票机、自助值机设备等）和交互区域信息，构建了一个包含交互和非交互动作类别的数据集。

Result: 在真实公共环境数据上，所提方法成功识别了手部与固定物体的交互动作（如操作设备）以及非交互动作（如行走、站立），准确率达到99.25%。

Conclusion: 通过整合环境物体信息和多任务学习架构，显著提升了人体-物体交互动作的识别性能，证明了场景上下文信息对于复杂动作识别的重要性。
...
TL;DR: 提出IRDFusion框架，通过跨模态特征对比和筛选策略，使用MFRM和DFFM模块实现高质量特征融合，在多个数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有多光谱目标检测方法在特征融合时容易保留多余背景或噪声，限制了感知性能，需要新的融合框架来解决这个问题

Method: 提出互特征精炼模块(MFRM)增强模态内和模态间特征表示，差分特征反馈模块(DFFM)动态计算模态间差分特征作为指导信号，形成迭代关系图差分引导特征融合机制(IRDFusion)

Result: 在FLIR、LLVIP和M3FD数据集上实现了最先进的性能，在各种挑战性场景中 consistently 优于现有方法
...
TL;DR: SQAP-VLA是一个无需训练的结构化VLA推理加速框架，首次同时实现了最先进的量化和token剪枝，解决了现有方法的不兼容问题，实现了1.93倍加速和4.5%性能提升。
...
Method: 提出了SQAP-VLA框架，通过协同设计量化和token剪枝流程，提出新的量化感知token剪枝标准，在激进量化模型上工作，同时改进量化器设计以增强剪枝效果。

Result: 在标准VLA模型上应用SQAP-VLA，显著提高了计算效率和推理速度，同时成功保留了核心模型性能，实现了1.93倍加速和高达4.5%的平均成功率提升。

Conclusion: SQAP-VLA是第一个能够同时实现先进量化和token剪枝的训练免费VLA推理加速框架，通过克服不兼容性问题，为VLA模型的实际部署提供了有效的解决方案。
...
TL;DR: S-BEVLoc是一种基于鸟瞰图的自监督LiDAR全局定位框架，无需地面真值位姿，通过地理距离构建训练三元组，在KITTI和NCLT数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前LiDAR全局定位方法依赖GPS或SLAM里程计提供的地面真值位姿进行监督训练，但高精度地面真值获取成本高昂且费时，需要开发无需地面真值的自监督方法。

Method: 提出自监督框架S-BEVLoc，利用单张BEV图像构建训练三元组，通过已知地理距离的关键点中心BEV补丁，使用CNN提取局部特征，NetVLAD聚合全局描述符，并引入SoftCos损失增强学习。

Result: 在KITTI和NCLT大规模数据集上的实验表明，S-BEVLoc在位姿识别、闭环检测和全局定位任务中达到了最先进的性能，同时具有比监督方法更好的可扩展性。

Conclusion: S-BEVLoc成功证明了无需地面真值位姿的自监督LiDAR全局定位的可行性，为SLAM系统提供了更实用和可扩展的解决方案。
...
TL;DR: FPI-Det是一个专门用于检测人脸和手机交互的新数据集，包含22,879张图像，涵盖多种场景，旨在解决现有通用基准在细粒度人机交互检测方面的不足。


<details>
  <summary>Details</summary>
Motivation: 移动设备的广泛使用给视觉系统带来了新的挑战，需要不仅识别物体还要理解行为上下文，现有通用基准无法充分捕捉人脸、手部和设备之间的细粒度交互关系。

Method: 构建了包含22,879张图像的数据集，涵盖工作场所、教育、交通和公共场景，提供同步的人脸和手机标注，数据集具有极端尺度变化、频繁遮挡和多样化拍摄条件。

Result: 评估了代表性的YOLO和DETR检测器，提供了基线结果，并分析了在不同物体大小、遮挡水平和环境条件下的性能表现。
...
TL;DR: ZeroPlantSeg是一种零样本分割方法，用于从顶视图图像中分割莲座状植物个体，无需训练即可实现植物个体的层次分割
...
Conclusion: ZeroPlantSeg提供了一种有效的零样本植物个体分割解决方案，减少了标注需求并提高了跨域泛化能力
...
TL;DR: 该研究通过数据构建和模型架构的协同改进，将CLIP应用于人物表征学习，提出了WebPerson数据集和GA-DMS框架，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: CLIP在人物表征学习中面临两个关键挑战：缺乏大规模标注的人物中心视觉语言数据，以及全局对比学习难以保持细粒度匹配所需的判别性局部特征且容易受到噪声文本标记的影响。

Method: 1) 开发基于MLLMs上下文学习能力的抗噪声数据构建管道，自动过滤和标注网络图像，创建了包含500万高质量人物中心图像-文本对的WebPerson数据集；2) 提出GA-DMS框架，通过梯度-注意力相似性分数自适应掩码噪声文本标记，并引入掩码标记预测目标来增强细粒度语义表征学习。

Result: 大量实验表明，GA-DMS在多个基准测试中实现了最先进的性能。

Conclusion: 该研究通过数据构建和模型架构的协同改进，成功将CLIP应用于人物表征学习，解决了数据稀缺和全局对比学习局限性问题，取得了优异的性能表现。
...
Method: 使用潜在扩散模型(LDM)，包含三项关键技术：1)Radon掩码增强策略(RMAS)生成20万+多样训练样本；2)正负掩码约束嵌入几何一致性；3)透明医学注意力(TMA)机制增强病灶相关区域。
...
TL;DR: 本研究比较了人工神经网络(ANN)和持久同调(PH)方法在噪声环境下预测2D二值图像Betti数的性能，发现ANN在噪声条件下表现优于传统的PH方法。


<details>
  <summary>Details</summary>
Motivation: 探索在存在噪声的情况下，人工神经网络是否能够比传统的持久同调方法更有效地推断数据的拓扑结构，因为ANN能够从训练数据中学习上下文和几何先验。

Method: 使用监督神经网络预测2D二值图像的Betti数，并与基于立方复形和符号欧几里得距离变换(SEDT)的PH流程进行比较。实验使用了一个合成数据集和两个真实世界数据集。

Result: 研究表明，在噪声条件下，ANN方法能够超越PH方法，这归因于ANN从训练数据中学习上下文和几何先验的能力。

Conclusion: 尽管仍处于发展阶段，但使用ANN进行拓扑估计在结构噪声条件下为PH提供了一个有吸引力的替代方案。
...
TL;DR: 这篇调查论文首次采用数据集驱动的视角，分析视频理解领域的发展，展示数据集特征如何引导模型架构演进，并为未来通用视频理解提供路线图。


<details>
  <summary>Details</summary>
Motivation: 现有调查主要按任务或模型家族分类，忽视了数据集对架构演化的结构性影响。本文旨在填补这一空白，从数据集驱动的角度重新审视视频理解模型的发展。

Method: 采用数据集驱动分析方法，重点关注运动复杂性、时间跨度、层次化组合和多模态丰富性等数据集特征，重新解读从双流网络、3D CNN到序列模型、Transformer和多模态基础模型等里程碑架构。

Result: 建立了数据集、归纳偏置和架构的统一框架，展示了不同模型架构如何响应特定数据集特征的压力，提供了模型设计与数据集不变性对齐的实用指导。

Conclusion: 通过将数据集、归纳偏置和架构统一到一个连贯框架中，本调查不仅提供了全面的回顾，还为推进通用视频理解提供了规范性路线图，强调在可扩展性和任务需求之间平衡的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有深度学习细胞检测模型无法像病理学家那样在不同放大倍数间切换来理解多尺度语义依赖关系的问题，缺乏多尺度重叠标注数据集是主要障碍。

Method: 组织OCELOT 2023挑战赛，提供包含673对来自306个TCGA全玻片图像的多器官数据集，包含重叠细胞检测和组织分割标注，分为训练、验证和测试集。

Result: 参赛模型显著提升了对细胞-组织关系的理解，最佳模型在测试集上相比不包含细胞-组织关系的基线细胞检测模型F1分数提升了7.99。

Conclusion: 多尺度语义的融入对细胞检测至关重要，传统仅关注细胞的方法性能有限，需要开发能够理解细胞与组织相互作用的模型来达到人类水平性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决无人机图像中目标检测面临的挑战，包括小目标密集排列、尺度变化和遮挡等问题，需要开发更有效的实时检测系统。

Method: 1. 引入通道门控注意力上采样/下采样(AU/AD)双路径机制，减少特征层传播误差并保留细节；2. 在特征融合中采用CSP-PAC技术，使用并行空洞卷积处理局部和上下文信息，促进多尺度特征整合。
...
TL;DR: 提出了一种无需训练的知识聚焦框架来解决KB-VQA中的知识冗余问题，通过低噪声查询检索、大模型提取关键知识片段以及选择性知识整合策略，显著提升了问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有的KB-VQA方法直接将检索到的知识信息增强到模型中，忽略了大量的知识冗余问题，这些冗余信息在回答过程中引入了噪声，影响了模型性能。

Method: 1) 从图像-问题对中提取关键部分创建低噪声查询；2) 使用大模型从检索到的知识中识别和提取对答案有益的关键片段；3) 引入选择性知识整合策略，仅在模型对回答问题缺乏信心时整合知识

Result: 广泛的实验表明，该框架在知识获取的准确性和关键性方面表现优异，超越了现有的最先进方法。

Conclusion: 该训练无关的知识聚焦框架有效缓解了知识冗余带来的噪声问题，通过增强知识相关性和减少冗余，显著提升了KB-VQA任务的性能。
...
Result: 在余干县分类中达到mIoU 74.50%、mAcc 82.73%、mF1 84.94%，在水体、植被和裸地分类中获得最高IoU，在70%训练集比例下训练时间增加有限且分类效果接近最优

Conclusion: CWSSNet模型具有良好的鲁棒性，在小样本训练条件下保持可靠性能，有效解决了高光谱图像特征冗余问题，提升了地物分类精度
...
TL;DR: 本文提出了Real-World Robustness Dataset (RRDataset)，用于在三个维度上全面评估AI生成图像检测模型：场景泛化、网络传输鲁棒性和重数字化鲁棒性，揭示了现有检测方法在真实世界条件下的局限性。
...
Method: 构建RRDataset数据集，包含7个主要场景的高质量图像，考察检测器在社交媒体多次分享后的性能，以及4种重数字化方法处理后的图像检测效果。对17个检测器和10个视觉语言模型进行基准测试，并开展192人参与的大规模人类研究。

Result: 基准测试结果揭示了当前AI检测方法在真实世界条件下的局限性，强调了借鉴人类适应能力开发更鲁棒检测算法的重要性。

Conclusion: 该研究强调了在复杂真实世界条件下评估AI生成图像检测方法的必要性，并展示了人类少样本学习能力在检测任务中的优势，为开发更鲁棒的检测算法提供了重要见解。
...
TL;DR: Dark-ISP是一个轻量级自适应的图像信号处理插件，直接在暗光环境下处理Bayer RAW图像，通过解构传统ISP流程为可微分组件，实现端到端的物体检测训练。


<details>
  <summary>Details</summary>
Motivation: 解决低光环境下物体检测的挑战，现有方法要么使用信息损失的RAW-RGB图像，要么采用复杂框架，需要一种轻量且自适应的解决方案。

Method: 将传统ISP流程解构为线性（传感器校准）和非线性（色调映射）子模块，作为可微分组件通过任务驱动损失优化，每个模块具有内容感知适应性和物理先验知识，并设计自增强机制促进子模块协作。

Result: 在三个RAW图像数据集上的实验表明，该方法在低光环境下以最少的参数超越了基于RGB和RAW的最先进检测方法。

Conclusion: Dark-ISP提供了一种有效的端到端低光物体检测解决方案，通过自适应ISP处理实现了优异的性能表现。
...
TL;DR: VQualA 2025挑战赛评估大型多模态模型在视觉质量比较任务上的表现，通过数千个从粗到细的质量比较任务来测试模型的开源推理能力。


<details>
  <summary>Details</summary>
Motivation: 评估和提升最先进的大型多模态模型在多个图像间进行开放式详细视觉质量差异推理的能力，推动开放域视觉质量比较研究。

Method: 构建包含数千个从粗到细粒度视觉质量比较任务的新基准，涵盖单图像、图像对和多图像组，采用2AFC二元偏好和多选题等整体评估协议。
...
TL;DR: MGTraj是一种新颖的多粒度目标引导轨迹预测模型，通过从粗到细的递归编码和变换器精炼网络，在多个粒度级别上整合特征，显著提升了人类轨迹预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有目标引导方法在粗粒度目标预测和细粒度轨迹补全之间存在粒度鸿沟，中间时间粒度的潜在效用未被充分探索，多粒度表示虽能捕捉不同尺度的人类动态模式，但如何有效整合到目标引导框架中仍具挑战。

Method: MGTraj递归地从粗到细粒度编码轨迹提案，每个级别使用基于变换器的递归精炼网络(RRN)捕获特征并预测渐进精炼，采用权重共享策略整合不同粒度特征，并使用速度预测作为辅助任务增强性能。

Result: 在ETH/UCY和斯坦福无人机数据集上的综合实验结果表明，MGTraj优于基线方法，在目标引导方法中实现了最先进的性能。

Conclusion: 多粒度建模方法有效提升了人类轨迹预测性能，MGTraj通过递归精炼和跨粒度特征整合成功解决了目标引导框架中的粒度整合挑战。
...
TL;DR: MatCha是首个材料表征图像理解基准，包含1500个需要专家级知识的问题，评估显示现有MLLMs在真实材料表征场景中表现有限
...
Method: 构建MatCha基准，涵盖材料研究的四个关键阶段和21个不同任务，包含1500个需要专家级专业知识的问题，评估最先进的MLLMs模型性能

Result: 现有MLLMs与人类专家存在显著性能差距，在处理需要高级专业知识和复杂视觉感知的问题时表现下降，简单的few-shot和思维链提示无法有效缓解这些限制
...
TL;DR: Medverse是一个通用的上下文学习模型，专为3D医学影像设计，能够在无需重新训练的情况下处理多种医学图像分析任务，包括分割、转换和增强。


<details>
  <summary>Details</summary>
Motivation: 当前医学影像的上下文学习模型存在两个关键限制：无法同时实现高保真预测和全局解剖理解，且缺乏跨多种任务和解剖区域的统一模型。

Method: 采用next-scale自回归上下文学习框架，从粗到细逐步优化预测；提出块状交叉注意力模块，在保持计算效率的同时促进上下文与目标输入之间的长程交互。

Result: 在涵盖未见过的临床中心、器官、物种和成像模态的广泛数据集上评估，Medverse显著优于现有基线方法。

Conclusion: Medverse为医学影像的上下文学习建立了新的范式，展示了在多样化医学图像处理任务中的强大通用性和性能。
...
Method: 构建FLUX-Reason-6M数据集（600万FLUX生成图像+2000万双语描述），采用六维特征分类和生成思维链设计；创建PRISM-Bench评估基准，包含7个评估轨道，使用先进视觉语言模型进行人机对齐评估。

Result: 对19个领先模型的评估揭示了关键性能差距，识别出需要改进的具体领域。数据集构建消耗15,000 A100 GPU天，为社区提供了前所未有的资源。

Conclusion: 该研究为推理导向的文本到图像生成提供了重要数据集和评估标准，将推动下一代开源模型的发展，所有资源已开源发布。
...
TL;DR: 提出CoAtNeXt混合模型用于胃组织图像分类，结合CoAtNet架构和ConvNeXtV2块，集成CBAM注意力机制，在两个公开数据集上表现优异，超越现有CNN和ViT模型。


<details>
  <summary>Details</summary>
Motivation: 胃病早期诊断至关重要，但传统组织病理学检查完全依赖人工，工作强度大且存在诊断差异，需要自动化、可靠高效的胃组织分析方法。
...
Conclusion: CoAtNeXt是胃组织图像组织病理学分类的稳健架构，在二分类和多分类任务中表现优异，有潜力辅助病理学家提高诊断准确性并减少工作负担。
...
TL;DR: 提出了首个针对全景X射线影像的大规模多模态指令数据集MMOral和评估基准MMOral-Bench，并基于此开发了专门的口腔医学模型OralGPT，显著提升了大型视觉语言模型在牙科领域的表现。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在牙科等专业医学领域的有效性尚未充分探索，特别是全景X射线影像由于密集的解剖结构和细微的病理线索，现有医学基准无法充分评估。

Method: 构建包含20,563张标注图像和130万条指令实例的MMOral数据集，涵盖属性提取、报告生成、视觉问答和图像对话等任务类型。基于Qwen2.5-VL-7B进行监督微调，开发OralGPT模型。

Result: 评估64个大型视觉语言模型，发现最佳模型GPT-4o仅达到41.45%准确率。OralGPT经过单轮微调后性能提升24.73%，显著优于现有模型。

Conclusion: MMOral数据集和OralGPT模型为智能牙科发展提供了重要基础，推动了多模态AI系统在牙科临床应用的进步。
...
Method: DATE方法包含时间戳注入机制(TIM)和时序感知相似性采样策略(TASS)。TIM将视频帧嵌入与文本时间戳标记交错构建连续时间参考系统；TASS将视频采样重新定义为视觉-语言检索任务，采用两阶段算法确保语义相关性和时序覆盖

Result: 在小时级长视频基准测试中取得了显著改进，特别是在绝对时间理解和关键事件定位方面。7B模型在某些基准上甚至超越了72B模型

Conclusion: DATE方法有效解决了长视频理解中的时序推理挑战，通过显式时间建模和语义引导采样策略显著提升了多模态大语言模型的时序感知能力
...
Method: 从冗余模型开始，通过块级剪枝和功能解耦损失迭代剪除冗余模块，实现动态高效的3D分割。

Result: 轻量版PSP-Seg-S在五个数据集上性能与nnU-Net相当，但GPU内存使用减少42-45%，训练时间减少29-48%，参数量减少83-87%。
...
TL;DR: 本文提出Code-as-Thought (CaT)方法，通过自适应选择代码推理或直接视觉推理路径来解决图表理解问题，使用强化学习训练选择策略，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有图表理解方法存在局限性：依赖外部工具导致脆弱性，或使用单一推理策略难以验证中间步骤。需要一种能够自适应选择最优推理路径的方法。

Method: 提出CaT方法将图表信息转换为可验证的符号格式，引入视觉可编程性概念，让VLM学习选择代码推理路径或直接视觉推理路径，使用包含数据准确性和决策奖励的双重奖励系统进行强化学习训练。

Result: 实验表明该方法在多样化图表理解基准测试中表现出强大且鲁棒的性能，能够动态选择最优推理路径。

Conclusion: 研究表明VLM不仅可以学习推理，还能学习如何推理，为每个任务动态选择最佳推理路径，解决了现有方法的局限性。
...
TL;DR: 提出了一种能够处理训练时未见过的MRI模态的脑部病变分割模型，通过在U-net架构中集成模态无关输入通道和模态特定通道，结合人工MRI模态增强训练，实现在多种病理类型和模态上的有效分割。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态脑MRI分割模型通常局限于固定的模态组合，无法有效处理推理时出现的新模态。有些模型虽然能泛化到未见模态，但可能丢失模态特定的判别信息。本研究旨在开发一个能够处理训练时未见模态、已见模态以及两者异质组合的模型。

Method: 对U-net架构进行简单实用的改进，集成模态无关输入通道和模态特定输入通道。开发图像增强方案合成人工MRI模态，通过差异性地改变病理和健康脑组织的外观来创建人工对比度，同时保持真实的解剖完整性。

Result: 在包含5种病理类型（中风、肿瘤、创伤性脑损伤、多发性硬化和白质高信号）和8种模态的8个MRI数据库上评估，结果表明该方法既能有效处理训练时见过的MRI模态，又能处理新的未见模态来改进分割效果。

Conclusion: 通过简单的架构改进和创新的数据增强策略，成功实现了对任意可用成像模态的分割能力，为脑部病变检测和分析提供了更灵活实用的工具。
...
TL;DR: 提出了一种针对无人机空对空目标检测的模型无关开集检测框架，通过嵌入空间熵建模处理未知物体拒绝，在保持检测精度的同时增强对损坏飞行数据的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统闭集检测器在域偏移和飞行数据损坏情况下性能显著下降，对安全关键应用构成风险，需要开发能够处理未知物体的鲁棒开集检测方法。

Method: 基于嵌入检测器的开集检测框架，通过嵌入空间熵建模估计语义不确定性，结合谱归一化和温度缩放技术增强开集判别能力，并采用背景拒绝机制。

Result: 在AOT空中基准测试和真实飞行测试中验证，相比标准YOLO检测器获得高达10%的相对AUROC提升，背景拒绝在不影响检测精度的情况下进一步增强了鲁棒性。

Conclusion: 该方法特别适用于动态空对空环境中的可靠无人机感知，为安全关键应用提供了有效的开集检测解决方案。
...
TL;DR: SlotSAR是一种新颖的SAR图像目标中心学习框架，无需掩码标注即可从复杂背景杂波中分离目标表示，通过多级特征提取和注意力机制实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: SAR图像中的目标常被复杂背景杂波（如地形反射和斑点噪声）干扰，这些杂波在强度和模式上与目标相似，导致模型提取纠缠或虚假特征，影响目标表示的清晰度。

Method: 提出SlotSAR框架：1）从SARATR-X提取高级语义特征；2）从小波散射网络提取低级散射特征；3）设计多级slot注意力模块整合高低级特征，增强slot表示的区分性。

Result: 实验结果表明，SlotSAR在SAR图像处理中达到了最先进的性能，相比现有OCL方法能更好地保持结构细节。

Conclusion: SlotSAR通过多级特征互补和注意力机制，有效解决了SAR图像中目标与背景杂波分离的挑战，为无掩码标注的目标表示学习提供了有效解决方案。
...
TL;DR: PHCP是一个无需联合训练的无监督异构协作感知框架，通过推理时自训练适配器实现动态特征对齐，在少量无标签数据下达到与全数据集训练SOTA方法相当的性能
...
Result: 在OPV2V数据集上的广泛实验表明，PHCP在多样化异构场景中表现强劲，仅使用少量无标签数据就达到与全数据集训练SOTA方法相当的性能

Conclusion: PHCP成功证明了在推理时直接解决异构协作感知问题的可行性，无需联合训练或预先存储模型，为实际应用提供了实用解决方案
...
TL;DR: 本文对视觉语言模型在零样本图像分类中的语言引导和纯视觉推理能力进行了全面评估，发现语言和视觉具有互补优势，并提出基于类精度的简单融合方法提升性能


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在零样本分类中主要依赖图像-文本对齐，但其纯视觉推理能力尚未得到充分探索，需要系统评估两种方式的性能差异和互补性

Method: 使用包括SigLIP 2和RADIOv2.5在内的多种双编码器VLM模型，在ImageNet-1k数据集上评估语言引导和纯视觉分类性能，分析提示设计、类多样性、k-NN邻居数等关键因素，并提出基于类精度的学习无关融合方法

Result: 语言和视觉在图像分类中表现出互补优势，不同类别适合不同的推理方式，提出的融合方法有效提升了分类性能

Conclusion: 视觉语言模型的纯视觉推理能力值得深入探索，语言和视觉的互补性可以通过简单融合方法有效利用，为提升零样本分类性能提供了新思路
...
Method: 提出Better Understanding Generation (BUG)工作流，结合大型多模态模型，采用图像转提示技术从聊天对话中自动生成和精细定制服装设计

Result: 创建了FashionEdit数据集模拟真实服装设计工作流，在生成相似性、用户满意度和质量三个维度进行评估，证明了模型有效性
...
TL;DR: 本文研究手术技能评估的少样本学习，发现领域相关的小数据集预训练优于大规模不相关数据集，加入手术特定数据可提升性能但需注意领域相似性


<details>
  <summary>Details</summary>
Motivation: 手术技能评估标注稀缺且耗时，需要专家共识，少样本学习提供了可扩展的解决方案，但预训练策略在SSA中尚未充分探索

Method: 将SSA构建为少样本任务，评估不同自监督预训练策略，分析领域相似性和领域差距对迁移性的影响，在三个少样本设置下进行实验

Result: 领域相关小数据集表现优于大规模不相关数据集，在1-shot、2-shot和5-shot设置下分别达到60.16%、66.03%和73.65%的准确率，加入手术特定数据平均提升1.22%准确率和2.28% F1分数

Conclusion: 预训练数据与目标任务的领域相关性比数据规模更重要，适当加入领域相关的外部数据可以显著提升少样本手术技能评估性能
...
Result: 在多样化视频数据集上的实验评估表明，该框架在不同道路和环境条件下具有可靠性和适应性，能够识别非联网车辆的危险驾驶行为。

Conclusion: 这种基于视觉的方法为检测分心和受损驾驶提供了一种有效解决方案，无需依赖车辆间通信，具有广泛的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统学习方法在固有图像分解中产生的纹理缺失和过度平滑问题，特别是在复杂场景（变化光照和丰富纹理）下的挑战。

Method: 设计纹理引导正则化项，将分解问题转化为优化框架，通过纹理信息推断光照和材质信息，分离材质纹理和光照效果。

Result: 结合新颖的纹理感知先验能够产生优于现有方法的结果，为真实世界图像生成高质量的固有图像。

Conclusion: 纹理引导的正则化方法在固有图像分解中表现出色，特别是在处理复杂光照和纹理场景时具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 研究PnP方法和扩散模型之间的区别与联系，特别是在去噪机制和采样过程方面的差异，旨在为单像素成像等逆问题提供更好的解决方案。

Method: 将扩散过程解耦为三个可解释阶段：去噪、数据一致性强制和采样；提出混合数据一致性模块，线性组合多个PnP式保真度项，直接在去噪估计上应用混合校正。

Result: 在单像素成像任务上的实验结果表明，该方法实现了更好的重建质量，改善了测量一致性而不破坏扩散采样轨迹。

Conclusion: 通过统一框架成功整合了学习先验和物理模型，混合数据一致性模块有效提升了逆问题求解的性能，为相关领域提供了新的解决方案。
...
TL;DR: 本文提出了一种全自动两阶段框架，通过超声图像分析和多源数据融合来实现无创颅内压分级，显著提高了测量的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 颅内压升高对脑功能构成严重威胁，需要及时监测。虽然腰椎穿刺是金标准，但其侵入性和风险促使需要无创替代方案。视神经鞘直径(ONSD)是一个有前景的生物标志物，但当前临床测量方法存在操作不一致、主观性强和阈值变异等问题。

Method: 开发了一个全自动两阶段框架：第一阶段处理眼底超声视频，进行帧级解剖分割、基于国际共识声明的规则关键帧识别和精确ONSD测量；第二阶段融合ONSD指标和临床特征来预测ICP分级。
...
<details>
  <summary>Details</summary>
Motivation: 现代IC制造中存在多样化的精细缺陷，传统方法需要外部正常样本进行比对，但在IC图像中由于布局变化和对准困难，这种方法很脆弱。观察到缺陷主要是局部的，而每个图像仍包含丰富的可重复正常模式。
...
Conclusion: 该无监督框架能够有效处理IC制造中的缺陷分割问题，无需外部正常样本支持，具有良好的性能和鲁棒性。
...
TL;DR: DRiFt是一个医学视觉语言模型框架，通过特征解耦技术分离临床相关信号和任务无关噪声，提高模型在分布偏移下的鲁棒性和泛化能力。
...
Method: 提出结构化特征解耦框架DRiFt，使用参数高效调优（LoRA）和可学习提示令牌显式分离临床相关信号和任务无关噪声，并通过生成高质量、临床基础的图像-文本对来增强跨模态对齐。

Result: 相比之前的基于提示的方法，在分布内性能上提高了+11.4%的Top-1准确率和+3.3%的Macro-F1分数，同时在未见数据集上保持强大的鲁棒性。

Conclusion: 解耦任务相关特征和精心设计的对齐策略显著增强了模型泛化能力，减少了在领域偏移下的不可预测行为，为构建更安全、更可信的临床用视觉语言模型提供了重要见解。
...
TL;DR: FS-Diff是一种基于扩散模型的联合图像融合和超分辨率方法，通过语义引导和清晰度感知机制，在军事侦察等实际应用中处理低分辨率、弱语义信息的多模态图像，生成高质量融合结果。


<details>
  <summary>Details</summary>
Motivation: 现实应用中（如军事侦察、远程检测），多模态图像的目标和背景结构容易损坏，存在低分辨率和弱语义信息的问题，导致现有融合技术效果不佳。需要一种能够同时处理图像融合和超分辨率的方法。

Method: 将图像融合和超分辨率统一为条件生成问题，使用清晰度感知机制进行自适应低分辨率感知和跨模态特征提取。采用双向特征Mamba提取全局特征，通过改进的U-Net网络实现随机迭代去噪过程，在多个噪声级别进行训练。

Result: 在六个公共数据集和新构建的AVMS基准（600对图像）上的实验表明，FS-Diff在多个放大倍数下优于最先进方法，能够恢复更丰富的细节和语义信息。
...
<details>
  <summary>Details</summary>
Motivation: 图像级自监督学习虽然取得了进展，但在学习补丁级密集表示时存在过度分散现象，即同一实例/类别的补丁在表示空间中分散，影响了密集下游任务的性能。

Method: 1) 通过蒸馏补丁对应关系打破严格空间对齐；2) 提出噪声容忍排序损失，将AP损失扩展到连续目标；3) 设计对象感知过滤器，通过交叉注意力将补丁映射到基于对象的空间。

Result: 在各种任务上的实证研究充分证明了该方法的有效性，代码已开源。

Conclusion: 该方法通过显式语义集中成功解决了密集自监督学习中的过度分散问题，为补丁级表示学习提供了有效解决方案。
...
Result: 在哈佛数据集上的实验表明，该方法在变化输入数量的医学图像融合中达到最佳性能，同时在红外-可见光、多曝光和多焦点图像融合任务中也表现出有效性和优越性。

Conclusion: FlexiD-Fuse方法能够端到端处理任意数量的输入模态，在多种图像融合任务中均表现出优异的性能，为临床诊断提供了更灵活的解决方案。
...
TL;DR: 本研究针对撒哈拉以南非洲地区MRI数据稀缺问题，开发了一个轻量高效的3D Attention UNet模型，通过迁移学习和残差块增强，在资源受限环境下实现了优异的脑胶质瘤分割性能。


<details>
  <summary>Details</summary>
Motivation: 解决撒哈拉以南非洲地区高质量标注医学影像数据稀缺的问题，为资源受限的临床环境开发可部署的高性能脑肿瘤分割模型。

Method: 采用3D Attention UNet架构，集成残差块增强，利用BraTS 2021数据集预训练权重进行迁移学习，在BraTS-Africa数据集上进行评估。

Result: 在95例MRI数据上取得优异分割性能：增强肿瘤(ET)Dice分数0.76，坏死和非增强肿瘤核心(NETC)0.80，周围非功能性半球(SNFH)0.85。模型仅90MB，单例推理时间低于1分钟。

Conclusion: 该模型在数据质量和数量有限的情况下表现出良好的泛化能力，具有在低资源医疗环境中部署的实用性，有助于缩小全球健康AI公平性差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度伪造检测数据集存在局限性，使用过时的生成方法、真实性低或仅包含单一人脸图像，无法有效检测现代AI生成的合成媒体。特别是在政治敏感背景下，深度伪造技术加剧了错误信息的传播。

Method: 通过分析社交媒体帖子识别深度伪造传播的多模态方式，进行人类感知研究评估合成图像的真实性，构建包含真实图像和对应合成图像的大规模数据集，并建立众包对抗平台让参与者生成具有挑战性的合成图像。

Result: 研究发现最新开发的专有模型生成的合成图像越来越难以与真实图像区分，普通公众难以准确识别。构建的数据集包含300万真实图像和96.3万高质量合成图像，覆盖多种生成模型。

Conclusion: 通过构建专门针对政治背景的综合数据集和持续更新的众包对抗平台，能够确保深度伪造检测方法保持鲁棒性和适应性，有效保护公共讨论免受复杂错误信息威胁。
...
TL;DR: 该论文提出使用全身线性和角动量作为约束来连接局部运动与全局运动，通过引入新的损失函数来确保生成的运动动量与真实数据一致，从而减少脚部滑动和抖动，改善平衡性。


<details>
  <summary>Details</summary>
Motivation: 现有研究将人体运动分解为根关节的局部运动和全局运动分别处理，但这两个组件并非独立。全局运动源于与环境的互动，而这种互动又由身体配置变化驱动。现有模型难以精确捕捉这种物理耦合关系，而从关节扭矩和外部力推导全局轨迹计算复杂且昂贵。

Method: 提出使用全身线性和角动量作为约束来连接局部运动与全局运动。动量反映了关节级动力学对身体空间运动的聚合效应，提供了物理基础的方式来关联局部关节行为与全局位移。在此基础上引入新的损失项，强制生成的动量分布与真实数据中的观测动量保持一致。

Result: 引入该损失函数后，减少了脚部滑动和抖动，改善了平衡性，同时保持了恢复运动的准确性。

Conclusion: 通过动量约束连接局部和全局运动是一种有效的物理基础方法，能够显著改善运动生成的质量和物理合理性。
...
TL;DR: 本文提出了一种无需预分割或标注的漫画线稿图像区域对应预测方法，使用基于Transformer的框架学习图像内和跨图像的块级相似性，并通过边缘感知聚类和区域匹配算法实现连贯的区域级对应。


<details>
  <summary>Details</summary>
Motivation: 漫画线稿图像之间的区域对应关系理解是漫画处理的基础任务，能够支持自动线稿上色和中间帧生成等下游应用，但这一任务在无预分割或标注的现实场景中仍未被充分探索。

Method: 将线稿图像分割成块集，提出基于Transformer的框架学习块级相似性，应用边缘感知聚类和区域匹配算法将块级预测转换为连贯的区域级对应关系。

Result: 在多个数据集上的实验表明，该方法实现了高块级准确率（96.34%）并生成一致的区域级对应关系。

Conclusion: 该方法在无需预标注的情况下有效预测漫画线稿的区域对应关系，展示了在实际漫画应用中的潜力。
...
TL;DR: 提出SGDF方法和GDCN网络解决多视图聚类中低质量数据问题，通过生成扩散融合机制提升聚类性能
...
TL;DR: DualTrack是一种新颖的双编码器架构，通过解耦的局部和全局编码器分别处理超声图像中的细粒度特征和高级解剖特征，实现了传感器无关的3D超声重建，在大型公共基准测试中达到了最先进的精度。


<details>
  <summary>Details</summary>
Motivation: 传统3D超声系统成本高且复杂，而现有的传感器无关3D超声方法要么忽略全局特征，要么将其与局部特征提取紧密耦合，限制了鲁棒建模能力。需要一种能够同时有效利用局部和全局特征的方法。

Method: 提出DualTrack双编码器架构：局部编码器使用密集时空卷积捕获细粒度特征；全局编码器使用图像主干网络（如2D CNN或基础模型）和时间注意力层嵌入高级解剖特征和长程依赖关系；轻量级融合模块结合这些特征来估计轨迹。

Result: 在大型公共基准测试中，DualTrack实现了最先进的精度和全局一致的3D重建，平均重建误差低于5毫米，优于先前的方法。

Conclusion: DualTrack通过解耦的局部和全局特征提取，有效解决了传感器无关3D超声重建问题，证明了双编码器架构在同时处理不同尺度特征方面的优势，为临床3D超声的广泛应用提供了有前景的替代方案。
...
TL;DR: 该论文提出Align4Gen方法，通过将视频扩散模型的中间特征与预训练视觉编码器的特征表示对齐来提升视频生成质量，并基于新指标分析不同编码器的适用性。
...
Method: 提出Align4Gen方法，包含新指标评估视觉编码器的判别性和时序一致性，并设计多特征融合和对齐机制集成到视频扩散模型训练中。

Result: 在无条件和类别条件视频生成任务上，Align4Gen方法在多个指标上显示出改进的视频生成效果。
...
TL;DR: InterAct是一个大规模3D人机交互基准数据集，通过整合和优化现有数据，解决了现有数据集在运动质量、标注和接触问题方面的不足，提供了30.70小时的高质量人机交互数据。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模人体运动捕捉数据集在人机交互建模方面存在局限，缺乏高质量的运动和标注数据，且存在接触穿透、漂浮和手部运动不正确等伪影问题。

Method: 1) 整合和标准化21.81小时的人机交互数据并添加详细文本标注；2) 提出统一的优化框架，基于接触不变性原理减少伪影并修正手部运动，同时保持人机关系并引入运动变化，将数据集扩展到30.70小时；3) 定义六个基准任务并开发统一的人机交互生成建模方法。

Result: 实现了最先进的性能表现，广泛实验验证了该数据集作为推进3D人机交互生成研究的基础资源的实用性。
...
TL;DR: 本研究探讨了基于深度学习的阿尔茨海默病MRI诊断中存在的种族和性别相关的捷径学习与偏见问题，通过多数据集和多模型实验证实了这种偏见的存在，并提出了更公平诊断工具的基础。


<details>
  <summary>Details</summary>
Motivation: 深度学习算法在MRI脑部成像诊断中可能遭受捷径学习问题，即使用与输出标签无关的虚假特征进行预测。当这些特征与受保护属性（如种族和性别）相关时，会导致对代表性不足群体的性能偏见。

Method: 首先研究DL算法是否能从3D脑部MRI扫描中识别种族或性别，以确认基于种族和性别的分布偏移存在。然后调查训练集在种族或性别上的不平衡是否会导致模型性能下降，表明捷径学习和偏见。最后对不同脑区域的特征归因进行定量和定性分析。使用多个数据集和DL模型（ResNet和SwinTransformer）。

Result: 实验证明基于深度学习的AD分类中存在种族和性别相关的捷径学习和偏见。DL算法能够从脑部MRI扫描中识别种族和性别，训练集不平衡会导致模型性能下降，特征归因分析揭示了不同脑区域在受保护属性和AD分类任务中的差异。

Conclusion: 本研究揭示了DL-based AD诊断中的偏见问题，为开发更公平的脑部MRI诊断工具奠定了基础。代码已开源供进一步研究使用。
...
Method: 采用权重共享的Siamese编码器架构，集成LoRA和Adapter模块，使用SAM2和DINOv3作为骨干网络，配合轻量级解码器。
...
Conclusion: PeftCD在准确性、效率和泛化性之间实现了最佳平衡，为大规模VFMs在真实遥感变化检测应用中的适配提供了强大且可扩展的范式。
...
Method: 使用常微分方程系统构建机制模型捕捉肿瘤时间动态（包括放疗效果），结合梯度引导的DDIM进行图像合成，确保与预测生长和患者解剖结构对齐

Result: 在BraTS成人和儿童胶质瘤数据集上训练，在60个纵向儿童DMG病例上评估，生成逼真的随访扫描，提供肿瘤生长概率图，显示95% Hausdorff距离的临床相关生长范围和方向性
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型基准测试主要关注识别准确性，但忽略了模型识别错误选项的关键能力，这在安全关键应用中至关重要。

Method: 基于全景场景图数据集构建，利用细粒度场景图标注提取真实实体和关系，使用GPT-4-Turbo生成选择题，并经过严格人工筛选，每个问题都包含'以上都不是'选项。

Result: 评估了多种最先进的多模态大语言模型（包括通用和专用推理模型），并分享了有价值的发现和见解。

Conclusion: HumbleBench通过明确包含错误选项拒绝测试，填补了当前评估套件的关键空白，为安全关键环境中的模型可靠性提供了更现实的衡量标准。
...
<details>
  <summary>Details</summary>
Motivation: 探索多模态学习中理解和生成任务的统一范式，通过重建目标实现编码器和解码器的相互增益，提升视觉理解和图像生成的质量。

Method: 提出UAE框架：1) 使用大规模长上下文图像描述预训练解码器；2) 通过强化学习进行Unified-GRPO三阶段训练：冷启动、生成促进理解、理解促进生成；3) 引入Unified-Bench评估基准。

Result: 随着RL训练进展，编码器自动生成更具描述性的标题，解码器展现出理解复杂描述的能力，实现高保真度的图像重建，证明了双向信息流的有效性。

Conclusion: 该方法成功实现了多模态理解与生成的统一，通过重建目标促进了编码器和解码器的协同提升，为多模态学习提供了新的insightful范式。
...
<details>
  <summary>Details</summary>
Motivation: 现有VAE或扩散方法无法充分建模人体运动的几何结构和物理约束，需要一种能在黎曼流形上精确建模姿态、速度和加速度动态的更高阶运动先验。
...
Conclusion: NRMF通过几何感知的神经距离场方法，为3D人体运动恢复提供了更鲁棒和物理合理的解决方案，在多个任务上实现了显著性能提升。
...
Method: 通过最优参数化线性去噪器展示其与深度神经去噪器相似的局部性特性，从理论和实验证明局部性源于自然图像像素相关性

Result: 开发了比先前专家设计的替代方案更能匹配深度扩散模型预测分数的解析去噪器

Conclusion: 扩散模型中的局部性主要来自图像数据集的统计特性，而非网络架构的归纳偏置，这一发现有助于构建更准确的解析模型
...
<details>
  <summary>Details</summary>
Motivation: 当前空间智能模型受限于大规模高质量训练数据的稀缺性，现有数据集在规模、多样性和标注丰富度方面存在局限，特别是缺乏真实动态场景的相机运动真值数据。

Method: 收集超过21,000小时的原始视频，通过分层过滤流程处理成270万个视频片段，总计7,089小时动态内容，并使用标注流程添加相机位姿、深度图、动态掩码、结构化描述和序列化运动指令等详细空间语义信息。

Result: 构建了包含丰富多样场景、相机运动和密集3D标注的大规模数据集，数据分析显示其丰富性和多样性能够直接促进模型泛化能力和性能提升。

Conclusion: SpatialVID数据集为视频和3D视觉研究社区提供了重要资源，解决了当前数据稀缺问题，有助于推动空间智能模型的进一步发展。
