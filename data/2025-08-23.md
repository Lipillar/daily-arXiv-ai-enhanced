...
Method: 采用协同训练框架，在单一SFT阶段整合三种安全行为：积极（合法/亲社会）、消极（无过滤/风险倾向）和拒绝（拒绝导向/保守），通过系统指令动态激活不同行为模式。
...
<details>
  <summary>Details</summary>
Motivation: 现有多语言数学推理基准主要通过翻译生成，保留了英语中心实体，缺乏反映准确本土实体（如人名、组织名、货币）的社会文化任务数据集，导致低资源语言的数学推理能力落后于英语。

Method: 开发了一个LLM驱动的文化本地化框架，能够从现有资源自动构建包含本土名称、组织和货币的数学应用题数据集。

Result: 实验表明，翻译的基准测试可能在适当的社会文化背景下掩盖真实的多语言数学能力，该框架有助于减轻英语中心实体偏见，并在引入本土实体时提高各种语言的鲁棒性。

Conclusion: 该框架有效解决了多语言数学推理中文化本地化数据稀缺的问题，通过自动生成本土化数据集提升了LLM在低资源语言中的数学推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何利用少量易得的数据资源改进通用指令调优大语言模型的机器翻译能力，特别是针对斯洛文尼亚语这样的低资源语言

Method: 使用Direct Preference Optimization (DPO)方法，通过程序化策划和增强公共数据集的子集来训练模型。生成训练数据集时使用两个LLM（GaMS-9B-Instruct和EuroLLM-9B-Instruct）翻译英文维基百科文章，并基于启发式规则和COMET等自动评估指标对翻译结果进行质量排序

Result: 微调后的模型在翻译维基百科文章时，相比基线模型分别获得了约0.04和0.02的COMET分数提升，并且更一致地避免了语言和格式错误

Conclusion: DPO方法能够有效利用相对较少的数据资源显著提升大语言模型的机器翻译性能，特别是在低资源语言场景下表现出色
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的对话式可解释AI系统在英语方面表现良好，但面临多语言训练数据稀缺和自定义输入支持有限的问题，阻碍了多语言泛化能力。

Method: 1) 扩展MultiCoXQL多语言数据集覆盖5种语言；2) 提出新的解析方法提升多语言解析性能；3) 创建Compass数据集支持自定义输入提取；4) 使用不同规模的LLM和BERT模型进行单语、跨语言和多语言评估。

Result: 在MultiCoXQL数据集上评估了三种LLM的多种解析策略，在Compass数据集上进行了全面的多语言评估，展示了所提方法的有效性。

Conclusion: 该研究通过构建多语言数据集和改进解析方法，有效解决了ConvXAI系统在多语言环境和自定义输入处理方面的挑战，为多语言可解释AI系统的发展提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时对齐方法推理成本过高，限制了实际应用，需要找到更高效的对齐方式

Method: 使用奖励偏移推测采样算法，让草稿模型与人类偏好对齐，目标模型保持不变，通过修改接受标准和奖励token分布来恢复RLHF最优解

Result: 在测试时弱到强对齐实验中，以显著降低的推理成本获得了优越的黄金奖励分数

Conclusion: 该方法有效解决了测试时对齐的效率瓶颈问题，实现了效果和效率的双重提升
...
<details>
  <summary>Details</summary>
Motivation: 现有召回指标依赖词汇重叠导致错误，LLM评估方法容易产生幻觉和错位，在医疗、法律等关键领域需要更可靠的完整性评估

Method: 三阶段框架：1)将答案分解为自包含事实 2)通过词汇和语义过滤缩小候选匹配范围 3)通过结构化蕴含检查验证对齐

Result: 在三个长形式问答基准测试中，相比强基线方法在召回准确性方面有显著提升
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型虽然在各种语言任务中表现出色，但由于其自回归特性，容易产生幻觉和复杂推理问题。特别是在结构化预测任务中，需要确保输出的结构一致性。

Method: 结合LLM与组合推理方法，研究不同提示策略来估计LLM置信度值，使用符号推理增强预测的结构一致性，并通过结构化预测目标进行校准和微调。

Result: 实验表明，无论采用何种提示策略，在提示基础上添加符号推理都能产生更一致和准确的预测。使用结构化学习目标进行校准和微调可显著提升困难任务的性能。

Conclusion: 在LLM时代，结构化学习仍然具有重要价值，将符号推理与LLM结合可以有效解决幻觉和结构一致性问题，提升模型在结构化预测任务中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于Common Crawl的数学数据集由于脆弱的提取启发式方法、有损的HTML到文本转换以及无法可靠保留数学结构，导致质量下降，需要更鲁棒的科学文本提取方法。

Method: 开发了新颖的领域无关管道，利用lynx进行布局感知渲染和基于LLM的定向清理阶段，恢复各种格式的数学内容（MathJax、KaTeX、MathML），保持方程和代码块的结构完整性，去除样板文件，标准化为LaTeX表示法。

Result: 构建了Nemotron-CC-Math-3+（133B tokens）和Nemotron-CC-Math-4+（52B tokens）两个高质量数学语料库，其中Nemotron-CC-Math-4+超越了所有现有开放数学数据集，包含比之前最高质量数学预训练数据集FineMath-4+多5.5倍的token。在Nemotron-T 8B模型上预训练后，在MATH和MBPP+基准上分别获得+4.8到+12.6和+4.6到+14.3的提升，同时改善了MMLU和MMLU-Stem的通用领域性能。

Conclusion: 这是第一个能够从噪声网络规模数据中可靠提取科学内容（包括数学）的管道，在数学、代码和通用推理方面带来了可衡量的提升，为开放数学预训练语料库设立了新的最先进水平。作者开源了代码和数据集以支持开源社区。
...
<details>
  <summary>Details</summary>
Motivation: 人们经常提出带有错误假设的问题，这类问题没有常规答案。大型语言模型(LLM)由于幻觉问题经常生成误导性回答，需要先识别错误假设才能正确回答。

Method: 将问题简化为事实验证，利用外部证据来减轻幻觉。通过生成和验证原子假设的方法，结合检索到的证据进行回答。

Result: 在五个LLM上的实验表明：(1)整合检索证据是有益的；(2)生成和验证原子假设能带来更多改进，并通过指定错误假设提供可解释的答案。

Conclusion: 利用外部证据和原子假设验证的方法能有效识别问题中的错误假设，减少LLM的幻觉，提供更准确和可解释的答案。
...
<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型和视觉语言模型在处理需要深度推理、持续上下文理解、实体跟踪和多步指令跟随的复杂多轮视觉任务时仍面临挑战，现有基准无法充分捕捉真实世界多模态交互的复杂性。

Method: 构建了包含300个复杂多轮对话场景的MMDR-Bench数据集；提出了CoLVLM Agent框架，采用迭代的"记忆-感知-规划-执行"循环来增强现有LVLM模型，无需重新训练基础模型。

Result: CoLVLM Agent在MMDR-Bench上平均人类评估得分达到4.03，显著超越GPT-4o(3.92)和Gemini 1.5 Pro(3.85)，在推理深度、指令遵循和错误抑制方面表现出明显优势。

Conclusion: 该框架的模块化设计和迭代方法在复杂多模态交互中表现出有效性，能够保持长时间对话中的稳定性能，为解决多模态深度推理任务提供了有效解决方案。
...
Method: 在通道级别应用非结构化稀疏性，对KV进行剪枝，并在注意力分数计算时动态恢复被剪枝的条目。

Result: 在相同内存预算下可处理更长序列，相比基于驱逐的方法减少30%以上KV缓存存储，即使在80%的激进剪枝比例下性能下降也小于5%。

Conclusion: SPARK方法有效平衡了效率与模型精度，且与现有KV压缩和量化技术正交兼容，可进一步加速处理。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在领域特定场景中的性能不足问题，传统RAG方法存在幻觉和延迟问题，而持续预训练成本高且缺乏跨域灵活性。

Method: 提出S2K框架，采用内部-外部知识自选择策略、选择性监督微调、结构化推理数据生成管道，并集成GRPO来增强推理能力。

Result: 在医疗、法律和金融QA基准测试中，S2K一致优于现有方法，性能匹配领域预训练大语言模型但成本显著降低。

Conclusion: S2K提供了一种成本效益高的解决方案，通过渐进式知识获取策略有效解决了领域知识长尾分布问题，实现了与昂贵预训练方法相当的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工设计的任务特定演示或精选库，专家工作量大且提示工程复杂低效，难以应对工具多样性和任务难度扩展的挑战。

Method: 提出自引导的逐步经验回放(SEER)方法，从持续更新的经验池中进行细粒度逐步检索，增量式扩充经验池，实现模型性能的持续提升。

Result: 在ToolQA基准测试中，简单问题平均提升6.1%，困难问题提升4.7%；在τ-bench真实场景中，Qwen2.5-7B和72B模型分别获得7.44%和23.38%的准确率提升。

Conclusion: SEER方法通过动态经验池和逐步检索机制，有效解决了大语言模型在多步骤工具调用中的工具选择、参数生成和工具链规划问题，实现了持续的性能改进。
...
Method: 使用六种方法生成检查表，在八种模型规模上进行实验，分析检查表项目与人工评估的相关性，通过成对比较和直接评分任务进行评估。

Result: 选择性使用检查表在成对比较设置中能提升评估性能，但在直接评分中效果不一致；即使与人工评分相关性低的检查表项目也反映了人工编写的标准。

Conclusion: 需要更清晰地定义客观评估标准来指导人工和自动评估，因为人工评估本身可能存在不一致性。
...
<details>
  <summary>Details</summary>
Motivation: 小语言模型在资源受限环境中具有计算优势，但词汇相关组件（嵌入和语言建模头）由于词汇量大，占用大量内存，成为边缘设备部署的关键瓶颈。现有静态词汇剪枝方法存在信息损失和缺乏灵活性的问题。

Method: 基于词汇局部性原理（单次推理只需少量词汇子集）和词汇相关组件计算特性的不对称性，提出VocabTailor框架：通过卸载嵌入实现内存约束处理，为LM头采用混合静态-动态词汇选择策略，实现按需加载词汇组件。

Result: 在各种下游任务的综合实验中，VocabTailor将词汇相关组件内存使用减少高达99%，任务性能仅有最小程度或没有下降，显著优于现有静态词汇剪枝方法。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在英语指令跟随方面表现出色，但在泰语等低资源语言中的性能仍未充分探索。现有基准测试通常依赖翻译，缺乏现实应用所需的文化和领域特定细微差别。

Method: 通过多阶段质量控制流程创建人工编写的泰语数据集，涵盖四个专业领域和七种任务类型。进行两项研究：(1)零样本评估显示在文化和专业特定任务上的性能差距；(2)指令调优研究，通过消融实验分离本土监督的效果。

Result: 在WangchanThaiInstruct上微调的模型在领域内和领域外基准测试中均优于使用翻译数据的模型。零样本评估揭示了在文化和专业特定任务上的显著性能差距。

Conclusion: 研究结果强调了需要基于文化和专业背景的指令数据，以改善低资源、语言多样化环境中LLM的对齐性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决代码切换语音技术中数据集稀缺的问题，因为代码切换在真实对话中很常见但对多语言语音技术构成挑战。

Method: 使用SWORDS算法（同义词替换单词），通过将选定单词替换为翻译词并考虑词性来生成代码切换语音，保持句子语义不变。
...
Result: 教师角色LLMs表现出比人类教师更理想化和极化的人格特征，在抽象道德推理方面表现优异，但在情感复杂情境中表现较差。推理能力强的模型更容易受到有害提示注入攻击，存在能力与安全性之间的悖论。

Conclusion: 这是首个评估教师角色LLMs伦理和心理对齐的基准，为教育AI提供了重要的评估工具，揭示了模型能力与安全性之间的权衡关系。
...
<details>
  <summary>Details</summary>
Motivation: RAG系统在检索上下文与LLM参数知识冲突时，往往无法正确解决这种冲突，导致模型偏向错误的外部信息而非正确的内部知识

Method: 提出CARE框架，包含上下文评估器和基础LLM。上下文评估器通过紧凑记忆令牌嵌入和对抗软提示训练，识别不可靠上下文并生成指导信号
...
Method: 提出了一个时间常识提取管道，利用大语言模型从文本中自动挖掘时间常识，并基于SAMSum和RealNews语料库构建了TComQA数据集。通过众包验证数据集质量。

Result: TComQA数据集在提取时间常识方面达到了超过80%的精确度。使用TComQA训练的模型在时间问答任务上表现优于基于现有数据集微调的大语言模型。
...
Result: 在多种语言上评估，包括UCLA语音语料库的零样本测试，显示出强大的跨语言泛化能力，参数量少于当前方法但性能相当。
...
<details>
  <summary>Details</summary>
Motivation: 知识图谱补全模型评估存在挑战，不同数据集和指标（如MRR、Hit@k）的排名结果不一致，难以进行整体比较和模型选择，需要统一的评估框架。

Method: 基于距离平均解的评价方法（EDAS），将多数据集、多指标的性能综合为单一标准化分数（0-1范围），提供全局视角的评估。

Result: 在FB15k-237和WN18RR等基准数据集上的实验表明，EDAS能有效整合多指标多数据集性能，提供一致、稳健且可推广的评估框架。

Conclusion: EDAS为解决知识图谱补全模型评估中的不一致性问题提供了有效的元度量方法，支持更明智的模型选择和跨数据集公平评估。
...
Method: 系统性地回顾了大语言模型评测基准的现状和发展，将283个代表性基准分为三类：通用能力（涵盖核心语言学、知识和推理等方面）、领域特定（关注自然科学、人文社会科学和工程技术等领域）和目标特定（关注风险、可靠性、智能体等）。

Result: 识别出当前基准存在数据污染导致分数膨胀、文化语言偏见导致不公平评估、缺乏过程可信度和动态环境评估等问题。

Conclusion: 为大语言模型评测基准的未来创新提供了可参考的设计范式，强调了需要解决现有基准的局限性以更准确地评估模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的可信度问题日益受到关注，但现有评估方法往往只关注狭窄方面，忽视了多模态带来的风险。需要建立一个全面的评估框架来系统性地分析和缓解这些可信度问题。

Method: 提出了MultiTrust-X基准，定义了一个三维框架：包含5个可信度方面（真实性、鲁棒性、安全性、公平性、隐私性）、2种新型风险类型（多模态风险和跨模态影响）以及多种缓解策略。基于该分类法，构建了32个任务和28个精选数据集。
...
Conclusion: 研究结果提供了未来改进的实用见解，如推理在更好平衡安全性和性能方面的益处。基于这些见解，提出了推理增强安全对齐（RESA）方法，使模型具备思维链推理能力来发现潜在风险，取得了最先进的结果。
...
Method: 基于信息论框架，利用drafter输出分布的熵和边际不确定性度量，动态调整每轮推测生成的token数量，并使用相同置信度信号调制验证过程

Result: 在机器翻译和摘要任务上相比标准推测解码显著加速，同时保持或改进了BLEU和ROUGE分数

Conclusion: 该方法提供了一种原则性的即插即用方法，可在不同不确定性条件下实现大语言模型的高效鲁棒解码
...
Method: 在固定数据、计算和优化条件下，将词汇表从24K扩展到196K进行对照研究，量化token化文本复杂度，分析词级损失分解，并通过约束嵌入范数来验证假设

Result: 大词汇表主要通过降低前2500个高频词的不确定性来减少交叉熵，尽管稀有词损失上升；约束嵌入范数会逆转增益，表明模型利用了而非受制于token频率不平衡

Conclusion: 词汇表大小的效益应重新理解为"降低token化文本复杂度有帮助"，这为tokenizer-模型协同设计提供了简单原则性杠杆，并阐明了预训练中语言模型扩展的损失动态
...
Method: 系统分析134篇相关论文，引入基于证据的LLM文本生成的统一分类法，调查300个评估指标（涵盖7个关键维度），重点关注使用引用、归属或引文进行证据文本生成的方法。

Result: 建立了该领域的统一分类体系，识别了不同方法的特征和代表性技术，为基于证据的文本生成研究提供了系统化的分析框架和评估基准。

Conclusion: 研究揭示了基于证据的LLM文本生成领域的关键挑战和发展方向，为该领域的标准化和未来发展提供了重要指导，有助于提升LLM输出的可靠性和可信度。
...
<details>
  <summary>Details</summary>
Motivation: 大型音频-语言模型虽然具备音频感知能力，但其在处理音频和文本模态间冲突信息时的表现尚未得到充分研究，需要评估模型在多模态冲突情况下的信息优先级选择能力。

Method: 开发了首个专门评估音频-文本不一致对处理的综合基准MCR-BENCH，通过广泛的音频理解任务评估，研究文本偏向的影响因素，并通过监督微调探索缓解策略，分析模型置信度模式。

Result: 研究发现LALMs在模态不一致时显著偏向文本输入，经常忽视音频证据，导致音频中心任务性能大幅下降，即使在矛盾输入情况下模型仍表现出持续的过度自信。

Conclusion: 研究结果强调了在训练过程中需要改进模态平衡，开发更复杂的融合机制，以增强处理冲突多模态输入时的鲁棒性，这对实际应用的可靠性具有重要意义。
...
<details>
  <summary>Details</summary>
Motivation: 大语音语言模型（LSLMs）发展受到碎片化架构和缺乏透明度的阻碍，模型权重发布时通常缺少对应的训练数据和配置，导致难以进行系统性比较和复现研究。

Method: 提出LLaSO框架，包含三个核心组件：1）1200万实例的语音-文本对齐语料库LLaSO-Align；2）1350万实例的多任务指令调优数据集LLaSO-Instruct；3）可复现的标准化评估基准LLaSO-Eval。并基于此训练了38亿参数的参考模型LLaSO-Base。

Result: LLaSO-Base模型取得了0.72的标准化分数，建立了强大且可复现的基线，超越了同类可比模型。分析发现，虽然更广泛的训练覆盖能提升性能，但在未见任务上仍存在显著的泛化差距，特别是在纯音频场景中。

Conclusion: 通过完整发布数据、基准测试和模型，LLaSO为LSLMs研究建立了基础性开放标准，有助于统一研究努力并加速社区驱动的进展。
...
Method: 对隐私保护语言建模方法进行系统性综述研究，深入分析各种方法的优缺点

Result: 提供了隐私保护语言建模方法的全面概述，揭示了现有方法的优势和局限性
...
Conclusion: M-Help数据集填补了心理健康求助检测领域的重要空白，为AI模型提供了更全面的训练基础，有助于更精准地识别和帮助需要支持的人群
...
<details>
  <summary>Details</summary>
Motivation: 消除语言理解中的误解，特别是在非对等词汇（如食物、服装、文化传统词汇等）的翻译方面，因为这些词汇在目标语言中往往没有直接对应词，需要专业方法来处理。

Method: 采用文献研究方法，系统分析并提出了从源语言到目标语言的非对等词汇翻译的不同方法和规则。

Result: 成功提出了多种非对等词汇的翻译方法，并将25个达里语和乌兹别克语的非对等词汇专业地翻译成了英语和俄语。

Conclusion: 通过系统的方法和规则，可以有效解决非对等词汇的翻译问题，促进语言间的准确理解和交流，但仍有大量词汇需要进一步研究和处理。
...
<details>
  <summary>Details</summary>
Motivation: 现有可编程任务导向对话代理的有效性依赖于准确的对话状态跟踪，需要更好的方法来提升状态跟踪的准确性和鲁棒性。
...
Conclusion: 执行感知的状态跟踪方法有效，PyTOD展示了通过代码生成和执行反馈来提升对话状态跟踪性能的可行性。
...
TL;DR: RadReason是一个创新的放射学报告评估框架，通过细粒度错误类型评分和可解释的评分理由，解决了现有评估方法过于粗略或黑盒的问题。


<details>
  <summary>Details</summary>
Motivation: 当前放射学报告自动生成评估缺乏临床基础、可解释性和细粒度指标，现有方法要么评分过于粗略，要么依赖不透明的黑盒模型，限制了在真实临床工作流程中的应用价值。
...
Conclusion: RadReason提供了一个临床基础、可解释且细粒度的放射学报告评估框架，通过创新的动态加权和优势缩放机制，实现了更稳定的优化和更好的专家临床判断对齐。
...
<details>
  <summary>Details</summary>
Motivation: 个性化营销能显著提升客户参与度和业务增长，研究表明良好执行的个性化策略可使收入提升达40%，但现有研究主要集中在推荐系统和个性化广告，个性化优惠生成领域潜力巨大。

Method: 使用对比学习方法微调Google的T5-Small编码器-解码器语言模型，采用InfoNCE损失函数在共享嵌入空间中对齐客户画像和相关优惠，通过自适应学习行为重塑潜在空间。

Result: 在模拟客户行为和优惠接受模式的合成数据集上评估，实验结果显示相比监督微调基线，优惠接受率提升了17%。

Conclusion: 对比学习目标在推进个性化营销方面具有显著效果，SLM4Offer模型通过创新的对比学习方法成功提升了个性化优惠生成的性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理用户主观行为和偏好时存在局限性，特别是当用户浏览行为具有高度异质性和主观性时，单一模型难以充分捕捉不同用户的个性化特征。

Method: 提出HeTLM（异质性感知语言模型训练）方法，使用页面级分词器训练小模型，并通过聚类特定参数集来处理用户异质性，控制参数数量相同的情况下进行比较。

Result: 小模型在浏览语言表示上优于大型预训练或微调模型；异质性聚类特定参数模型优于同族单一模型；实现了更高的平均性能和更低的方差，表明对齐效果改善。

Conclusion: 针对主观浏览行为，小模型和聚类感知训练方法比大型通用模型更有效，能够更好地捕捉用户异质性并实现个性化对齐。
...
<details>
  <summary>Details</summary>
Motivation: 传统的课程学习使用人工定义的难度指标（如文档复杂度）在语言模型预训练中效果有限，研究者希望探索更符合模型训练过程中实际感知难度的度量方法

Method: 使用训练数据影响力（training data influence）作为难度指标对训练样本进行排序，该分数估计单个训练样本对模型输出的影响程度，然后按照此指标从简单到复杂组织训练数据

Result: 采用模型中心难度指标的课程学习方法在基准测试中比随机顺序训练的效果高出10个百分点以上

Conclusion: 课程学习对语言模型预训练确实有益，但关键在于采用更符合模型视角的难度定义，而不是传统的人工中心难度指标
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏对小型语言模型性能和环境影响系统性评估的基准测试，需要建立标准化的评估框架来全面衡量SLM的效率权衡。

Method: 开发了SLM-Bench基准测试，在4种硬件配置下评估15个SLM模型，使用23个数据集覆盖14个领域，量化11个正确性、计算和能耗指标。

Result: 评估结果显示了SLM之间的多样化权衡关系，有些模型在准确性方面表现出色，而其他模型在能源效率方面更优。
...
Method: 构建HebID语料库（5536个句子，12种社会身份标注），对比多标签/单标签编码器和2B-9B参数生成式LLM，应用分类器分析政客社交媒体帖子和议会演讲。

Result: 希伯来语调优的LLM表现最佳（macro-F1=0.74），发现了身份表达的流行度差异、时间趋势、聚类模式和性别相关变化，精英话语与公众身份优先级存在差异。

Conclusion: HebID为希伯来语社会身份研究提供了全面基础，可作为其他非英语政治语境类似研究的模型，推动了跨文化社会身份计算分析的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自回归模型需要顺序生成token，效率较低。研究团队希望开发一种能够并行生成序列的扩散语言模型，提高生成效率并增强模型的灵活性。

Method: 采用离散扩散建模方法，通过迭代去噪并行优化序列。使用简单的训练技术，包括基于自回归的LLM初始化和上下文自适应的token级噪声重调度。

Result: Dream 7B在通用、数学和编程任务上一致优于现有扩散语言模型，展现出卓越的规划能力和推理灵活性，支持任意顺序生成、填充能力和可调节的质量-速度权衡。
...
Method: 设计基于代理流的自动化系统SafetyFlow，协调7个专业代理，集成多样化工具，将人类专业知识融入自动化流程，确保过程可控和成本可控。

Result: 构建的SafetyFlowBench数据集包含23,446个查询，具有低冗余度和强判别力。在49个先进LLM上评估安全性，验证了系统的有效性和效率。

Conclusion: SafetyFlow首次实现了完全自动化的基准构建流程，提供了全面的安全基准，显著提升了LLM安全评估的效率和效果。
...
Conclusion: 训练微型模型为特定高价值应用提供了一种经济高效的替代方案，显著降低了部署成本
...
<details>
  <summary>Details</summary>
Motivation: 发现LLMs作为判别器识别有害请求的能力强于作为生成器防御有害内容的能力，存在安全不一致性，希望通过对齐这两种能力来提升模型安全性。

Method: 提出SDGO框架，通过强化学习利用模型自身的判别能力作为奖励信号，进行迭代自改进来增强生成安全性。

Result: SDGO显著提高了模型安全性，在保持通用基准上有用性的同时，对分布外越狱攻击表现出鲁棒性能。

Conclusion: 通过对齐LLMs的判别和生成能力，SDGO实现了更紧密的能力耦合，仅需少量判别样本就能进一步增强生成能力，为模型安全提供了有效解决方案。
...
TL;DR: SurGE是一个新的科学综述生成评估基准，包含测试实例和百万级学术语料库，提出了自动化评估框架来衡量生成综述的四个维度，评估显示即使先进的自反思框架也难以胜任此任务。
...
Method: 引入SurGE基准，包括主题描述、专家撰写综述及其引用文献的测试实例集合，以及百万篇论文的学术语料库作为检索池，并提出自动化评估框架测量信息覆盖度、引用准确性、结构组织和内容质量四个维度。

Result: 对多样化基于LLM的方法评估表明，综述生成仍然极具挑战性，即使对先进的自反思框架也是如此。
...
<details>
  <summary>Details</summary>
Motivation: 位置偏差（PB）表现为不同上下文位置敏感性的不均匀分布，严重损害了长上下文理解能力。尽管先前工作试图通过修改架构来缓解PB，但显著的偏差仍然存在。

Method: 提出了Pos2Distill框架，基于位置到位置的知识蒸馏原理，利用位置诱导的固有差异来抵消位置偏差本身。针对检索和推理范式分别设计了Pos2Distill-R¹和Pos2Distill-R²两个专门实例。

Result: 使用Pos2Distill方法在长上下文检索和推理任务中实现了增强的均匀性和显著的性能提升，所有上下文位置都获得改善。两个专门系统展现出强大的跨任务泛化能力。

Conclusion: Pos2Distill通过知识蒸馏有效缓解了位置偏差问题，在保持各自任务优异性能的同时，实现了跨任务的相互泛化，为长上下文处理提供了有效的解决方案。
...
TL;DR: 本文对孟加拉语词干提取方法进行全面调查，指出该领域存在显著研究空白，批判现有评估方法，并提出孟加拉语词干提取器的发展方向。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语作为全球第七大语言面临数字资源不足的问题，词干提取对于这种低资源、高屈折性语言的分析处理至关重要，能显著降低算法复杂度。

Method: 通过文献综述方法，系统调查现有孟加拉语词干提取方法，分析研究现状和存在的问题。

Result: 发现现有研究存在明显断层，缺乏可复现的实现方案，评估方法不够相关，难以应对孟加拉语丰富的形态和方言多样性。

Conclusion: 建议开发更强大的孟加拉语词干提取器，推动持续研究以提升语言分析和处理能力，填补该领域的研究空白。
...
TL;DR: 提出了EcomMMMU电商多模态数据集和SUMEI方法，发现产品图像并不总是提升模型性能，有时反而会降低效果，提出基于视觉效用预测的策略性多图像使用方法


<details>
  <summary>Details</summary>
Motivation: 电商平台拥有丰富的多模态数据，但现有数据集规模有限且设计不足，无法系统研究图像是否真正增强产品理解能力，或可能引入冗余甚至降低性能

Method: 构建包含406,190样本和8,989,510图像的EcomMMMU数据集，包含8个核心任务和专门VSS子集；提出SUMEI方法，通过预测视觉效用来策略性地使用多图像进行下游任务

Result: 分析发现产品图像并不总是提升性能，有时反而会降低效果，表明MLLMs在有效利用丰富视觉内容方面存在困难；SUMEI方法在综合实验中展现出有效性和鲁棒性

Conclusion: 电商多模态任务中需要更智能的图像使用策略，SUMEI通过数据驱动的方式有效解决了多图像利用问题，为MLLMs在电商领域的应用提供了重要见解
...
TL;DR: Deep-DxSearch是一个基于强化学习的端到端医疗诊断代理系统，通过检索增强推理显著提升医疗大语言模型的诊断准确性，在常见和罕见疾病诊断中超越GPT-4o等强基线模型。


<details>
  <summary>Details</summary>
Motivation: 医疗大语言模型在诊断中存在知识鸿沟和幻觉问题，现有检索增强方法对外部知识利用不足且推理过程缺乏可追溯性，需要开发更有效的诊断辅助系统。

Method: 构建大规模医疗检索语料库，将LLM作为核心代理、检索语料作为环境，通过强化学习训练代理策略，使用格式、检索、推理结构和诊断准确性等多维度奖励函数进行端到端训练。

Result: 在多个数据中心超越提示工程和无训练RAG方法，诊断准确性显著提升，在分布内和分布外设置下均优于GPT-4o、DeepSeek-R1等强基线模型。

Conclusion: Deep-DxSearch通过强化学习驱动的代理式RAG框架有效解决了医疗诊断中的知识利用和推理可追溯性问题，为临床医生提供更可靠精确的初步诊断支持。
...
TL;DR: ReasonZoo基准测试显示，工具集成推理(TIR)能显著提升大语言模型在数学和非数学推理任务中的表现，并通过新提出的效率指标(PAC和AUC-PCC)证明TIR能减少过度思考，提高推理效率。
...
Conclusion: 研究结果强调了TIR的领域通用优势及其在推进LLM复杂推理任务能力方面的潜力。
...
TL;DR: LiveMCP-101是一个包含101个真实世界查询的基准测试，用于评估AI代理使用多种MCP工具解决多步骤任务的能力，实验显示即使前沿LLM的成功率也低于60%。


<details>
  <summary>Details</summary>
Motivation: 虽然MCP为工具集成提供了标准化框架，但缺乏在真实动态场景中评估AI代理使用多样化MCP工具有效解决多步骤任务的基准测试。

Method: 通过迭代LLM重写和人工审查精心策划101个真实世界查询，引入基于真实执行计划而非原始API输出的新颖评估方法，更好地反映真实环境的动态特性。

Result: 实验表明即使前沿LLM的成功率也低于60%，详细消融和错误分析揭示了工具编排中的不同失败模式和令牌使用效率低下问题。
...
Result: 在WFLW、COFW和300W三个面部关键点基准数据集上达到最先进性能，训练收敛速度比传统方法快2.2倍，同时保持更好或相当的精度。

Conclusion: Soft-argmax并非面部关键点检测的唯一有效方法，基于结构化预测的替代方案能够提供更快的训练收敛速度和竞争性的性能表现。
...
<details>
  <summary>Details</summary>
Motivation: 传统卷积神经网络在处理复杂数据结构时存在局限，研究旨在利用GCN处理关系数据的优势，结合Voronoi图的空间分割特性来提升图像分类性能，特别是在复杂场景和细粒度分类任务中。

Method: 将图像表示为图结构（像素或区域作为顶点），使用对应的Delaunay三角剖分优化图表示，然后应用图卷积网络进行分类处理。

Result: 在多个基准数据集上实现了显著的预处理效率提升和分类准确率提高，超越了现有最先进方法，特别是在复杂场景和细粒度分类任务中表现优异。
...
Conclusion: YOPO证明了物体检测和9自由度姿态估计可以统一实现高性能，无需额外数据，为RGB-only姿态估计提供了有效解决方案
...
TL;DR: 提出Paired-Sampling Contrastive Framework统一训练方法，通过自动匹配的真实和攻击自拍对学习模态无关的活体线索，在物理-数字攻击检测基准上达到2.10% ACER，优于现有方案。


<details>
  <summary>Details</summary>
Motivation: 传统人脸识别系统使用独立模型处理物理呈现攻击和数字伪造，增加了系统复杂性和推理延迟，且无法应对组合攻击向量。需要统一的检测框架。

Method: 采用配对采样对比学习框架，利用自动匹配的真实和攻击自拍照对，学习与模态无关的活体检测特征。

Result: 在第6届人脸反欺骗挑战赛统一物理-数字攻击检测基准上取得2.10%的平均分类错误率，计算量仅为4.46 GFLOPs，训练时间不到1小时。

Conclusion: 该方法提供了一个轻量级、高效的统一活体检测解决方案，优于现有方法，具有实际部署价值。
...
Method: 通过在混合步骤区间注入扰动实现攻击效果，无需处理所有时间步；采用选择性RGB通道策略，对红色通道应用注意力图，对绿色和蓝色通道使用GradCAM引导的扰动。

Result: 在ImageNet数据集上，对VGGNet源模型，TAIGen对ResNet成功率为70.6%，对MNASNet为80.8%，对ShuffleNet为97.8%；PSNR超过30dB，保持良好视觉质量。

Conclusion: TAIGen实现了最低的鲁棒准确率，表明其是最具影响力的攻击方法，防御机制对其生成的图像净化效果最差，同时生成速度比现有扩散攻击快10倍。
...
<details>
  <summary>Details</summary>
Motivation: 现有隐蔽视觉感知方法主要局限于mask域，未能充分利用RGB域的潜力，且在处理不确定性方面存在不足

Method: 将CVP任务建模为数学优化问题并展开为多阶段深度网络，包含CORE模块（mask域可逆建模）、CARE模块（RGB域可逆建模）和FINE模块（基于Bernoulli扩散模型的细化）
...
Method: 采用混合视觉变换器架构，包含新颖的Mix Twin编码器（交替使用空间缩减全局注意力和局部组注意力机制），以及轻量级LR-ASPP解码器进行多尺度特征聚合，实现甲烷分割和饮食分类的统一框架。

Result: 在分割任务上达到74.47% mIoU和83.63% mF1，仅使用3.348M参数和3.428G FLOPs，推理速度114.9 FPS；饮食分类准确率达到100%，建立了首个牛肉牛甲烷排放数据集（11,694标注帧）。
...
TL;DR: CurveFlow提出了一种新的流匹配框架，通过引入曲率正则化来学习非线性轨迹，显著提升了文本到图像生成中的语义一致性和图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有整流流模型基于数据与噪声分布之间的线性轨迹，这种线性强制零曲率可能导致生成过程经过数据流形的低概率区域，影响生成图像与文本描述的语义对齐。

Method: 引入曲率正则化技术，直接惩罚轨迹内在动态的突变，学习平滑的非线性轨迹，构建曲率感知的流匹配框架。

Result: 在MS COCO 2014和2017数据集上的实验表明，CurveFlow在文本到图像生成任务中达到最先进性能，在BLEU、METEOR、ROUGE和CLAIR等语义一致性指标上显著优于标准整流流变体和其他非线性基线。

Conclusion: 曲率感知建模显著增强了模型忠实遵循复杂指令的能力，同时保持高质量的图像生成效果，验证了轨迹曲率与语义对齐之间的重要关联。
...
Result: 仅在合成失真上训练的HiRQA能有效泛化到真实退化场景（如镜头光晕、雾霾、运动模糊、低光条件），在合成和真实基准测试中达到最先进性能，轻量版HiRQA-S推理时间仅3.5ms。

Conclusion: HiRQA通过自监督的层次排序和质量对齐方法，解决了NR-IQA的泛化问题，在不依赖主观标签或参考图像的情况下实现了优异的性能，具有良好的可扩展性和实时部署能力。
...
TL;DR: 本文提出了一种基于投资组合理论的边缘资源管理策略，用于在动态边缘环境中保证多视角3D重建的可靠性，通过遗传算法优化相机选择来应对时空相关的中断问题。


<details>
  <summary>Details</summary>
Motivation: 多视角3D重建在应急响应等关键应用中需要近实时性能，但边缘环境的动态性和操作困难会导致相机操作的中断，从而持续降低重建质量。需要一种可靠的方法来保证在时空相关中断情况下的重建质量。

Method: 采用投资组合理论启发的边缘资源管理策略，通过遗传算法解决投资组合理论优化问题，快速收敛到最优相机选择方案，以应对可能的系统中断。

Result: 使用公开和定制3D数据集验证，证明所提出的相机选择策略相比传统基线策略，在时空中断情况下能够有效保证可靠的3D重建质量。

Conclusion: 基于投资组合理论的边缘资源管理策略能够有效应对动态边缘环境中的时空相关中断，为关键应用提供可靠的多视角3D重建服务。
...
Method: 集成专用医学视觉编码器和LVLM核心，采用多任务提示工程和多阶段微调，从眼底图像理解病理特征并生成详细诊断报告

Result: 在DDR数据集上达到84.55%平衡准确率和79.92% F1分数，概念检测达到77.95% BACC和66.88% F1，人工评估确认生成解释的高流畅性和临床实用性

Conclusion: XDR-LVLM能够弥合自动化诊断与临床需求之间的差距，通过提供稳健且可解释的见解来促进临床采用
...
<details>
  <summary>Details</summary>
Motivation: 城市网格模型虽然越来越普及，但缺乏真实纹理限制了其在虚拟城市导航和自动驾驶中的应用。现有图像和视频扩散模型难以直接应用于3D场景生成，要么无法保证跨视图一致性，要么难以准确遵循预定义相机路径。

Method: 采用三阶段流水线：1) 使用级联外绘ControlNet生成几何一致的稀疏视图；2) 通过AGInpaint组件传播更密集的中间视图；3) 使用GCAlign模块全局消除视觉不一致性。同时通过3D高斯泼溅技术在网格表面初始化高斯球来重建3DGS场景。

Result: 该方法在几何对齐和生成质量方面优于现有方法，能够生成高质量、风格一致的室外场景，并支持通过重光照和风格迁移技术进行多样化渲染。

Conclusion: MeSS方法成功解决了城市网格模型的纹理生成问题，为虚拟城市导航和自动驾驶应用提供了高质量的3D场景合成解决方案，具有很好的实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 在自动驾驶等安全关键应用中，现有方法使用规则智能体训练强化学习代理，但需要确保规则智能体被正确建模以发现潜在故障

Method: 使用基于学习的方法来推导规则智能体的对抗行为策略，通过对抗训练来识别可能导致系统失败的场景

Result: 评估表明对抗智能体能够有效降低所有规则智能体的累积奖励，证明其能够成功制造故障场景

Conclusion: 该方法为安全关键系统提供了一种有效的对抗测试框架，能够识别和验证系统在对抗环境下的脆弱性
...
<details>
  <summary>Details</summary>
Motivation: 现有肾脏病理学数据集和自动化方法主要提供二元语义掩码，限制了形态学量化的精确性，传统后处理技术在分离复杂肾脏组织结构时效果有限。

Method: 整合分水岭、骨架化和形态学操作到统一框架中，采用自适应几何细化和可定制超参数调优，针对不同功能单元类别进行系统参数优化。

Result: 实验结果表明该方法优于单独的经典方法和简单组合，能够稳健地分离二元掩码中的粘连和异质结构，实现更优的实例分离。

Conclusion: DyMorph-B2I管道能够促进更准确的肾脏病理学形态计量分析，该工具已公开可用。
...
Method: 采用改进的时空特征提取方法，通过循环神经网络聚合特征，在基于图神经网络的事故预测框架上进行优化

Result: 在三个公开数据集上实验表明，STAGNet模型在平均精度和平均碰撞时间指标上均优于先前方法，跨数据集测试也表现良好
...
TL;DR: TriMM是首个前馈式3D原生生成模型，通过多模态协作编码整合RGB、RGBD和点云数据，利用triplane潜在扩散模型生成高质量3D资产，在小规模训练数据下实现与大模型竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成架构大多局限于单模态或3D结构，忽略了多模态数据的互补优势，限制了可用训练数据集的范围。为了充分利用多模态进行3D建模，需要开发能够整合不同模态优势的生成模型。

Method: 1) 引入协作多模态编码，整合模态特定特征同时保持其独特表征优势；2) 引入辅助2D和3D监督提升多模态编码的鲁棒性和性能；3) 基于嵌入的多模态代码，使用triplane潜在扩散模型生成高质量3D资产。

Result: 在多个知名数据集上的实验表明，TriMM通过有效利用多模态，在使用少量训练数据的情况下，实现了与大规模数据集训练模型竞争的性能。在RGB-D数据集上的额外实验验证了将其他多模态数据集整合到3D生成的可行性。

Conclusion: TriMM成功展示了多模态数据在3D生成中的价值，通过协作多模态编码和triplane扩散模型，能够生成纹理和几何细节都更优越的3D资产，为多模态3D生成提供了有效解决方案。
...
TL;DR: 提出了一种面向中心的原型对比聚类框架，通过软原型对比模块和双一致性学习模块解决类间冲突和原型漂移问题
...
Method: 包含软原型对比模块（使用样本属于聚类中心的概率作为权重计算类别原型）和双一致性学习模块（对齐同一样本的不同变换和不同样本的邻域）
...
<details>
  <summary>Details</summary>
Motivation: 传统无人机视觉语言导航存在轨迹长、机动复杂的问题，需要人工干预或过于详细的指令。利用无人机高机动性提供多粒度视角，同时保持可控的运动空间。

Method: 提出双无人机协作框架AeroDuo：高空无人机使用多模态大语言模型进行环境推理，低空无人机采用轻量级多阶段策略进行导航和目标定位，仅交换坐标信息确保效率。

Result: 构建了包含13,838条协作轨迹的HaL-13k数据集，包含未见地图和未见目标验证集，系统评估模型在新环境和陌生目标上的泛化能力。

Conclusion: 双高度无人机协作方法能够有效利用各自优势，高空提供环境推理，低空负责精确导航，通过最小化信息交换实现高效协作，为无人机VLN任务提供了新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然在各生成任务中达到最先进效果，但其顺序生成过程需要多步采样，效率较低。现有方法如DDIM通过非马尔可夫过程减少采样步数，但缺乏对原始扩散过程能否以马尔可夫方式实现相同效率的理解。

Method: 提出跳过步采样机制，允许在迭代生成过程中绕过多个中间去噪步骤，而不是传统的逐步细化。该方法从与标准扩散模型相同的训练目标推导而来，表明通过马尔可夫方式的加速采样是预训练扩散模型的内在特性。

Result: 在OpenAI ADM、Stable Diffusion和Open Sora等流行预训练扩散模型上的大量实验表明，该方法能够以显著减少的采样步骤实现高质量生成。

Conclusion: 跳过步采样是一种有效的扩散模型加速方法，它证明了马尔可夫方式的加速采样是预训练扩散模型的内在能力，并且可以与DDIM等现有方法结合使用以获得更好的生成效果。
...
Result: 实验证明Comp-X能高效理解编码请求，具备出色的文本交互能力，同时在单一框架下保持可比压缩性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法主要针对工业场景设计，在病理学中面临计算限制、组织结构多样性和缺乏可解释性等挑战，特别是在疾病相关数据有限的情况下。
...
TL;DR: RATopo提出了一种冗余分配策略，通过改进Transformer解码器结构实现一对多分配，提升车道拓扑推理性能
...
Method: 交换Transformer解码器的交叉注意力和自注意力层顺序，保留冗余车道预测；实例化多个并行交叉注意力块增强车道多样性
...
TL;DR: 本文提出了DesignCLIP框架，利用CLIP视觉语言模型处理设计专利分析，通过类别感知分类和对比学习，在专利分类和检索任务上超越现有方法。
...
Result: DesignCLIP在专利分类和检索等下游任务中 consistently 超越基线和最先进模型，展示了多模态方法在专利分析中的潜力。

Conclusion: 多模态方法有望推动专利分析的进步，DesignCLIP框架为设计专利应用提供了有效的解决方案，并能通过多模态专利检索增强设计创意和创新。
...
TL;DR: TPA是一种利用图像-文本基础模型和提示感知对比学习的超声视频胎儿先天性心脏病分类方法，通过时间建模和不确定性量化实现最先进的性能


<details>
  <summary>Details</summary>
Motivation: 解决超声视频中先天性心脏病检测面临的图像噪声、探头位置变异性问题，克服现有机器学习方法忽视时间信息、仅限于二分类且缺乏预测校准的局限性

Method: TPA方法使用图像编码器提取视频子片段每帧特征，通过可训练时间提取器聚合以捕捉心脏运动，利用边缘铰链对比损失将视频表示与类别特定文本提示对齐，并引入CVAESM模块学习潜在风格向量来调节嵌入和量化分类不确定性

Result: 在CHD检测私有数据集和EchoNet-Dynamic公共数据集上，TPA获得85.40%的宏F1分数，预期校准误差降低5.38%，自适应ECE降低6.8%，在三分类任务上宏F1提升4.73%

Conclusion: TPA是一个集成时间建模、提示感知对比学习和不确定性量化的框架，在胎儿先天性心脏病超声视频分类中表现出卓越性能，为临床可靠性提供了增强的校准能力
...
TL;DR: 本文提出了BasketLiDAR数据集和新型MOT框架，结合LiDAR点云和多视角相机数据，在篮球场景中实现实时3D球员追踪，解决了传统相机方法在遮挡条件下的性能限制。


<details>
  <summary>Details</summary>
Motivation: 传统多相机系统受限于2D视频数据和复杂3D重建处理，难以实现实时分析。篮球场景中10名球员快速复杂移动和频繁遮挡使得多目标追踪(MOT)极具挑战性。

Method: 构建首个体育MOT领域的多模态数据集BasketLiDAR，包含LiDAR点云和同步多视角相机数据。提出基于LiDAR高精度3D空间信息的新型MOT算法，包括纯LiDAR实时追踪管道和LiDAR-相机融合的多模态追踪管道。

Result: 实验结果表明，该方法实现了传统相机方法难以达到的实时操作，在遮挡条件下仍能保持优异的追踪性能。数据集包含4,445帧和3,105个球员ID，完全同步的3个LiDAR传感器和3个多视角相机数据。

Conclusion: BasketLiDAR数据集和提出的MOT框架为体育分析提供了有效的解决方案，结合LiDAR的3D空间优势，成功解决了篮球场景中的实时追踪挑战，为战术分析和性能评估提供了可靠工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有COD方法依赖大量训练和计算资源，基础模型如SAM需要高质量提示但手动生成成本高，需要更高效实用的解决方案

Method: 两阶段方法：第一阶段通过无监督聚类构建检索数据库进行特征检索生成粗掩码提示；第二阶段使用SAM2进行精分割

Result: 在基准COD数据集上性能达到或超越SOTA方法，所有实验在个人笔记本上完成，计算效率高
...
TL;DR: VideoEraser是一个无需训练的即插即用框架，通过两阶段方法有效防止文本到视频扩散模型生成不良内容，在四个任务上平均减少46%的不良内容生成。


<details>
  <summary>Details</summary>
Motivation: 文本到视频扩散模型可能被滥用来生成有害或误导性内容，这些模型通常使用未经授权的个人身份、艺术作品和有害材料进行训练，导致不良内容的不可控生产和传播。

Method: VideoEraser采用两阶段过程：选择性提示嵌入调整（SPEA）和抗干扰噪声引导（ARNG），作为即插即用模块与现有T2V扩散模型无缝集成。

Result: 在对象擦除、艺术风格擦除、名人擦除和显式内容擦除四个任务上，VideoEraser在效能、完整性、保真度、鲁棒性和泛化性方面均优于现有方法，平均减少46%的不良内容生成。

Conclusion: VideoEraser提供了一种有效的训练无关解决方案，能够显著抑制T2V扩散模型中不良内容的生成，为解决隐私、版权和安全问题提供了实用工具。
...
TL;DR: 本研究开发了一个端到端深度学习框架，使用姿态检测和序列建模技术来预测行人过马路意图，比较了GRU、LSTM和1D CNN三种模型的效果。
...
Conclusion: 通过融合DINOv2的语义表示，RCDINO成功提升了多模态3D目标检测的性能，为自动驾驶和机器人应用提供了有效的解决方案。
...
Result: 发现视频信息提取主要在早期层完成，形成两阶段处理；中间层对视频问答有超大影响；时空建模更依赖语言引导检索而非视频token自注意力。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在图像分析中取得显著进展，但大型完全训练模型的计算需求仍然是一个考虑因素。传统微调方法通常更新所有参数，可能导致过拟合和更高的计算成本。

Method: BioTune采用进化算法来识别用于微调的聚焦层集合，旨在优化模型在目标任务上的性能，通过选择性微调层来减少可训练参数数量。

Result: 在九个不同领域的图像分类数据集上的评估表明，BioTune相比AutoRGN和LoRA等现有微调方法，实现了竞争性或改进的准确性和效率。

Conclusion: 通过将微调过程集中在相关层的子集上，BioTune减少了可训练参数数量，可能降低计算成本，并促进跨不同数据特征和分布的更高效迁移学习。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS压缩方法存在两个限制：(1)只能将中等规模场景压缩到兆字节范围，对大规模场景或场景集合仍不实用；(2)缺乏对长期归档后场景变化的适应机制

Method: 提出图像条件高斯泼溅量化器(ICGS-Quantizer)，联合利用高斯间和属性间相关性，使用跨所有训练场景的共享码本，编码、量化和解码过程联合训练，支持基于捕获图像的场景条件解码
...
TL;DR: DriveSplat提出了一种基于神经高斯表示的动态-静态解耦方法，用于驾驶场景的高质量3D重建，通过区域体素初始化、可变形神经高斯和深度/法线先验监督，在Waymo和KITTI数据集上实现了最先进的新视角合成性能。
...
TL;DR: 本文针对深度学习模型在抽象推理方面的瓶颈，通过Raven渐进矩阵问题作为基准测试，提出因果链建模方法并改进基线模型DIO，逐步提出三种改进方法来解决互信息最大化的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型在抽象推理方面存在根本性瓶颈，Raven渐进矩阵问题被用作评估抽象推理能力的权威基准。本文旨在通过解决RPM问题来提升机器智能的抽象推理能力。

Method: 采用因果链建模视角分析RPM任务的完整因果链，设计基线模型DIO的网络架构。针对DIO优化目标的局限性，逐步提出三种改进方法来解决互信息最大化的问题。

Result: 实验发现DIO模型的优化目标（最大化上下文与正确选项间互信息的变分下界）无法让模型真正获得预定义的人类推理逻辑，主要原因是下界紧致性影响互信息最大化效果，以及互信息作为统计度量无法捕捉因果关系。

Conclusion: 需要克服互信息最大化的局限性，本文提出了三种渐进式改进方法来更好地解决RPM问题，提升模型的抽象推理能力。
...
Method: 基于脉冲神经网络的关键帧提取器、动态聚合图推理器解耦上下文一致性、变分推理重建模块处理多通道特征融合的不确定性和噪声
...
TL;DR: 本文分析了MAE如何学习输入图像的空间相关性，推导了线性MAE学习的特征，发现掩码比例和补丁大小可以选择捕获短程和长程空间相关性的特征，并扩展到非线性MAE分析。


<details>
  <summary>Details</summary>
Motivation: 尽管MAE在视觉基础模型预训练中表现强大，但在新数据集上需要大量超参数调优（掩码比例、补丁大小、编码器/解码器层数），MAE超参数与下游任务性能之间的关系尚未得到充分探索。

Method: 通过分析推导线性MAE学习的特征，研究掩码比例和补丁大小如何选择捕获不同范围空间相关性的特征，并将分析扩展到非线性MAE，验证MAE表示如何适应数据集的空间相关性。

Result: 研究发现MAE表示能够适应数据集的空间相关性，超越了二阶统计量，掩码比例和补丁大小可以作为选择特征范围的有效参数。

Conclusion: 研究提供了关于如何在实际应用中选择MAE超参数的见解，揭示了MAE学习空间相关性的机制，为超参数调优提供了理论指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于学习的多帧方法主要采用滑动窗口方式聚合相邻帧信息，但这种方法没有考虑整个视频片段的联合优化，忽略了滑动窗口外的全局时间信息，导致计算冗余和性能次优。

Method: 设计了双向传播策略，包含前向和后向传播分支。首先使用局部时间运动融合(LTMF)模块建模目标帧与相邻帧的局部时空依赖关系，然后使用全局时间运动融合(GTMF)模块进一步聚合全局传播特征与局部融合特征。最后融合双向聚合特征并输入检测头进行检测。

Result: 大量实验证明，BIRD方法不仅达到了最先进的性能，而且显示出快速的推理速度。

Conclusion: 提出的双向时间信息传播方法能够有效利用局部和全局时间信息，在红外小目标检测任务中实现了优异的性能和效率。
...
TL;DR: 该论文使用YOLOv8目标检测框架开发了一种基于深度学习的汽车表面微小凹陷自动检测方法，通过定制数据集和实时数据增强技术，在检测精度和推理速度方面表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统汽车损伤检测方法劳动密集、手动操作且经常忽略微小表面缺陷，需要更快速精确的自动化检测解决方案。

Method: 创建包含不同光照条件、角度和纹理的标注汽车表面图像数据集，使用YOLOv8m模型及其定制变体YOLOv8m-t4和YOLOv8m-t42，采用实时数据增强技术进行训练。

Result: YOLOv8m-t42模型表现最佳，精度0.86、召回率0.84、F1分数0.85，mAP@0.5达到0.60，PR曲线面积0.88，优于YOLOv8m-t4模型。

Conclusion: YOLOv8m-t42模型在微小凹陷检测方面具有更高的准确性和实用性，适合实时应用如自动化保险评估和汽车检测。
...
Method: 提出MATR模型，使用Transformer架构进行双阶段序列对齐，编码查询视频和目标视频之间的相关性，并采用自监督预训练技术来定位视频中的随机片段。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS在极稀疏视角输入下性能严重下降，因为依赖SfM进行初始化但SfM在稀疏视角下无法准确重建3D几何结构

Method: 使用稠密立体模块逐步估计相机位姿并重建全局稠密点云进行初始化；提出一致性视角插值模块生成额外监督信号；引入多尺度拉普拉斯一致性正则化和自适应空间感知多尺度几何正则化

Result: 在仅使用2个训练视角的极稀疏条件下，PSNR指标显著提升2.75dB，合成图像失真最小且保留丰富高频细节

Conclusion: 该方法有效解决了3DGS在稀疏视角输入下的性能瓶颈，为实际应用提供了更实用的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 解决现有轻量级模型在医学图像分割中因性能妥协和缺乏注意力机制导致的全局上下文感知能力不足问题，以及同一卷积核下的通道冗余问题

Method: 采用异构层内核提取局部高频信息并缓解通道冗余，集成稀疏Transformer-卷积混合分支捕获低频全局信息
...
<details>
  <summary>Details</summary>
Motivation: 现有的单视角方法ECON在3D人体重建方面表现良好，但缺乏利用多视角信息的能力。为了提升重建质量和几何精度，需要开发能够有效整合多视角信息的方法

Method: 提出JMBO算法，跨所有输入视图联合拟合SMPL-X人体模型，确保多视角一致性。使用优化后的身体模型作为低频先验，通过法线图积分添加几何细节，整合前后视角的法线图来捕捉精细表面细节

Result: 实验结果表明MExECON相比单视角基线方法在保真度上持续提升，与现代少样本3D重建方法相比具有竞争力

Conclusion: MExECON成功扩展了单视角方法到多视角场景，无需网络重新训练即可实现多视角收益，在穿衣人体重建方面表现出色
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态图像融合方法面临模态不对齐、高频细节破坏和任务特定限制等挑战，需要一种通用且高效的解决方案来提升融合质量和应用范围。

Method: 提出AdaSFFuse框架，包含自适应近似小波变换(AdaWAT)进行频率解耦，以及空间-频率Mamba块实现跨域融合，通过可学习映射动态调整以适应不同模态。

Result: 在红外-可见光、多焦点、多曝光和医学图像融合四个任务上的大量实验表明，AdaSFFuse具有卓越的融合性能，同时保持低计算成本和紧凑网络结构。

Conclusion: AdaSFFuse通过创新的频率解耦和跨域融合机制，有效解决了多模态图像融合的关键挑战，在性能与效率之间实现了良好平衡，为通用图像融合提供了有力解决方案。
...
TL;DR: ExtraGS是一个用于自动驾驶场景轨迹外推的框架，结合几何和生成先验，通过Road Surface Gaussian表示和Far Field Gaussians处理远距离物体，并采用自监督不确定性估计来选择性整合生成先验。
...
Method: 采用两阶段流程：运动初始化和运动细化。第一阶段交互式分割语义组并定义关键帧，通过插值生成粗略动画；第二阶段使用基于组的位移网络(GDN)细化动画，预测组特定位移场并利用文本到视频模型的先验
...
TL;DR: D3FNet是一种用于高分辨率遥感图像中细粒度窄道路分割的网络，通过差分注意力扩张提取、双流解码融合和多尺度扩张策略，显著提升了窄道路的提取精度。


<details>
  <summary>Details</summary>
Motivation: 解决高分辨率遥感图像中窄道路提取的挑战，包括宽度有限、拓扑结构碎片化和频繁遮挡等问题。传统模型对通用道路宽度过拟合，难以处理细粒度、遮挡和低对比度的道路段。

Method: 基于D-LinkNet编码器-解码器架构，引入三个关键创新：1)差分注意力扩张提取模块增强细微道路特征并抑制背景噪声；2)双流解码融合机制整合原始特征和注意力调制特征；3)多尺度扩张策略(1,3,5,9)减少网格伪影并改善连续性。
...
Method: 将测试时自适应重新构建为高斯概率推理任务，使用逐渐更新的类别均值和共享协方差矩阵建模类别条件似然。引入基于CLIP先验和历史知识库的轻量级正则化来纠正可能的似然偏差。无需源数据、梯度更新或完整访问目标数据。

Result: 在多样化基准测试上的广泛实验表明，该方法在各种分布偏移下实现了最先进的性能，具有卓越的可扩展性和鲁棒性。

Conclusion: ADAPT提供了一种高效、可扩展的测试时自适应解决方案，通过概率推理框架和轻量级正则化机制，解决了现有方法在计算效率和分布建模方面的局限性。
...
TL;DR: 提出一种两阶段训练策略，通过邻域感知软掩码自适应地为局部变化强烈的像素分配更高权重，引导INR模型早期关注高频细节，然后过渡到全图像训练，有效缓解频谱偏差问题。


<details>
  <summary>Details</summary>
Motivation: 隐式神经表示(INRs)存在频谱偏差问题，神经网络倾向于学习低频分量而难以捕捉高频细节(如锐利边缘和精细纹理)。现有方法主要通过架构修改或特殊激活函数来解决，本文提出从训练过程直接引导的新方向。

Method: 采用两阶段训练策略：1)使用邻域感知软掩码自适应地为局部变化强烈的像素分配更高权重，促使模型早期聚焦于精细细节；2)过渡到全图像训练。这种方法能够频率感知地为像素分配重要性。

Result: 实验结果表明，该方法 consistently 提高了重建质量，并且能够与现有的INR方法互补。

Conclusion: 作为在图像INR中为像素分配频率感知重要性的开创性尝试，本工作为缓解频谱偏差问题提供了新的途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有的半定规划松弛方法求解点云配准问题需要数百秒时间，计算效率低下，需要开发更快速的可证明全局最优求解方法。

Method: 采用线性时间凸松弛技术和contractor方法加速分支定界算法，在给定旋转轴的情况下求解截断最小二乘问题。

Result: 能够在0.5秒内求解100个点的3D点云配准问题，比STRIDE SDP求解器快100倍，并提供全局最优性证明。

Conclusion: 该方法显著提高了点云配准问题的求解效率，为处理高异常值率的鲁棒配准问题提供了实用解决方案。
...
Method: 在尼泊尔奇旺国家公园使用可见光/近红外相机陷阱和热红外无人机收集图像，构建深度学习模型（YOLOv11s和增强版Faster RCNN）进行自动识别，并进行空间模式分析。

Result: YOLOv11s模型表现最佳（精度96.2%，召回率92.3%），无人机热成像提供补充视角，空间分析识别出明确的热点区域和潜在冲突区域。
...
TL;DR: Grounded VideoDiT是一个视频大语言模型，通过扩散时间潜在编码器、物体接地表示和混合令牌方案，解决了现有视频LLM在时间感知精度上的不足，在多个基准测试中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频LLM在整体推理方面取得了显著进展，但在时间感知上仍然粗糙：时间戳仅隐式编码，帧级特征在捕捉连续性方面较弱，语言视觉对齐经常偏离感兴趣的实体。

Method: 1）扩散时间潜在（DTL）编码器增强边界敏感性和保持时间一致性；2）物体接地表示将查询实体显式绑定到局部视觉证据；3）混合令牌方案使用离散时间令牌提供显式时间戳建模。
...
TL;DR: 提出了一种弱监督方法，通过人类操作员对初始分割结果进行质量评级，训练评级模型来改进树实例分割，在正确识别树实例方面提升了34%的性能。
...
Method: 使用非微调模型或闭式算法获得初始分割结果，由人类操作员进行质量评级，训练评级模型分类分割输出，最后利用评级模型的反馈微调分割模型。

Result: 原始分割模型在正确识别树实例方面提升了34%，同时显著减少了非树实例的预测数量。

Conclusion: 该方法有效改善了树实例分割性能，但在稀疏林区小树或复杂环境中仍存在挑战，性能会有所下降。
...
Method: 基于关键特征一致性假设，通过计算特征重要性来指导对抗点云搜索，优先破坏不同架构都可能采用的关键特征，并在损失函数中约束最大偏差以保证不可感知性

Result: 在ModelNet40和ScanObjectNN数据集上的大量实验表明，CFG方法大幅优于现有最先进的攻击方法

Conclusion: CFG方法通过关键特征引导有效提升了对抗点云的迁移性，为黑盒3D对抗攻击提供了有效解决方案
...
TL;DR: 本文提出MapKD框架，通过知识蒸馏将多模态教师模型的知识转移到轻量级视觉学生模型，实现在线高精地图构建的高效推理。


<details>
  <summary>Details</summary>
Motivation: 现有在线高精地图构建方法依赖离线地图先验和多模态传感器，导致推理时计算开销大且受限于过时地图信息。需要开发低成本、高效率的纯视觉解决方案。

Method: 提出多级跨模态知识蒸馏框架MapKD，采用Teacher-Coach-Student范式：1）多模态教师模型；2）视觉教练模型桥接跨模态知识转移；3）轻量级学生模型。引入TGPD和MSRD两种蒸馏策略进行特征对齐和语义学习指导。

Result: 在nuScenes数据集上，学生模型性能提升+6.68 mIoU和+10.94 mAP，同时显著加速推理速度。

Conclusion: MapKD框架成功实现了从多模态模型到纯视觉模型的有效知识转移，为自动驾驶系统提供了高效、低成本的在线高精地图构建解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的LoD1和LoD2建筑模型缺乏立面细节，而LoD3模型虽然包含窗户、门等细节元素，但传统方法需要手动建模，难以大规模应用。

Method: 利用光线到模型先验分析获得的冲突图(CMs)，开发语义冲突图生成器(SCMG)生成合成CMs，通过语义分割真实和合成CMs，并结合纹理模型的分割结果进行置信度融合。

Result: 实验结果表明，该方法在建筑开口分割和重建方面有效，结合纹理分割的不确定性感知融合达到61%的性能。
...
TL;DR: StreamMem是一种查询无关的KV缓存压缩机制，用于流式视频理解，通过注意力分数压缩视觉token的KV缓存，在内存受限的长视频场景中实现高效问答。
...
Method: 提出StreamMem机制，以流式方式编码新视频帧，使用视觉token与通用查询token之间的注意力分数来压缩KV缓存，并维护固定大小的KV内存。
...
TL;DR: 提出了分子增强的All-in-SAM模型，通过整合分子信息来改进计算病理学中的细胞分割和分类性能，减少标注工作量并提高在资源有限环境中的可访问性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉基础模型（如SAM）在细粒度语义分割方面存在挑战，特别是在识别特定细胞亚型时表现不佳，需要开发更精确的病理图像分析方法。

Method: 采用全栈方法：1）分子增强学习减少像素级标注需求；2）使用SAM适配器增强模型语义理解；3）整合分子导向校正学习（MOCL）提高分割精度。
...
TL;DR: Waver是一个高性能的多模态生成基础模型，能够统一处理图像和视频生成，支持文本到视频、图像到视频和文本到图像生成，在多个基准测试中达到顶尖水平。


<details>
  <summary>Details</summary>
Motivation: 为了解决现有视频生成模型在运动复杂性、时间一致性和分辨率方面的限制，开发一个统一的框架来同时处理图像和视频生成任务。
...
TL;DR: ATLAS是一个从60万高分辨率扫描数据学习的高保真人体模型，通过将网格表示基于人体骨骼来解耦形状和骨骼基础，解决了现有方法在捕捉多样姿态和形状细节方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有参数化人体模型在捕捉多样化姿态和形状细节方面存在困难，主要由于训练数据多样性有限和建模假设限制，且外部身体表面与内部骨骼之间存在问题依赖关系。

Method: 使用240个同步相机捕获的60万高分辨率扫描数据，明确解耦形状和骨骼基础，将网格表示基于人体骨骼，采用非线性姿态校正方法。

Result: ATLAS在拟合未见过的多样化姿态对象方面表现更准确，定量评估显示非线性姿态校正比线性模型更有效地捕捉复杂姿态。
...
TL;DR: SceneGen是一个无需优化或资源检索的端到端框架，能够从单张场景图像同时生成多个3D资产的几何和纹理，并支持扩展到多图像输入。


<details>
  <summary>Details</summary>
Motivation: 解决从场景图像中同时合成多个3D资产的挑战性问题，推动VR/AR和具身AI领域的3D内容生成应用。

Method: 提出SceneGen框架，包含特征聚合模块整合视觉和几何编码器的局部和全局场景信息，通过位置头在单次前向传播中生成3D资产及其相对空间位置。

Result: 广泛的定量和定性评估证实了该方法的高效性和鲁棒生成能力，在单图像训练基础上能够扩展到多图像输入并获得更好的生成性能。
...
TL;DR: VAREdit是一个基于视觉自回归模型的图像编辑框架，通过重新定义图像编辑为多尺度预测问题，解决了扩散模型在指令引导编辑中的全局纠缠问题，实现了更高的编辑精度和效率。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在指令引导图像编辑中存在全局去噪过程导致编辑区域与整个图像上下文纠缠的问题，造成意外的伪修改和编辑指令遵循性差。自回归模型通过序列化合成过程提供了更好的因果性和组合性机制。

Method: 提出VAREdit框架，将图像编辑重新定义为下一尺度预测问题。基于源图像特征和文本指令生成多尺度目标特征。引入尺度对齐参考(SAR)模块，在第一个自注意力层注入尺度匹配的调节信息，解决细尺度源特征无法有效指导粗尺度目标特征预测的问题。

Result: 在标准基准测试中，VAREdit比领先的扩散方法高出30%以上的GPT-Balance分数。完成512×512图像编辑仅需1.2秒，比同等规模的UltraEdit快2.2倍。

Conclusion: VAREdit通过自回归框架和多尺度预测方法，显著提升了图像编辑的指令遵循性和效率，为指令引导图像编辑提供了新的有效解决方案。
...
Method: 将组推理建模为二次整数分配问题：候选输出作为图节点，选择子集以优化样本质量（一元项）并最大化组多样性（二元项）。通过渐进剪枝候选集来提高运行效率。

Result: 实验表明该方法相比独立采样基线和最新推理算法，显著提升了组多样性和质量。

Conclusion: 该框架可泛化到文本到图像、图像到图像、图像提示和视频生成等多种任务，使生成模型能够将多个输出视为连贯的组而非独立样本。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉扩散模型由于训练数据分辨率限制和计算资源约束，在高分辨率生成时容易出现重复模式和质量问题

Method: 针对不同类型的视频生成架构提出专用变体，基于开源视频生成框架实现高分辨率I2V和V2V合成

Result: 实验验证了该范式在扩展图像和视频模型高分辨率生成能力方面的优越性

Conclusion: CineScale能够显著提升预训练模型的高分辨率视觉生成能力，无需微调即可实现8K图像和4K视频生成
...
TL;DR: 本文提出了首个开源手术伤口数据集SurgWound和基准测试，并开发了WoundQwen三阶段学习框架，用于手术伤口诊断和个性化护理。
...
Method: 1) 创建包含697张手术伤口图像的SurgWound数据集，由3名专业外科医生标注8个临床属性；2) 建立包含视觉问答和报告生成的基准测试；3) 提出WoundQwen三阶段框架：第一阶段用5个MLLM预测伤口特征，第二阶段用2个MLLM进行诊断，第三阶段整合结果生成全面报告。

Result: 开发了首个开源手术伤口数据集和基准测试，提出了有效的三阶段诊断框架，能够分析详细伤口特征并提供个性化护理指导。

Conclusion: 该研究为手术伤口筛查提供了重要资源和方法，有助于实现个性化伤口护理、及时干预和改善患者预后，推动了该领域的发展。
