...
Motivation: 探讨LLMs认知偏见的来源，区分预训练、微调或训练随机性对偏见的影响。

Method: 1. 多次微调模型以研究训练随机性对30多种认知偏见的影响；2. 引入交叉微调（cross-tuning），交换指令数据集以隔离偏见来源。
...
<details>
  <summary>Details</summary>
Motivation: 评估LLMs作为人类受试者代理的可靠性，特别是在面对调查问题的扰动时是否表现出与人类相似的响应偏差。

Method: 测试了9种不同的LLMs，使用世界价值观调查（WVS）的问题，并应用11种扰动（如问题重述和答案结构调整），模拟了超过167,000次访谈。

Result: 所有模型均表现出不同程度的近因偏差，且对语义变化（如重述）和组合扰动敏感。较大模型通常更稳健，但仍存在局限性。

Conclusion: 提示设计和鲁棒性测试对使用LLMs生成合成调查数据至关重要，以避免潜在的偏差。
...
<details>
  <summary>Details</summary>
Motivation: 合成文本在AI开发中具有潜力，但需要系统评估其效用、公平性、隐私风险等，以促进隐私保护。

Method: 提供多维度评估工具，支持用户上传或生成合成数据，并在医疗和法律领域验证其功能。

Result: 通过标准化评估指标，提升合成文本的实用性和隐私保护效果。

Conclusion: SynthTextEval为合成文本的评估提供了标准化工具，有助于推动隐私保护的AI开发。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs在医疗领域的广泛应用，其安全性问题日益突出，尤其是对患者和临床医生的潜在影响。现有评估多关注通用安全基准，缺乏针对医疗领域的具体评估。

Method: 提出医疗领域安全评估协议，构建包含466个样本的PatientSafetyBench，覆盖5个关键类别，并以MediPhi模型为案例进行红队测试。

Result: 首次从患者、临床医生和普通用户三个视角定义了医疗LLMs的安全评估标准，为医疗领域的安全部署提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要在单对话环境中进行，而实际课堂中存在多组并发对话，需要解决重叠语音问题。

Method: 分析了单对话和多组对话中的打断检测，开发了一种对重叠语音鲁棒的打断识别方法。

Result: 提出了一种适用于课堂部署的打断识别方法，并揭示了打断在协作互动中的语言和韵律特征。

Conclusion: 研究为未来在多组对话中跟踪打断行为提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前研究依赖有限证据且对输出控制不足，需改进反驳言论的生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 解决医疗领域中生成式AI输出内容在个性化与可读性之间的平衡问题，以提升患者教育和医疗公平性。

Method: 提出MedReadCtrl框架，通过指令调整控制输出复杂度，同时保持医学意图不变。

Result: 在多个数据集和任务中表现优于GPT-4，专家偏好率达71.7%，尤其在低识字水平群体中效果显著。

Conclusion: MedReadCtrl为医疗AI提供了一种可扩展的解决方案，能够生成易读且准确的临床内容，促进患者教育和医疗公平。
...
Method: 采用LLMs、人工标注和自动提示优化（APO）构建管道，生成细粒度数据集。
...
<details>
  <summary>Details</summary>
Motivation: 目标是开发轻量级、可解释的模型，能够用自然语言解释时间序列模式，适用于隐私敏感或设备端部署。

Method: 使用合成数据集（均值回归时间序列）生成自然语言注释，并用这些注释监督小型Qwen模型的微调。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如监督微调或偏好优化）可能导致LLM代理因动作空间探索不足而选择看似合理但次优的动作。

Method: 提出SAND框架，结合自我一致性动作采样和执行引导的动作评估，生成逐步动作思考轨迹，并用于微调LLM代理。

Result: 在两个交互代理任务中，SAND比初始监督微调平均提升20%，并优于现有最佳方法。

Conclusion: SAND通过动作思考显著提升了LLM代理的性能，解决了动作探索不足的问题。
...
Method: 采用两阶段框架，先收集已验证轨迹，再在训练中重放这些轨迹，结合新生成的数据进行优化。
...
Method: 提出废话指数和四种废话分类，在三个数据集上进行实证评估。

Result: RLHF显著加剧废话，CoT提示放大特定废话形式，政治语境中废话普遍。

Conclusion: 研究揭示了AI对齐的系统性挑战，为提升LLM真实性提供新思路。
...
<details>
  <summary>Details</summary>
Motivation: 利用任务分解（人类自然规划的关键部分）提升小型开源LLM的性能尚未充分探索。

Method: PLAN-TUNING框架包括从大型LLM提取规划轨迹，并通过监督和强化学习目标微调小型模型。

Result: 在GSM8k和MATH基准测试中，性能平均提升7%；在OlympiadBench和AIME 2024上分别提升10%和12%。
...
<details>
  <summary>Details</summary>
Motivation: 提升LLM的推理能力是研究重点，但直接模拟代码执行易导致对复杂数据结构和算法的过度依赖。
...
Method: 采用DyGIE++框架，结合MatSciBERT和PubMedBERT等预训练模型，进行数据标注、整合和微调，以提升信息提取准确性。

Result: 微调的PubMedBERT在NER任务中F1-score达82.19%，MatSciBERT在RE任务中F1-score为66.10%，优于通用科学模型。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在AI中的广泛应用，人们对其能力边界（如幻觉问题）和代理任务的实际应用产生了浓厚兴趣。

Method: 从计算复杂性的角度分析LLM推理能力，展示其在复杂计算和代理任务中的局限性。

Result: 研究表明LLM无法完成超出特定复杂度的计算和代理任务，且无法验证超出其能力范围的准确性。

Conclusion: 论文揭示了LLM在复杂任务中的局限性，并讨论了其实际应用中的潜在影响。
...
Conclusion: 该框架和系统有效填补了非英语心理支持数据集的空白，并提供了高质量的心理支持服务。
...
<details>
  <summary>Details</summary>
Motivation: 探索LLMs在少样本提示下的表现，尤其是在数据质量较差或标注不一致的情况下，是否能够超越传统微调的小语言模型（SLMs）。

Method: 使用精心设计的少样本提示，结合LLMs，并尝试了辩论式提示和不同示例选择策略。

Result: 在CheckThat! 2025任务中，系统在阿拉伯语和波兰语中排名第一，其他语言中进入前四，尤其在阿拉伯语数据上表现稳健。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究多关注生成任务，且依赖开放领域数据（如维基百科），忽略了检索挑战。本文旨在填补这一空白，研究特定领域中的跨语言检索行为。

Method: 使用真实企业数据集构建基准，涵盖查询和文档的所有语言组合，并分析多语言检索行为。提出一种强制均衡检索的策略。

Result: 研究发现检索是跨语言场景的主要瓶颈，性能下降显著。提出的策略显著提升了跨语言和整体性能。

Conclusion: 跨语言检索在特定领域应用中存在挑战，改进检索策略可显著提升性能，为实际RAG应用提供了优化方向。
...
<details>
  <summary>Details</summary>
Motivation: 探索SFT和RL在多模态推理任务中的独立作用及联合效果，以优化VLM的推理能力。

Method: 通过多种训练策略（两阶段、交替、渐进）及数据混合、模型融合方法，评估SFT和RL的效果。

Result: SFT提升复杂问题性能但导致冗长，RL促进泛化和简洁性；两者结合未能实现叠加优势。
...
TL;DR: M4Doc是一个单模态到多模态对齐框架，利用多模态大语言模型（MLLMs）提升文档图像机器翻译（DIMT）的性能，解决了数据不足和视觉-文本信息交互的挑战。


<details>
  <summary>Details</summary>
Motivation: 文档图像机器翻译（DIMT）面临训练数据有限和视觉-文本信息交互复杂的挑战，需要一种高效的方法提升翻译质量和泛化能力。

Method: 提出M4Doc框架，通过将图像编码器与预训练的多模态大语言模型（MLLMs）对齐，学习视觉-文本关联，并在推理时绕过MLLM以保持高效。

Result: 实验表明M4Doc显著提升了翻译质量，尤其在跨域泛化和复杂文档图像场景中表现突出。

Conclusion: M4Doc通过轻量级设计有效解决了DIMT的挑战，为文档图像翻译提供了高效且高质量的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前RAG方法主要关注准确性和召回率，而忽略了检索效率的重要性。本文旨在证明通过改进提示和少量训练数据，可以在不依赖大规模微调的情况下提升RAG性能。

Method: 采用标准ReAct流程并改进提示，结合少量监督和基于强化学习的微调，以减少检索次数并提升效率。

Result: 改进后的方法在HotPotQA等基准测试中优于现有技术，且能以近半的检索成本实现竞争性RAG指标。

Conclusion: 通过优化提示和少量训练数据，可以在不依赖大规模微调的情况下显著提升RAG的效率和性能。
...
<details>
  <summary>Details</summary>
Motivation: 语音伪装替换（PCR）已成为中文内容审核的主要障碍，但现有方法依赖规则生成的扰动，忽略了真实用户的创造性。

Method: 提出四分类法，构建真实数据集RedNote，评估现有LLM模型，并测试拼音提示策略。

Result: 最佳模型F1-score仅0.672，零样本思维链提示效果更差，但拼音提示策略显著提升准确率。

Conclusion: 研究首次全面分类中文PCR，揭示了检测器的局限性，并提出轻量级改进方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法依赖耗时的人工参考摘要，NOIR旨在提供自动化工具，优化摘要算法和提示。

Method: 利用语言模型嵌入测量语义相似性，结合摘要长度压缩，量化语义保留与压缩的权衡。

Result: NOIR能有效捕捉语义保留与压缩的权衡，并与人类对摘要质量的感知一致。

Conclusion: NOIR为自动化评估摘要质量提供有效工具，适用于多种摘要任务。
...
<details>
  <summary>Details</summary>
Motivation: 观察到多头注意力（MHA）性能随头数增加而提升，但需保持每头隐藏维度足够大，因此希望以低成本实现更高性能。

Method: 通过低维头表示投影到高维空间，模拟更多头和隐藏维度，并引入参数高效注意力聚合（PEAA）控制参数量。

Result: 在多种数据集和任务上验证了SAS的有效性，显著优于其他注意力变体。

Conclusion: SAS方法在保持模型紧凑的同时，模拟更大模型的行为，显著提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 微调大型语言模型（LLMs）以扩展知识库资源消耗巨大，现有RAG方法在可扩展性和准确性上存在局限。

Method: K2RAG采用分治法，整合密集/稀疏向量搜索、知识图谱和文本摘要，并引入预处理步骤以减少训练时间。

Result: K2RAG在MultiHopRAG数据集上表现优异，最高平均答案相似度0.57，Q3相似度0.82，训练时间减少93%，执行速度快40%。
...
<details>
  <summary>Details</summary>
Motivation: 传统认为传输嵌入向量比原始文本更隐私安全，但Vec2Text方法挑战了这一假设，因此需要进一步验证其效果和隐私风险。

Method: 复制Vec2Text框架，验证其原始结果，并通过参数敏感性分析、敏感输入重构实验及量化防御研究扩展实验。

Result: Vec2Text在理想条件下能有效重构文本，但对输入长度敏感；高斯噪声和量化技术可降低隐私风险。

Conclusion: 需谨慎使用文本嵌入向量，并进一步研究NLP系统的防御机制。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的后训练对齐是关键挑战，因为并非所有标记对模型性能贡献均等。

Method: 利用当前策略与参考模型之间的标记级对数概率差异，选择高影响力标记进行对齐。

Result: 在Arena-Hard和MT-Bench等基准测试中，Selective-DPO优于标准DPO和蒸馏基线。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机源于对自动语音识别（ASR）及代码切换（CS）语言的日益关注，旨在填补相关研究空白。

Method: 通过收集并手动标注同行评审论文，系统分析了语言、数据集、指标、模型选择及性能。

Result: 研究总结了当前研究进展、可用资源，并指出了端到端ASR中代码切换的挑战。

Conclusion: 本文为未来研究提供了方向，指出了现有研究的不足与机遇。
...
Method: StreamUni利用LSLM和语音CoT生成多阶段输出，实现分段、策略决策和翻译生成，并提出流式CoT训练方法。
...
<details>
  <summary>Details</summary>
Motivation: 将符号知识整合到数据驱动学习算法中，需要语义一致的公式嵌入，并且这些嵌入必须可逆以实现具体需求。

Method: 训练一个Transformer解码器模型，通过STL语法的小词汇表生成有效公式，并在语义空间中优化。

Result: 模型在1个周期后即可生成有效公式，10个周期后能泛化到逻辑语义，且生成的公式更简洁且语义接近参考。
...
<details>
  <summary>Details</summary>
Motivation: 先前研究主要关注注意力头，本文从技能神经元（特别是重复神经元）的角度分析其对ICL性能的影响。

Method: 通过实验比较不同层深度的重复神经元对ICL的影响，并与归纳头（induction heads）的效果对比。

Result: 发现重复神经元对ICL性能的影响随层深度变化，并提出了减少重复输出同时保持ICL能力的策略。
...
<details>
  <summary>Details</summary>
Motivation: 传统指令调优中仅对响应标记计算损失，忽略了提示标记的重要性，可能导致性能不佳和鲁棒性不足。

Method: 提出Weighted Instruction Tuning (WIT)，系统研究提示和响应标记的加权对性能的影响，并通过实验验证其效果。

Result: 实验表明，适度加权提示标记和高加权响应标记能显著提升模型性能，并为后续偏好对齐训练提供更好起点。

Conclusion: WIT方法优于传统指令调优，为开发更鲁棒和通用的模型提供了新思路，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 扩展单字分词方法，利用平行数据中的源语言标记优化目标语言分词，以实现跨语言语义对齐。

Method: 在固定源分词器的基础上，学习目标分词器，最大化跨语言语义对齐。

Result: 机器翻译质量未提升，但语言建模困惑度一致降低。
...
Conclusion: 研究揭示了RAG中核心指复杂性的挑战，为知识密集型AI应用的改进提供了指导。
...
<details>
  <summary>Details</summary>
Motivation: 扩展Alpay代数的自引用框架，探索AI系统与文档对齐的博弈理论方法。

Method: 引入复合算子ϕ(⋅,γ(⋅))，结合固定点迭代和局部子博弈解决，利用范畴论和信息论工具。

Result: 证明了语义均衡的存在性与唯一性，并开发了验证工具如ϕ-拓扑和Yoneda引理测试。

Conclusion: 该框架为AI嵌入空间提供了实用的语义模式传播工具，超越了纯数学抽象。
...
Method: DocCHA将诊断过程分解为三个阶段：症状提取、病史获取和因果图构建，每个模块使用可解释的置信度评分。

Result: 在两个中文咨询数据集上，DocCHA诊断准确率提升5.18%，症状召回率提高30%，对话轮次仅小幅增加。

Conclusion: DocCHA实现了结构化、透明且高效的诊断对话，为多语言和资源受限环境中的可信LLM临床助手铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 现有动态主题建模技术缺乏统一的流程和用户友好的探索工具，难以有效揭示文本数据中的主题演变趋势。

Method: DTECT提供统一工作流，包括数据预处理、多模型架构、评估指标，并引入LLM驱动的自动主题标注、趋势分析和交互式可视化。

Result: DTECT显著提升了主题建模的可解释性和用户友好性，支持更高效的主题动态跟踪。

Conclusion: DTECT通过整合多项功能，为用户提供了一个强大的工具来理解和分析文本数据的主题演变。
...
Method: MIRIX包含六种结构化记忆类型（核心、情景、语义、程序、资源记忆和知识库），并通过多代理框架动态控制记忆更新和检索。
...
Result: IM-RM因依赖标记级线索，泛化能力较差，尤其是在标记级分布变化时。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法在透明性和可扩展性上不足，需要更严格的基准测试。

Method: 构建MedThink-Bench基准，提出LLM-w-Ref框架，结合专家级推理和可扩展性评估。

Result: LLM-w-Ref与专家判断高度相关，小模型表现优于大模型。

Conclusion: MedThink-Bench为医学推理评估提供了基础工具，促进LLM在临床中的安全应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有MEL方法未解决提及歧义和模态内容动态选择问题，导致语义匹配不充分。
...
<details>
  <summary>Details</summary>
Motivation: 随着图像生成模型（如Stable Diffusion）能力的提升，其潜在的伦理、法律和社会问题日益突出。去学习技术被提出以消除不良概念，但其鲁棒性尚未充分验证。

Method: Recall通过优化对抗性图像提示，利用扩散模型的多模态条件能力，测试现有去学习方法的鲁棒性。

Result: 实验表明，Recall在对抗效果、计算效率和语义保真度上优于现有基线，揭示了去学习机制的脆弱性。
...
<details>
  <summary>Details</summary>
Motivation: 促进深度学习模型在生物医学图像分析中的透明性、信任度和临床采用，弥补现有综述在模态感知和多模态学习方面的缺失。

Method: 系统分类XAI方法，提出模态中心分类法，分析多模态学习和视觉语言模型的作用，总结评估指标和开源框架。

Result: 提供了针对生物医学图像分析的XAI方法综合分类，强调了多模态学习的潜力，并总结了评估工具和挑战。

Conclusion: 该综述为生物医学图像分析中可解释深度学习的发展提供了深入基础，指出了未来研究方向。
...
TL;DR: 论文研究了多模态大语言模型（MLLMs）中的幻觉现象，提出模态冲突是导致幻觉的原因，并构建了MMMC数据集。通过三种方法（提示工程、监督微调和强化学习）缓解幻觉，实验表明强化学习效果最佳。
...
TL;DR: 论文提出了一种在GNSS不可用环境下，利用YOLOv8模型和视觉特征匹配进行自主海上目标船只识别与定位的方法。
...
Result: 在MBZIRC2023竞赛中进行了真实环境实验，验证了方法的有效性，并评估了视角对检测和定位精度的影响。

Conclusion: 该方法在GNSS不可用环境下能有效识别和定位目标船只，为自主海上监视提供了可行方案。
...
Method: 采用对比学习增强编码器特征提取，结合MASPP和CA模块优化多尺度特征融合和边界重建。
...
TL;DR: 通过EEG信号与多级语义描述对齐，结合预训练扩散模型，实现视觉解码的先进方法。


<details>
  <summary>Details</summary>
Motivation: 探索如何从EEG信号中解码视觉体验，解决EEG空间分辨率不足的问题。

Method: 使用对比学习将EEG信号映射到多级语义描述，再通过预训练扩散模型生成图像。

Result: 在EEGCVPR数据集上取得最优性能，揭示了EEG信号与语义层次的神经认知关联。

Conclusion: 语义中介框架为EEG视觉解码提供了认知对齐的新途径。
...
TL;DR: 论文分析了现有视频生成模型的局限性，并提出了改进长视频生成的方法和分类体系。
...
Method: 研究32篇论文，识别关键架构和训练策略，构建分类体系。

Result: 提出了一种新的分类法，并比较了不同方法的架构设计和性能特征。

Conclusion: 通过系统分析，为长视频生成提供了改进方向和分类框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有衣物更换重识别方法依赖额外模型或标注，资源消耗大。本文探索颜色信息作为轻量级替代方案。

Method: 提出CSCI方法，利用RGB信息直接学习颜色相关偏差（Color See）并分离身份特征（Color Ignore），引入S2A自注意力机制防止信息泄露。

Result: 在四个CC-ReID数据集上验证，图像和视频重识别性能均有提升，如LTCC提升2.9%，PRCC提升5.0%。

Conclusion: 颜色信息可作为低成本解决方案，有效减少衣物更换重识别中的外观偏差。
...
TL;DR: 提出了一种自动化视频分割流程，通过机器学习实现文本提示的灵活对象检测、逐帧图像分割和视频跟踪，显著提升VFX生产效率。


<details>
  <summary>Details</summary>
Motivation: 解决VFX制作中掩模生成慢且资源密集的问题。

Method: 采用机器学习技术，包括文本提示的对象检测、逐帧图像分割和视频跟踪，结合容器化部署。

Result: 显著减少人工操作，加快初步合成速度，提供全面的分割数据。

Conclusion: 该流程显著提升了VFX生产效率，并被艺术家快速采用。
...
<details>
  <summary>Details</summary>
Motivation: 传统生物识别在多样化活动中面临身份特征与运动和外观变化纠缠的挑战，现有方法依赖额外视觉数据但易受提取误差影响。

Method: 提出DisenQ框架，通过结构化文本指导解耦生物特征、运动和非生物特征，减少对额外视觉数据的依赖。

Result: 在三个活动视频基准测试中取得最优性能，并在传统视频识别任务中表现出强泛化能力。

Conclusion: DisenQ通过语言指导有效解耦特征，提升了活动生物识别的准确性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有LMMs在语言覆盖上存在偏差和不公平输出，缺乏多语言能力的评估。

Method: 构建包含6,875对图像-文本的数据集，评估模型在Bias、Answer Relevancy和Faithfulness三个指标上的表现。

Result: 闭源模型（如GPT-4o、Gemini2.5）整体表现最佳，开源模型（如Gemma3、Qwen2.5）在跨语言和社会属性上表现优异。

Conclusion: LinguaMark基准和评估代码的发布旨在促进可复现性和进一步研究。
...
TL;DR: MagiC是一个评估多模态认知的基准，涵盖答案准确性、逐步推理质量及其与视觉证据的对齐。
...
TL;DR: 论文提出ADIEE方法，用于自动生成数据集并训练评分模型，以改进指令引导图像编辑的评估。


<details>
  <summary>Details</summary>
Motivation: 现有开源视觉语言模型（VLMs）在评估指令引导图像编辑时存在对齐问题，而专有模型缺乏透明度和成本效益。缺乏公开训练数据集进一步限制了开源模型的性能。

Method: 通过ADIEE方法生成大规模数据集（超10万样本），并微调LLaVA-NeXT-8B模型，使其能解码自定义标记的数值分数。

Result: 训练出的评分模型在所有基准测试中优于开源VLMs和Gemini-Pro 1.5，显著提升与人类评分的相关性及对比准确性。

Conclusion: 该评分模型可作为奖励模型，提升编辑选择和模型微调效果，显著改进MagicBrush模型在ImagenHub上的评分。
...
TL;DR: 提出了一种基于Kubelka-Munk理论的快速图像合成方法，用于虚拟试妆应用，提升肤色与粉底融合的真实性，并构建了一个可扩展的端到端框架。
...
Method: 近似Kubelka-Munk理论以加速图像合成，同时保持融合的真实性；构建依赖电商产品信息的端到端框架。

Result: 在真实化妆图像上验证，框架性能优于其他技术。

Conclusion: 提出的方法在真实性和可扩展性上均表现优异，适用于虚拟试妆应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉叙事系统在跨帧识别角色和对象时表现不佳，导致引用不一致和幻觉问题，原因是缺乏明确的跨帧实体连接训练。

Method: 采用对比强化学习框架，扩展Story Reasoning数据集，使用双组件奖励函数优化模型，微调Qwen Storyteller。
...
<details>
  <summary>Details</summary>
Motivation: 训练泛化性深度补全模型通常需要大量带标签的深度数据，标注成本高。PacGDC旨在通过伪几何合成减少标注需求。

Method: 利用2D到3D投影中的形状和位置一致性，合成多样伪几何数据；结合深度基础模型和插值/重定位策略，扩展数据覆盖范围。

Result: PacGDC在多个基准测试中表现出卓越的泛化能力，适应不同场景语义/尺度和深度稀疏性/模式。

Conclusion: PacGDC通过伪几何合成显著提升深度补全的泛化性，为标签高效训练提供新思路。
...
Method: MSAGSM结合多尺度时间膨胀和多头空间注意力，增强时空建模能力，并可轻松集成到2D主干网络中。

Result: 在五个PES基准测试中，MSAGSM显著提升性能，且计算开销低，达到新SOTA。

Conclusion: MSAGSM是一种高效且通用的模块，为PES任务提供了新的解决方案。
...
TL;DR: KeyRe-ID是一种基于关键点的视频行人重识别框架，结合全局和局部分支，利用人体关键点增强时空表征学习。
...
TL;DR: 提出了一种保留动物习惯行为的跨类别运动迁移框架，结合生成模型和语言模型，用于动画和虚拟现实应用。


<details>
  <summary>Details</summary>
Motivation: 现有运动迁移方法主要关注人类运动，忽视了动物特有的习惯行为，需要一种能保留这些行为的方法。

Method: 基于生成框架，引入习惯保留模块和类别特定习惯编码器，结合大语言模型处理未观察到的物种。
...
<details>
  <summary>Details</summary>
Motivation: 无约束照片集合更易获取但存在光照不一致和瞬态遮挡问题，现有方法无法有效解决。
...
TL;DR: Corvid是一种增强链式思维（CoT）推理能力的多模态大语言模型（MLLM），通过混合视觉编码器和精心设计的跨模态对齐连接器（GateMixer）提升性能，并利用高质量数据集MCoT-Instruct-287K进行两阶段训练，显著优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 当前开源MLLM在复杂结构化推理任务中存在显著局限性，尤其在需要深度推理的决策和问题解决任务中表现不足。

Method: Corvid采用混合视觉编码器和GateMixer连接器，结合两阶段CoT格式训练和推理时自验证策略，提升推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 工业点云分析中，现有基准数据集多关注低分辨率数据，难以检测细微异常，因此需要高分辨率数据和高效方法。

Method: 提出可扩展的3D异常生成流程，创建MiniShift数据集；开发Simple3D框架，结合MSND和LFSA技术，高效捕捉几何细节。

Result: Simple3D在MiniShift和其他基准测试中表现优异，精度和速度均超越现有方法，实时推理达20 fps。
...
<details>
  <summary>Details</summary>
Motivation: 超声成像存在大量噪声，影响自动分割的准确性，需要一种更鲁棒的方法。

Method: 设计了AFSA模块和LGSA模块，分别通过相邻帧语义感知和局部-全局特征融合来减少噪声干扰。

Result: 在四个基准数据集上表现优于现有方法，同时推理速度更快。

Conclusion: DSANet在噪声鲁棒性和效率上均有显著提升，适用于超声视频分割。
...
TL;DR: 本研究提出了一种基于颜色阈值技术和深度卷积神经网络（DCNN）的方法，用于检测皮肤病变中的蓝白色面纱（BWV），显著提高了BWV检测的准确性，并利用可解释人工智能（XAI）解析模型决策过程。


<details>
  <summary>Details</summary>
Motivation: 蓝白色面纱（BWV）是黑色素瘤诊断的关键特征，但现有研究对其检测有限。本研究旨在填补这一空白，提供一种高效且可解释的BWV检测方法。

Method: 使用颜色阈值技术将非注释皮肤病变数据集转换为注释数据集，设计并训练了一个自定义层的DCNN模型，用于分类BWV的存在。

Result: 模型在不同数据集上表现出色，测试准确率最高达95.05%，显著优于现有方法。

Conclusion: 结合XAI的DCNN模型为早期黑色素瘤诊断提供了可靠工具，显著提升了BWV检测性能。
...
TL;DR: Objectomaly是一个基于对象感知的OoD分割框架，通过三阶段方法优化边界精度和异常评分，显著提升性能。
...
Method: 三阶段框架：1) 粗粒度异常评分（CAS）；2) 对象感知评分校准（OASC）；3) 边界精确化（MBP）。

Result: 在多个基准测试中达到SOTA性能（AuPRC达96.99，FPR95降至0.07）。
...
TL;DR: 提出了一种基于GAN的盲人脸图像恢复框架，通过局部统计特征变换和退化无关特征嵌入，显著提升了恶劣天气下的人脸识别精度。


<details>
  <summary>Details</summary>
Motivation: 恶劣天气条件下的人脸图像质量下降，导致识别精度降低，现有方法缺乏针对性模块，导致纹理和结构失真。

Method: 结合局部统计特征变换（SFFT）和退化无关特征嵌入（DAFE），SFFT增强局部统计分布对齐，DAFE实现鲁棒特征提取。
...
TL;DR: 本文提出了一种生成时态不可学习样本（TUEs）的框架，用于防止个人视频数据被未经授权用于训练深度跟踪器，解决了现有方法在视频数据隐私保护中的局限性。
...
Method: 提出了一种生成时态不可学习样本（TUEs）的新框架，并引入时态对比损失，以破坏现有跟踪器的学习能力。

Result: 实验表明，该方法在视频数据隐私保护方面达到最先进性能，且具有强泛化能力。

Conclusion: TUEs框架有效保护视频数据隐私，适用于大规模视频数据集，并具有跨模型、跨任务的可迁移性。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究忽视了全局SD地图与在线HD地图的关联，导致自动驾驶车辆在现实世界中难以有效利用在线HD地图进行混合导航。
...
<details>
  <summary>Details</summary>
Motivation: 现有偏好优化方法存在次优均值寻求问题，需改进对齐方式。
...
Method: 采用分组图网络，结合径向场和时空边聚合关节特征，通过组内和组间交互模块捕获多尺度依赖，并利用等变MLP更新关节位置特征。
...
<details>
  <summary>Details</summary>
Motivation: 动态场景的4D对象分割因缺乏标注数据集而未被充分研究，MUVOD旨在解决这一问题。

Method: 收集17个多视角视频场景，提供7830张RGB图像及分割掩码，并设计评估指标和基线方法。

Result: MUVOD包含459个实例和73类对象，支持跨视角和跨帧跟踪，为4D分割提供基准。
...
<details>
  <summary>Details</summary>
Motivation: 当前隐式变形场方法在空间一致性上存在不足，且稀疏时间信号的插值潜力未被充分探索。

Method: 采用样条轨迹表示，显式控制节点自由度，并引入低秩时变空间编码。

Result: 在稀疏输入的时间插值和动态场景重建中表现优异，同时提升了运动一致性。

Conclusion: 该方法在保持空间一致性和运动连贯性方面优于现有技术，无需依赖启发式初始化或刚性约束。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感基础模型通常针对特定模态（如光学RGB或多光谱数据），导致与任务模态不匹配，且模型庞大难以微调。

Method: 提出MAPEX模型，基于多模态数据预训练，采用模态条件令牌路由机制和模态感知剪枝技术，生成高效的任务专用模型。

Result: 在多个遥感数据集上验证，MAPEX性能优于全监督训练和现有遥感基础模型。

Conclusion: MAPEX通过多模态专家混合和剪枝技术，有效解决了模态不匹配和微调难题，为遥感任务提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 探讨VLMs在抽象推理任务中的线性推理瓶颈及其成因，揭示其潜在的可解决性。

Method: 引入线性可分性上限（LSC）评估VLMs视觉嵌入的线性分类性能，并通过后调优（postfix tuning）验证休眠推理路径的存在。

Result: 发现线性瓶颈普遍存在且源于语言模型的推理路径问题，任务依赖性干预可解决，但复杂关系推理需更深层模型权重调整。

Conclusion: 研究表明稳健推理需针对性对齐而非单纯改进表示学习，为VLM分析提供了新视角。
...
TL;DR: 提出了一种结合扩散引导知识蒸馏和深度引导特征融合的新方法DGKD-WLSS，用于弱监督低光语义分割任务，显著提升了性能。
...
Method: 通过扩散引导知识蒸馏（DGKD）对齐正常光和低光特征，并结合深度引导特征融合（DGF2）利用深度图作为光照不变的几何先验，增强结构特征学习。

Result: 实验证明DGKD-WLSS在低光条件下的弱监督语义分割任务中达到了最先进的性能。

Conclusion: DGKD-WLSS通过结合DGKD和DGF2，有效解决了低光环境下的语义分割问题，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 解决工业异常检测中因领域偏移导致的性能下降问题。
...
<details>
  <summary>Details</summary>
Motivation: 洪水灾害频发且破坏性强，现有方法在覆盖范围和细节上存在不足，需要更高效的洪水测绘技术。
...
Method: 提出多视角训练数据生成管道，结合Bald Converter、数据增强修复模型和面部微调扩散模型；引入极坐标嵌入和时间注意力层确保视角一致性。
...
TL;DR: 本文改进了SAM2框架的视频目标跟踪能力，通过分层运动估计和优化内存银行，显著提升了跟踪精度，尤其在长期遮挡和外观变化场景下表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 解决激光雷达扫描中图像语义标签稀疏和噪声问题，提升开放词汇分割的准确性和鲁棒性。

Method: 整合图像语义标签，确保时空一致性，并对抗图像级增强，随后训练3D网络。
...
TL;DR: 论文提出了SpatialViz-Bench，一个多模态空间可视化基准测试，用于评估大语言模型在空间可视化任务中的表现，发现现有模型存在明显缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法（如IQ测试或数学竞赛）可能因与训练数据重叠而不可靠，且空间可视化能力未被充分独立评估。
...
Method: ViLU训练一个二元分类器作为不确定性预测器，使用加权二元交叉熵损失区分正确与错误预测，适用于后处理设置。

Result: 在多样化数据集上的实验显示，ViLU相比现有方法有显著提升，适用于分类和大规模图像-文本数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法受限于领域特异性或对高层文本指导的依赖，导致运动细节丢失和不真实重建。

Method: 提出T-GVC框架，结合语义感知稀疏运动采样和轨迹对齐损失约束，减少码率并保留关键语义信息。

Result: 实验表明，T-GVC在超低码率下优于传统编解码器和现有端到端视频压缩方法。
...
TL;DR: 本文研究了深度学习面部表情识别（FER）模型中的年龄相关偏见，特别是对老年人的影响，并提出三种缓解策略，显著提高了老年人群体的识别准确性。


<details>
  <summary>Details</summary>
Motivation: 现有FER模型存在年龄相关的偏见，尤其是对老年人的识别性能较差，影响了公平性和可靠性。

Method: 研究分析了年龄对FER性能的影响，使用XAI技术识别偏见，并提出三种策略：多任务学习、多模态输入和年龄加权损失。

Result: 实验表明，提出的策略显著提高了老年人表情识别的准确性，尤其是易错表情（如中性、悲伤和愤怒）。

Conclusion: 通过简单的训练调整可以有效缓解FER中的年龄偏见，近似人口统计标签对促进大规模情感计算系统的公平性有价值。
...
TL;DR: 提出了一个全栈框架，通过强化学习扩展视觉语言模型（VLMs）对长视频的推理能力，包含数据集、训练管道和基础设施。
...
Method: 整合大规模数据集LongVideo-Reason、两阶段训练管道（CoT-SFT和强化学习）以及MR-SP基础设施。
...
<details>
  <summary>Details</summary>
Motivation: 儿科正畸中准确评估生长潜力对治疗策略至关重要，但传统关键点标注方法耗时且劳动密集。
...
TL;DR: TreeBench是一个用于评估视觉基础推理能力的诊断性基准，包含405个挑战性问题对，现有模型表现不佳。TreeVGR是一种联合监督定位和推理的训练范式，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型如OpenAI-o3虽能动态引用视觉区域，但缺乏全面评估其能力的基准。

Method: 提出TreeBench基准，基于三项原则构建，并通过TreeVGR训练范式联合监督定位和推理。

Result: TreeBench中现有模型准确率不足60%，TreeVGR显著提升了多个基准的表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有动态面部表情识别研究主要从深度学习角度进行特征学习，缺乏对AU-expression知识的利用。

Method: 设计AU损失函数，将AU-expression知识与深度学习网络结合，并在主流数据集上验证。

Result: AU-DFER在无需额外计算的情况下优于现有最优方法，并探索了解决数据标签不平衡的潜力。

Conclusion: AU-DFER首次将量化AU知识整合到DFER模型中，并强调了解决数据不平衡问题的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大视觉语言模型（LVLMs）在链式思维（CoT）推理中常忽略生成的中间推理内容，影响了其准确性和可靠性。
...
Method: 提出树感知扫描策略，基于特征相似性构建最小生成树，并通过上下遍历聚合空间拓扑特征。同时构建了可靠的数据集BlueDepth。

Result: Tree-Mamba在定性和定量评估中均优于现有方法，且计算效率高。

Conclusion: Tree-Mamba方法有效解决了水下深度估计问题，BlueDepth数据集为未来研究提供了可靠基础。
...
TL;DR: 论文提出了CrossVLAD数据集和CRAFT方法，用于评估和攻击统一视觉语言模型在多任务指令下的安全性。
...
Method: 引入CrossVLAD数据集和CRAFT攻击方法，通过区域中心和令牌对齐策略实现跨任务攻击。

Result: 实验表明CRAFT在跨任务攻击和目标对象改变成功率上优于现有方法。

Conclusion: CrossVLAD和CRAFT为统一视觉语言模型的安全性提供了新的评估工具和攻击方法。
...
TL;DR: 研究探讨了开源胸部X光数据集中是否存在数据集偏见，通过简单变换增加任务难度，并测试了多种网络架构。


<details>
  <summary>Details</summary>
Motivation: 医学图像因其敏感性难以开源，导致某些数据集被广泛使用。研究旨在验证这些数据集中是否存在偏见，以确保AI方法关注病理而非捷径。

Method: 对NIH、CheXpert、MIMIC-CXR和PadChest数据集进行简单变换，测试多种网络架构。

Result: 未明确提及具体结果，但希望通过实验验证偏见存在与否。
...
TL;DR: 提出一种简化的3D可提示分割方法，减少推理时间并消除滑动窗口的复杂性，同时实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: SAM的2D架构无法直接应用于3D医学影像（如CT或MRI），现有3D方法通常复杂且计算资源消耗大。

Method: 受SegVol启发，设计了一种简化的3D可提示分割方法，避免滑动窗口和复杂的提示管理。

Result: 实现了最先进的性能，同时显著降低推理时间和复杂性。
...
TL;DR: 本文提出了一种基于能量的解码方法，用于减少大型视觉语言模型中的物体幻觉问题，显著提高了三个VQA数据集的性能。


<details>
  <summary>Details</summary>
Motivation: 解决大型视觉语言模型中物体幻觉问题，现有方法存在限制或复杂性。

Method: 提出基于能量的解码方法，动态选择能量分数最低的隐藏状态层。

Result: 在三个基准测试（POPE、MME、MMVP）中显著提升准确率和F1分数，平均准确率提高4.82%，yes-ratio差距减少8.81%。

Conclusion: 该方法简单有效，减少了模型对“Yes”回答的偏见，并提升了性能。
...
TL;DR: 提出一种高频率双流脉冲神经网络（SNN），用于早期人类活动识别，在THU EACT-50数据集上准确率提升2%，并展示了其在体育动作捕捉中的应用。


<details>
  <summary>Details</summary>
Motivation: 早期识别人类活动对安全性和响应速度至关重要，事件视觉传感器的高时间分辨率和低延迟适合这一需求，但现有方法在处理速度和准确率上存在不足。

Method: 采用高频率双流SNN处理事件数据，实现早期预测，并在THU EACT-50数据集上进行测试。

Result: 新方法在最终准确率上超越先前工作2%，并在早期识别框架中展示了Top-1和Top-5识别分数。

Conclusion: 该方法在早期活动识别中表现出色，并在实际应用中（如体育动作捕捉）验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预训练模型的最终层特征，适应性有限，且参数效率低。稀疏性在MR任务中被忽视。

Method: 提出SDST架构，引入参考可变形自注意力机制，并首次将InternVideo2主干集成到ST框架中。

Result: 在QVHighlights、TACoS和Charades-STA上取得SOTA结果，参数数量减少73%。

Conclusion: SDST显著提升了VTG任务的性能，同时大幅降低了参数需求。
...
<details>
  <summary>Details</summary>
Motivation: 通过整合高光谱成像技术，实时提供荧光定量数据，以改善手术决策。关键挑战在于动态手术环境下蓝光和白光图像的跨模态配准。

Method: 提出X-RAFT模型，基于RAFT光流模型改进，采用独立编码器处理不同模态，并通过自监督流循环一致性进行微调。

Result: 相比基线方法，X-RAFT在评估指标上误差减少36.6%，优于现有跨模态光流方法（误差减少27.83%）。

Conclusion: X-RAFT在荧光定量测量中表现出色，为手术决策提供了更准确的实时数据支持。
...
TL;DR: 本文提出了一种基于深度学习的体素级变形估计方法，用于解决增材制造中CAD与XCT体积数据的配准问题，显著提高了配准精度和效率。


<details>
  <summary>Details</summary>
Motivation: 增材制造中的几何不准确性（如收缩和变形）会影响零件性能，传统DVC方法因缺乏真实变形场和大数据量而难以精确配准。

Method: 采用动态基于补丁的处理策略和深度学习，引入二值差异图（BDM）评估配准精度。

Result: 相比传统DVC方法，Dice分数提高了9.2%，体素匹配率提高了9.9%，交互时间从几天缩短至几分钟。

Conclusion: 该方法为基于深度学习的DVC技术奠定了基础，可提升增材制造的可靠性和效率，具有工业应用潜力。
...
TL;DR: SCOOTER是一个开源框架，用于评估无限制对抗攻击的不可感知性，提供众包研究指南、大规模人类与模型对比、开源工具和基准数据集。
...
Result: 发现现有攻击方法无法生成不可感知图像，GPT-4o仅部分有效，自动化视觉系统与人类感知不一致。
...
Result: 发现熵正则化和标签平滑结合效果最佳，后处理校准方法在少量OOD数据下更稳健，新校准方法改进有限，且校准改进常以牺牲分布内校准为代价。
...
TL;DR: 论文提出了S\textsc{urprise}3D数据集，用于评估复杂3D场景中的语言引导空间推理分割，解决了现有数据集中语义与空间关系混淆的问题。


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉语言研究中，空间推理能力未充分探索，且现有数据集存在依赖语义捷径而非真正理解空间关系的问题。

Method: 通过构建包含200k+视觉语言对和89k+无对象名称标注的空间查询的S\textsc{urprise}3D数据集，覆盖多种空间推理技能。

Result: 初步基准测试显示，当前最先进的3D视觉定位方法和3D-LLM面临显著挑战，验证了数据集的必要性。
...
TL;DR: 本文提出了一种基于3D卷积神经网络和差分帧融合模块的非接触式远程光电容积描记术（rPPG）网络，结合自注意力机制和动态混合损失函数，显著提升了心率测量的准确性、鲁棒性和泛化能力。
...
Method: 使用3D卷积神经网络重建rPPG信号，引入差分帧融合模块和自注意力机制，提出动态混合损失函数。

Result: 在PURE、UBFC-rPPG和MMPD数据集上表现优异，训练后模型在MMPD测试集上的MAE为7.58，优于现有方法。

Conclusion: 所提方法在复杂场景下具有更高的鲁棒性和泛化能力，为远程心率测量提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实应用中多模态数据常缺失或不完整，导致性能下降，现有静态提示方法缺乏灵活性且无法应对关键模态缺失。

Method: 引入动态适配器生成自适应提示，结合静态与动态提示的协同策略，平衡模态信息。

Result: 在三个常用视觉识别数据集上显著优于现有方法，适应性强且可靠。
...
TL;DR: 本文提出了一种基于Vision Transformers（ViT）的无标记肺肿瘤运动预测方法，比较了患者特定（PS）和多患者（MP）训练策略，发现PS模型在精度上更优，而MP模型在临床时间受限的情况下更具鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 精确预测肺肿瘤运动对质子治疗中的剂量投递至关重要。尽管深度学习在无标记方法中占主导，但基于Transformer的架构在此领域尚未被探索。

Method: 使用31名患者的4DCT扫描生成的数字重建放射影像（DRRs）训练MP模型，PS模型仅使用目标患者的规划数据。两种模型均预测1秒内的肿瘤运动，并通过ADE和FDE评估性能。

Result: PS模型在规划数据（T1）上表现更优，尤其在数据量较大时；而MP模型在治疗数据（T2）上无需重新训练即表现稳健。

Conclusion: ViT首次应用于无标记肿瘤运动预测，PS模型精度更高，MP模型更适合临床时间受限的场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有拼图求解器缺乏对现实挑战（如文物或文件碎片重组）的评估，需研究其鲁棒性。

Method: 引入三种拼图干扰（缺失、边缘腐蚀、内容腐蚀），评估启发式和深度学习求解器的表现。

Result: 标准求解器性能随干扰增加迅速下降，但深度学习模型通过微调和数据增强显著提升鲁棒性，Positional Diffusion模型表现最佳。

Conclusion: 研究为提升现实文物自动重建提供了方向，深度学习模型在干扰下更具潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决CIS中解耦框架导致的可塑性丧失和对输入数据顺序的依赖问题。

Method: 直接选择图像特征进行查询分配，确保完美对齐以保留对象性，并引入跨阶段一致性和视觉查询重放机制。

Result: SimCIS在多种分割任务、设置和输入顺序中优于现有方法。

Conclusion: SimCIS通过简单而有效的方法解决了CIS中的关键问题，具有广泛的应用潜力。
...
TL;DR: 论文介绍了3D-ADAM，首个大规模工业相关的高精度3D异常检测数据集，旨在解决现有数据集不足的问题。
...
Method: 构建3D-ADAM数据集，包含14,120个高分辨率扫描和27,346个标注缺陷实例，覆盖12类工业表面缺陷。

Result: 评估显示现有SOTA模型在该数据集上表现不佳，验证了其挑战性和工业相关性。

Conclusion: 3D-ADAM为开发鲁棒的3D异常检测模型提供了重要基准，推动现代制造环境的需求满足。
...
<details>
  <summary>Details</summary>
Motivation: 数字病理学领域近期涌现大量基础模型，但缺乏统一的评估标准，尤其在医疗领域需关注模型的不确定性和鲁棒性。

Method: 开发THUNDER基准测试工具，支持多模型、多数据集和多任务比较，分析特征空间及预测的鲁棒性和不确定性。

Result: 对23种基础模型在16个数据集上进行了全面比较，涵盖多样化任务、特征分析和鲁棒性评估。

Conclusion: THUNDER为数字病理学领域提供了高效的模型评估工具，支持未来研究的发展。
...
TL;DR: 本文提出了一种名为SLURPP的新型网络架构，结合了预训练的潜在扩散模型和显式场景分解，用于水下图像恢复，显著提升了计算效率和恢复质量。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的水下图像恢复方法在复杂场景中计算量大且易产生不真实伪影，需要更高效且准确的解决方案。

Method: 结合预训练的潜在扩散模型和显式场景分解，设计了一种物理基础的水下图像合成管道生成训练数据。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在医疗诊断中常生成与医学知识不符的响应，检索增强生成虽能提升准确性，但存在检索不足或过度依赖的问题。

Method: 提出MIRA框架，包含动态调整检索内容的模块和结合图像嵌入与医学知识库的多模态推理模块。

Result: 在公开医疗VQA和报告生成基准测试中，MIRA显著提升事实准确性，达到新SOTA。

Conclusion: MIRA通过优化检索和推理机制，有效解决了多模态医疗诊断中的事实一致性问题。
...
<details>
  <summary>Details</summary>
Motivation: 现代导航系统需要精确位置估计，而VSLAM依赖可靠的特征点提取，但传统方法计算量大，难以部署在资源受限平台上。

Method: 采用量化SuperPoint CNN，结合Brevitas库和FINN框架进行模型量化与硬件优化，并在FPGA SoC平台上实现。

Result: 在AMD/Xilinx Zynq UltraScale+平台上实现54 fps处理640x480像素图像，性能优于现有方案。

Conclusion: 量化技术显著提升了模型在视觉里程计任务中的效率与性能，适用于嵌入式系统。
...
TL;DR: 提出了一种新型的完全测试时间适应（TTA）策略CiCi框架，用于远程光电容积描记（rPPG）任务，结合生理学先验知识，通过时空一致性和不一致性提升模型适应性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在隐私和实时适应方面受限，无法满足实际部署需求，因此提出无需源数据的自监督适应框架。

Method: 基于生理学先验，利用时空一致性和不一致性，设计CiCi框架，并引入梯度动态控制机制以稳定适应。

Result: 在五个数据集上的实验表明，该方法在实时自监督适应中优于现有技术，达到最先进性能。

Conclusion: CiCi框架为rPPG任务提供了一种高效、稳定的测试时间适应方案，无需源数据即可实现实时性能提升。
...
TL;DR: 开发了一种实时识别算法，通过耳标和摄像头监控实验室小鼠，提高追踪效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 实验室小鼠的高密度饲养、相似外观和高活动性使得个体数据收集困难，需要更准确的实时监测方法。

Method: 结合多目标追踪器（MouseTracks）、基于Transformer的ID分类器（Mouseformer）和轨迹关联线性规划（MouseMap）的算法。

Result: 算法能以30帧/秒的速度准确识别小鼠耳标，减少ID切换，优于现有方法。

Conclusion: 该算法显著提升了小鼠追踪的效率和准确性，适用于不同品系和环境。
...
Conclusion: TinierHAR为高效HAR系统设计提供了实用指南，并开源所有材料以推动边缘HAR研究。
...
Method: 1) M3arsSynth数据重建管道，从NASA立体图像重建3D环境；2) MarsGen视频生成器，基于3D结构生成视频。
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散模型在原始视频数据训练下难以捕捉几何感知结构，需弥补其与物理世界3D本质的差距。

Method: 提出Angular Alignment和Scale Alignment两种对齐目标，利用预训练几何基础模型引导模型学习3D表示。

Result: 实验显示，该方法在视觉质量和3D一致性上显著优于基线方法。
...
TL;DR: OST-Bench是一个评估多模态大语言模型在线时空理解能力的基准，强调动态感知和记忆整合，发现现有模型在复杂时空推理任务上表现不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准多为离线评估，无法反映真实世界动态感知的挑战，因此设计OST-Bench以填补这一空白。

Method: 基于高效数据收集流程，构建包含1.4k场景和10k问答对的基准，评估模型在动态探索中的表现。

Result: 现有模型在复杂时空推理任务上表现不佳，准确率随探索时间和记忆增长而下降。

Conclusion: OST-Bench揭示了模型在动态感知中的核心挑战，为未来研究提供了方向和资源。
...
TL;DR: 研究发现CLIP模型在数据属性（如低属性密度、不完整描述和显著性偏差）影响下难以学习绑定问题，仅当数据满足特定属性时才能解决。


<details>
  <summary>Details</summary>
Motivation: CLIP模型在绑定问题上表现不佳，现有方法未能完全解决，怀疑问题根源在于数据属性。

Method: 通过合成数据集系统分析数据属性对CLIP绑定能力的影响。

Result: 发现自然数据的常见属性（如低属性密度、不完整描述和显著性偏差）对绑定性能有害，仅特定数据属性下CLIP能完美绑定。

Conclusion: 数据属性是解决CLIP绑定问题的关键，现有方法（如增加批量或硬负样本）无效。
...
<details>
  <summary>Details</summary>
Motivation: 视频大语言模型（LLMs）在处理大量时空令牌时面临计算复杂度二次增长的问题，现有方法未充分利用局部冗余性。

Method: STTM采用多粒度空间令牌转换和定向时间维度合并，通过四叉树结构实现粗到细的搜索。

Result: 在六个视频QA基准测试中表现优异，50%令牌预算下速度提升2倍，精度仅降0.5%；30%预算下速度提升3倍，精度降2%。

Conclusion: STTM是一种高效且通用的令牌合并方法，支持KV缓存复用，适用于不同问题对同一视频的处理。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估协议依赖粗粒度指标，掩盖了模型间的差异，缺乏神经科学基础，无法捕捉细粒度视觉区分。
...
Conclusion: 提出的框架有效解决了草图驱动的关键点检测问题，具有广泛适用性。
...
Result: KARL性能与现有自适应标记器相当，并分析了编码器/解码器大小、连续与离散标记化等因素的扩展规律。
...
TL;DR: 本文提出了一种名为\NickName的新方法，通过增强离散码本的表示能力，优化码本并减少信息损失，从而提升VQ-VAEs的重建质量。实验表明，该方法在多个基准测试中达到最优性能。
...
Method: 使用点互信息（PMI）量化词共现频率，通过合成和编辑自然图像验证PMI与模型准确性的相关性。
...
TL;DR: 提出一种半监督深度学习方法，用于多序列MRI数据的颈动脉血管壁和斑块分割，解决数据稀缺和形态复杂性问题。
...
Method: 采用两阶段网络：粗定位模型确定感兴趣区域，精细分割模型完成精确分割；提出多级多序列U-Net架构和半监督一致性学习。

Result: 在52名患者的五序列MRI数据上验证，实验表明方法有效，融合策略选择对U-Net架构性能至关重要。

Conclusion: 融合策略和半监督学习在数据有限的多序列MRI应用中具有提升颈动脉分割效果的潜力。
...
TL;DR: 研究探讨了使用深度学习自动编码器压缩工业X射线CT数据的方法，并比较了不同压缩率和架构对数据恢复质量的影响。


<details>
  <summary>Details</summary>
Motivation: 随着成像技术的发展，大量数据需要高效可靠的存储解决方案，尤其是工业X射线CT数据。

Method: 采用两种网络架构（D-CNN和VQ-VAE）对砂岩样本的XCT数据进行压缩，并引入边缘保护敏感指标评估解码质量。

Result: 不同架构和压缩率适用于不同分析需求，研究结果可帮助科学家优化数据存储和分析策略。

Conclusion: 深度学习压缩方法可根据需求灵活调整，为工业X射线CT数据存储和分析提供有效解决方案。
...
TL;DR: GridTD是一种基于多分辨率网格编码的无监督连续表示框架，用于压缩成像重建，结合了张量分解和多分辨率哈希网格编码的优势，实现了高效的高维图像重建。
...
Method: 提出GridTD框架，结合轻量级神经网络、输入张量分解模型和多分辨率哈希网格编码，实现高效重建。

Result: 理论分析和实验表明，GridTD在多个压缩成像任务中优于现有方法，成为最先进的重建方法。

Conclusion: GridTD通过结合多分辨率网格编码和张量分解，提供了一种高效且通用的压缩成像重建解决方案。
...
TL;DR: 提出一种多模态深度学习框架，整合CT、MRI、组织病理学、临床数据和基因组学，提高ccRCC复发预测准确性。
...
Method: 使用CLAM处理组织病理学图像，MeD-3D处理CT和MRI，MLP处理临床和基因组数据，通过早期和晚期融合策略整合多模态特征。

Result: 框架能够处理不完整数据，并整合多源信息，提升预测性能。
...
TL;DR: ArteryX是一种基于MATLAB的半自动化工具，用于从3D TOF MRA中高效、准确地量化脑血管特征，解决了现有方法的局限性。
...
Method: ArteryX采用基于血管融合网络的标记方法，自动追踪和管理血管，结合模拟框架验证特征。
