...
TL;DR: 提出统一的协同训练框架，通过单一SFT阶段集成多种安全行为（积极、消极、拒绝），使用系统指令或magic token实现推理时的动态行为切换，在保证安全性能的同时显著降低训练复杂度和部署成本。
...
Method: 统一的协同训练框架，在单一SFT阶段同时训练三种安全行为模式，通过系统级指令或magic token实现推理时的动态行为切换。
...
Conclusion: 这是初步结果而非最终发现，官方WMT25排名将基于更可靠的人工评估，届时将取代这个自动排名
...
TL;DR: 本文提出了一个LLM驱动的文化本地化框架，用于自动生成包含本土名称、组织和货币的数学应用题数据集，以解决低资源语言中多语言数学推理的不足。


<details>
  <summary>Details</summary>
Motivation: 现有多语言数学推理基准主要通过翻译生成，保留了英语中心实体，缺乏真实的文化背景和本土实体，导致低资源语言的数学推理能力评估不准确。

Method: 开发了一个LLM驱动的文化本地化框架，能够从现有数据源自动构建包含本土名称、组织和货币的数学应用题数据集。

Result: 实验表明，翻译基准在适当的社会文化背景下可能掩盖真实的多语言数学能力，而该框架能够减轻英语中心实体偏见，提高多语言环境下的鲁棒性。

Conclusion: 该框架有效解决了多语言数学推理中文化本地化数据稀缺的问题，为低资源语言提供了更准确的能力评估和更鲁棒的模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何利用相对较少但易于获取的数据资源，改进通用指令调优大语言模型的机器翻译能力，特别是针对斯洛文尼亚语这样的低资源语言

Method: 使用直接偏好优化(DPO)方法，基于程序化筛选的公共数据集子集进行训练。通过两个LLM模型翻译英文维基百科文章生成训练数据对，并基于启发式规则和COMET等自动评估指标进行质量排序

Result: 微调后的模型在翻译维基百科文章时，COMET评分分别比两个基线模型提高了约0.04和0.02，并且更一致地避免了语言和格式错误

Conclusion: DPO方法能够有效利用程序化筛选的数据资源显著提升大语言模型的机器翻译性能，特别是在低资源语言场景下表现出色
...
Method: 扩展CoXQL数据集为MultiCoXQL涵盖5种语言，提出新的解析方法，创建Compass数据集用于自定义输入提取，并使用不同规模的LLM和BERT模型进行评估。

Result: 在MultiCoXQL上评估了三种LLM的多种解析策略，在Compass上进行了单语、跨语言和多语言评估，涵盖了11种意图和5种语言。

Conclusion: 提出的多语言数据集和解析方法有助于解决ConvXAI系统的多语言泛化问题和自定义输入支持限制。
...
TL;DR: 提出奖励偏移推测采样算法(SSS)，通过对齐草稿模型来降低测试时对齐的推理成本，在保持目标模型不变的情况下实现高效的人类偏好对齐。
...
Method: 使用奖励偏移推测采样算法，让草稿模型与人类偏好对齐，而目标模型保持不变。通过修改接受准则和奖励令牌分布来利用对齐草稿模型与未对齐目标模型之间的分布偏移，恢复RLHF最优解。

Result: 在测试时弱到强对齐实验中，以显著降低的推理成本获得了优异的黄金奖励分数，验证了算法的有效性和效率。

Conclusion: SSS算法成功解决了测试时对齐的高成本问题，通过巧妙利用分布偏移实现了高效的人类偏好对齐，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在医学、法律等关键领域，机器生成文本的完整性至关重要，现有召回指标依赖词汇重叠容易出错，LLM评估方法存在幻觉和对齐问题。
...
TL;DR: 该论文提出将大语言模型与组合推理相结合的方法，通过符号推理增强LLMs的结构一致性和预测准确性，实验表明无论采用何种提示策略，符号推理都能带来更一致准确的结果


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在结构化预测任务中的幻觉和复杂推理问题，将LLMs的预测能力与推理方法提供的结构一致性相结合

Method: 结合LLMs与组合推理，探索有效的提示策略来估计LLM置信度值用于符号推理，并进行校准和使用结构化预测目标的微调

Result: 实验显示符号推理的加入显著提高了预测的一致性和准确性，结构化学习在LLM时代仍然具有重要价值

Conclusion: 通过将符号推理与LLMs结合，可以有效解决结构化预测任务中的问题，结构化学习对提升LLM性能仍然至关重要
...
TL;DR: 本文介绍Nemotron-CC-Math，一个从Common Crawl构建的大规模高质量数学语料库，通过新颖的领域无关管道实现鲁棒的科学文本提取，显著提升LLM的数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于Common Crawl的数学数据集由于脆弱的提取启发式方法、有损的HTML到文本转换以及无法可靠保留数学结构而导致质量下降，需要更高质量的数学预训练数据。

Method: 开发了一个新颖的领域无关管道，利用lynx进行布局感知渲染和基于LLM的针对性清理阶段，恢复各种格式的数学内容（MathJax、KaTeX、MathML），保持方程和代码块的结构完整性，同时去除样板文件并将符号标准化为LaTeX表示。

Result: 构建了Nemotron-CC-Math-3+（133B tokens）和Nemotron-CC-Math-4+（52B tokens）两个高质量数学语料库。Nemotron-CC-Math-4+超越了所有现有开放数学数据集，包含比之前最高质量的FineMath-4+多5.5倍的tokens。使用该语料库预训练的Nemotron-T 8B模型在MATH上获得+4.8到+12.6的提升，在MBPP+上获得+4.6到+14.3的提升，同时在MMLU和MMLU-Stem上的通用领域性能也有所改善。

Conclusion: 提出了第一个能够从嘈杂的网络规模数据中可靠提取科学内容（包括数学）的管道，在数学、代码和通用推理方面带来了可衡量的收益，并在开放数学预训练语料库中设立了新的最先进水平。代码和数据集已开源。
...
Result: 在五个LLM上的实验表明：(1)整合检索到的证据是有益的；(2)生成和验证原子假设能带来更多改进。

Conclusion: 通过外部证据和原子假设验证的方法，能够有效识别和回答包含错误假设的问题，减少LLM的幻觉并提供可解释的答案。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型和大视觉语言模型在处理需要深度推理、持续上下文理解、实体跟踪和多步骤指令跟随的复杂多轮视觉基础任务时仍面临重大挑战，现有基准无法充分捕捉真实世界多模态交互的动态性和复杂性。

Method: 构建了包含300个复杂多轮对话场景的MMDR-Bench数据集；提出了CoLVLM Agent框架，采用迭代的"记忆-感知-规划-执行"循环来增强现有LVLM模型，无需大规模重新训练基础模型。

Result: 在MMDR-Bench上的实验显示，CoLVLM Agent平均人类评估得分达到4.03，显著超越GPT-4o(3.92)和Gemini 1.5 Pro(3.85)，在推理深度、指令遵循和错误抑制方面表现出明显优势，并在长对话轮次中保持稳健性能。

Conclusion: CoLVLM Agent的模块化设计和迭代方法对于复杂多模态交互非常有效，为解决当前多模态模型在复杂推理任务中的局限性提供了有前景的解决方案。
...
TL;DR: SemToken是一种语义感知的分词框架，通过语义聚类减少token冗余，根据语义密度分配异构粒度，在长上下文建模中实现2.4倍token减少和1.9倍加速


<details>
  <summary>Details</summary>
Motivation: 现有分词方法如BPE和WordPiece仅基于频率统计，忽略文本的语义结构，导致语义冗余span的过度分词和上下文连贯性的利用不足，特别是在长上下文场景中
...
<details>
  <summary>Details</summary>
Motivation: 现有PRMs主要针对通用或STEM领域训练，在金融等专业领域中表现不足，因为金融推理需要更结构化、符号化且对事实和监管准确性敏感。

Method: 开发Fin-PRM，集成步骤级和轨迹级奖励监督，支持离线/在线奖励学习，应用于监督微调、强化学习和测试时推理三个关键场景。

Result: 在CFLUE和FinQA基准测试中，Fin-PRM在轨迹选择质量上持续领先，下游模型性能提升显著：监督学习12.9%、强化学习5.2%、测试时性能5.1%。

Conclusion: 领域专业化奖励建模对于使大语言模型与专家级金融推理对齐具有重要价值，Fin-PRM为金融领域推理提供了有效的监督框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有KV缓存压缩方法主要关注时间轴压缩，忽略了特征维度（通道轴）上的细粒度重要性变化，导致效率和准确性平衡受限

Method: 提出SPARK方法，在通道级别对KV缓存进行非结构化稀疏化剪枝，并在注意力分数计算时动态恢复被剪枝的条目

Result: SPARK在相同内存预算下可处理更长序列，相同序列长度下减少30%以上KV缓存存储，即使80%剪枝率性能下降也小于5%

Conclusion: SPARK方法有效解决了KV缓存瓶颈问题，与现有压缩和量化技术正交兼容，为实现更长序列处理提供了有效解决方案
...
<details>
  <summary>Details</summary>
Motivation: LLMs在通用QA表现良好但在领域特定场景中表现不佳，RAG存在幻觉和延迟问题，持续预训练成本高且缺乏跨域灵活性。领域知识的长尾分布导致部分有用内部知识未被充分利用。

Method: 提出S2K框架，采用内部-外部知识自选择策略和选择性监督微调，引入结构化推理数据生成流程并集成GRPO来增强推理能力，模拟人类渐进式学习过程。

Result: 在医学、法律和金融QA基准测试中，S2K持续优于现有方法，并以显著更低的成本达到领域预训练LLMs的性能水平。

Conclusion: S2K提供了一种成本效益高的解决方案，通过渐进式知识获取策略有效解决了领域特定QA中的挑战，证明了模拟人类学习过程的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手动设计任务特定演示或从精选库中检索，需要大量专家工作和复杂的提示工程，随着工具多样性和任务难度增加，效率低下

Method: 提出Stepwise Experience Recall (SEER)方法，从持续更新的经验池中进行细粒度逐步检索，增量式扩充经验池，包含过去成功的执行轨迹

Result: 在ToolQA基准上，简单问题平均提升6.1%，困难问题提升4.7%；在τ-bench真实场景中，使用Qwen2.5-7B和72B模型分别获得7.44%和23.38%的准确率提升

Conclusion: SEER通过自引导的经验检索机制有效解决了LLM在多步工具调用中的挑战，实现了持续性能改进，无需大量人工干预
...
TL;DR: 本文研究了自动检查表在生成任务评估中的有效性，发现选择性使用检查表在成对比较任务中能提升性能，但在直接评分中效果不一致。检查表项目即使与人工评分相关性低，也常反映人工评估标准，揭示了人工评估的不一致性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在生成任务自动评估中面临标准模糊的挑战，自动检查表生成作为一种有前景的方法，其实际有效性尚未充分探索。研究旨在确定检查表是否应该选择性使用，并分析其与人工评估的关系。
...
Conclusion: 研究结果强调了需要更明确地定义客观评估标准，以指导人工和自动评估，提高生成任务评估的可靠性和一致性。
...
<details>
  <summary>Details</summary>
Motivation: 小型语言模型在资源受限环境中具有计算优势，但词汇相关组件（嵌入和语言建模头）由于词汇量大而占用大量内存，成为边缘设备部署的关键瓶颈。现有的静态词汇剪枝方法存在信息丢失和缺乏灵活性的问题。

Method: 基于词汇局部性原理和词汇相关组件计算特性的不对称性，提出VocabTailor框架：1）通过卸载嵌入解决内存约束；2）为LM Head实现混合静态-动态词汇选择策略，按需加载词汇组件。

Result: 在多样化下游任务上的综合实验表明，VocabTailor能够将词汇相关组件的内存使用减少高达99%，同时任务性能仅有最小化或没有下降，显著优于现有的静态词汇剪枝方法。

Conclusion: VocabTailor通过动态词汇选择有效解决了SLM内存瓶颈问题，为边缘设备部署提供了高效的内存优化解决方案，在保持性能的同时大幅降低内存需求。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要依赖翻译数据，缺乏低资源语言（如泰语）的文化和领域特定细微差别，无法满足实际应用需求。

Method: 通过多阶段质量控制流程创建人工编写的泰语数据集，涵盖4个专业领域和7种任务类型，并进行零样本评估和指令微调研究。

Result: 使用WangchanThaiInstruct微调的模型在领域内和领域外基准测试中均优于使用翻译数据的模型。

Conclusion: 需要基于文化和专业背景的指令数据来改善低资源语言环境下LLM的对齐性能。
...
TL;DR: 提出UniCoM管道和SWORDS算法，通过同义词替换生成高质量代码切换语音样本，构建CS-FLEURS多语言语料库，在ASR和S2TT任务中表现优异


<details>
  <summary>Details</summary>
Motivation: 代码切换在真实对话中很常见，但缺乏合适的数据集限制了多语言语音技术的发展

Method: 使用SWORDS算法，根据词性选择性地用翻译替换单词来生成代码切换语音，保持句子语义不变

Result: CS-FLEURS语料库在可懂度和自然度方面表现优异，在客观和主观指标上都与现有数据集相当
...
TL;DR: 本文提出了EMNLP框架，用于评估教师角色大语言模型的人格特征、道德发展阶段和伦理风险，发现教师角色LLMs比人类教师更理想化和极化，在抽象道德推理方面表现优异但在情感复杂情境中表现不佳，推理能力强的模型更容易受到有害提示注入攻击。
...
Result: 教师角色LLMs表现出比人类教师更理想化和极化的人格特征，在抽象道德推理方面优秀但在情感复杂情境中表现不佳，推理能力强的模型更容易受到有害提示注入攻击，模型温度等超参数影响有限。
...
TL;DR: CARE方法通过上下文评估器和基础LLM来解决RAG中的上下文-记忆冲突问题，在QA和事实核查基准上平均提升5.0%性能


<details>
  <summary>Details</summary>
Motivation: 解决检索增强生成(RAG)中当检索到的外部上下文与LLM参数知识冲突时，模型无法正确处理这种上下文-记忆冲突的问题

Method: 提出Conflict-Aware REtrieval-Augmented Generation (CARE)，包含上下文评估器和基础LLM。上下文评估器通过压缩记忆token嵌入和基于grounded/adversarial软提示的训练来识别不可靠上下文并指导推理
...
<details>
  <summary>Details</summary>
Motivation: 理解事件需要掌握其时间上下文，但时间常识在自然语言中往往不明确提及，即使先进的大语言模型也难以生成需要时间常识推理的文本。自动挖掘时间常识有助于构建更鲁棒的语言模型。

Method: 提出时间常识提取管道，利用大语言模型从SAMSum和RealNews语料库中自动挖掘时间常识，构建TComQA数据集，并通过众包验证数据质量。

Result: TComQA数据集在提取时间常识方面达到超过80%的精确度，基于该数据集训练的模型在时间问答任务上优于使用现有数据集微调的大语言模型。

Conclusion: 该方法能够有效自动挖掘时间常识，构建高质量数据集，并提升模型在时间推理任务上的性能，为构建更智能的语言模型提供了重要基础。
...
TL;DR: CUPE是一个轻量级模型，能在120毫秒内捕捉音素关键特征，通过独立处理短固定窗口实现跨语言竞争性能，证明了通过建模音素长度窗口的基本声学模式可以实现有效的通用语音处理。


<details>
  <summary>Details</summary>
Motivation: 许多语音处理任务需要不受上下文影响的纯音素表示，但通用音素识别通常需要分析长语音段和语言特定模式，因此需要开发能够快速捕捉音素特征的轻量模型。

Method: CUPE处理短固定宽度窗口（120毫秒，约一个音素长度）并独立分析，通过监督和自监督训练在多种语言上进行评估，包括在UCLA语音语料库上的零样本测试。

Result: 尽管参数比现有方法少，CUPE实现了竞争性的跨语言性能，通过学习所有语言共有的基本声学模式，展示了强大的跨语言泛化能力。

Conclusion: 有效的通用语音处理可以通过在音素长度窗口内建模基本声学模式来实现，CUPE证明了这种方法的可行性。
...
TL;DR: 本文提出了EDAS元度量方法，用于统一评估知识图谱补全模型在多个数据集和不同指标下的综合性能，解决传统评估中指标冲突和数据集不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 知识图谱补全模型评估存在挑战，不同数据集和评估指标（如MRR、Hit@k）经常产生冲突的排名结果，难以进行整体性能比较和模型选择。

Method: 提出基于平均解距离的EDAS元度量方法，将模型在多个数据集和多样评估标准下的性能综合为单一标准化分数（0-1范围），提供全局性能视角。

Result: 在FB15k-237和WN18RR等基准数据集上的实验表明，EDAS能有效整合多指标、多数据集性能，形成统一排名，提供一致且鲁棒的评估框架。

Conclusion: EDAS为知识图谱补全模型评估提供了可靠、可解释的统一框架，支持更明智的模型选择并促进跨数据集评估的公平性。
...
Method: 系统回顾大语言模型评测基准的现状与发展，将283个代表性基准分为三类：通用能力（涵盖核心语言学、知识、推理等）、领域特定（聚焦自然科学、人文社科、工程技术等）、目标特定（关注风险、可靠性、智能体等）。

Result: 发现当前基准存在数据污染导致分数膨胀、文化语言偏见导致不公平评估、缺乏过程可信度和动态环境评估等问题。

Conclusion: 为未来基准创新提供了可参考的设计范式，强调需要解决现有基准的局限性以更全面准确地评估大语言模型能力。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的可信度问题日益受到关注，现有评估方法往往只关注狭窄方面，忽视了多模态引入的风险，需要建立一个全面的评估框架。

Method: 提出了三维框架：五个可信度维度（真实性、鲁棒性、安全性、公平性、隐私）、两种新型风险类型（多模态风险和跨模态影响）、多种缓解策略（数据、模型架构、训练和推理算法）。基于此构建了包含32个任务和28个数据集的MultiTrust-X基准。

Result: 实验发现当前模型存在显著漏洞，可信度与通用能力之间存在差距，多模态训练和推理会放大基础LLM的潜在风险。现有缓解策略在特定方面有所改进，但很少有方法能有效解决整体可信度问题，且许多方法会带来意外的权衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法使用静态生成长度和固定验证标准，无法适应不同模型不确定性和输入复杂度，限制了加速效果和适应性

Method: 利用草稿模型输出分布的熵和边界不确定性度量，动态调整每轮推测生成的token数量，并使用相同置信度信号调制验证过程，实现更灵活的token接受

Result: 在机器翻译和摘要任务上相比标准推测解码获得显著加速，同时保持或改进了BLEU和ROUGE分数

Conclusion: 该方法为大型语言模型在不同不确定性条件下提供了原则性的即插即用高效解码方案，减少回滚频率，提高资源利用率，保持输出保真度
...
TL;DR: 大型词汇表通过降低token化文本的复杂度来提升语言模型性能，主要受益于高频词汇的不确定性降低，而非缓解词汇不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 研究大型词汇表对语言模型性能提升的真正原因，现有实践倾向于使用越来越大的词汇表，但其益处来源尚不清楚。

Method: 进行控制实验，将词汇表从24K扩展到196K，同时保持数据、计算和优化不变，量化token化文本的复杂度，分析词汇频率分布和损失分解。

Result: 大于24K的词汇表主要通过降低前2500个高频词汇的不确定性来减少交叉熵，尽管稀有词汇的损失上升。词汇不平衡被模型利用而非遭受。

Conclusion: 词汇表大小的益处应重新理解为"降低token化文本复杂度有帮助"，这为tokenizer-模型协同设计提供了简单原则性的杠杆，并阐明了预训练中语言模型扩展的损失动态。
...
TL;DR: 本文系统分析了134篇关于基于证据的LLM文本生成的研究，提出了统一分类法，调查了300个评估指标，重点关注引用、归因和引用的方法，并指出了该领域的开放挑战和未来方向。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型(LLMs)的广泛应用引发了对其可靠性和可信度的担忧，需要建立基于证据的文本生成方法以确保输出的可追溯性和可验证性。但目前该领域存在术语不一致、评估实践孤立和缺乏统一基准的问题。

Method: 系统分析134篇论文，引入基于证据的LLM文本生成的统一分类法，调查300个评估指标（涵盖7个关键维度），重点关注使用引用、归因或引用的方法。

Result: 建立了该领域的统一分类框架，识别了代表性方法，并系统整理了评估指标体系。

Conclusion: 该研究为基于证据的LLM文本生成领域提供了系统性的分析框架，指出了当前存在的挑战，并为未来研究指明了有前景的发展方向。
...
TL;DR: 本文提出了MCR-BENCH基准测试，发现大型音频语言模型在处理音频-文本冲突信息时存在明显的文本偏向，经常忽略音频证据，导致音频中心任务性能下降。


<details>
  <summary>Details</summary>
Motivation: 大型音频语言模型(LALMs)虽然在音频感知方面有所增强，但其在处理音频和文本模态间冲突信息时的表现尚未得到充分研究，这在实际应用中可能带来可靠性问题。

Method: 开发了首个专门评估LALMs处理不一致音频-文本对的基准测试MCR-BENCH，通过广泛的音频理解任务评估，并研究影响因素和缓解策略，包括监督微调和模型置信度分析。

Result: 研究发现LALMs存在显著的文本偏向，当模态间存在不一致时，模型经常忽视音频证据，导致音频中心任务性能大幅下降，即使在矛盾输入下也表现出持续的过度自信。

Conclusion: 研究强调了在训练过程中需要改进模态平衡，并开发更复杂的融合机制，以增强处理冲突多模态输入时的鲁棒性。项目已在GitHub开源。
...
<details>
  <summary>Details</summary>
Motivation: 解决大型语音语言模型发展中存在的架构碎片化和缺乏透明度问题，促进研究的系统比较和可复现性。

Method: 构建包含1200万条语音文本对齐语料、1350万条多任务指令调优数据集和可复现评估基准的完整框架，并训练3.8B参数的参考模型。

Result: LLaSO-Base模型获得0.72的标准化分数，超越了可比模型，建立了强大的可复现基线。分析显示广泛训练覆盖能提升性能，但在未见任务上仍存在泛化差距。

Conclusion: 通过发布完整的数据、基准和模型堆栈，LLaSO为语音语言模型研究建立了基础性开放标准，有助于统一研究努力并加速社区驱动的进展。
...
TL;DR: 本文对隐私保护语言建模方法进行了全面研究，分析了现有方法的优势和局限性，为隐私保护语言模型研究提供了重要见解和未来方向。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型在各种应用中的广泛使用，模型可能记忆并泄露敏感数据的问题日益突出。隐私作为基本人权，保护语言模型中的隐私已成为关键研究领域，但目前对隐私风险程度和缓解方法的理解仍然有限。

Method: 本研究通过提供隐私保护语言建模方法的全面研究，深入概述了这些方法，重点分析了它们的优势，并调查了它们的局限性。

Result: 研究结果为隐私保护语言建模的持续研究做出了贡献，提供了有价值的见解。

Conclusion: 该研究明确了隐私保护语言建模的重要性，指出了现有方法的优缺点，并为未来研究方向提供了指导框架。
...
Conclusion: M-Help数据集填补了心理健康检测领域的重要空白，为AI模型提供了更全面的训练数据，有助于更有效地识别和帮助需要心理健康支持的人群。
...
TL;DR: 本文研究如何专业地将源语言中的非对等词汇翻译到目标语言，提出了不同的翻译方法和规则，并以达里语和乌兹别克语到英语、俄语的25个非对等词汇翻译为例。


<details>
  <summary>Details</summary>
Motivation: 消除语言理解中的误解，特别是针对不同文化背景下的非对等词汇（如食物、服装、文化传统词汇等），这些词汇在目标语言中往往没有直接对应词，需要进行专业翻译处理。

Method: 采用基于文献的研究方法，分析并提出了将非对等词汇从源语言翻译到目标语言的不同方法和规则。

Result: 研究成功提出了多种非对等词汇的翻译方法，并实际完成了25个达里语和乌兹别克语词汇到英语和俄语的专业翻译。

Conclusion: 通过系统的方法和规则，可以有效解决非对等词汇的翻译问题，促进不同语言和文化之间的准确理解和交流。
...
<details>
  <summary>Details</summary>
Motivation: 现有可编程任务导向对话代理的有效性依赖于准确的对话状态跟踪，但传统方法存在局限性，需要更有效的状态跟踪和错误纠正机制。

Method: PyTOD采用简单的约束解码方法，使用语言模型而非语法规则来遵循API模式，生成可执行代码进行状态跟踪，并利用策略和执行反馈进行错误纠正。

Result: 在具有挑战性的SGD基准测试中，PyTOD实现了最先进的对话状态跟踪性能，在准确性和鲁棒性方面均超越了强基线模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有的放射学报告自动评估方法要么提供粗糙的整体评分，要么依赖不透明的黑盒模型，缺乏临床基础、可解释性和细粒度指标，限制了在真实临床工作流程中的应用。

Method: 基于Group Relative Policy Optimization构建，包含两个关键创新：1) 基于实时F1统计动态调整临床挑战性错误类型权重的子分数动态加权；2) 基于子分数一致性调整策略梯度更新的多数引导优势缩放。
...
TL;DR: SLM4Offer是一个基于对比学习的生成式AI模型，通过微调T5-Small语言模型来生成个性化优惠，相比监督微调基线提升了17%的优惠接受率。
...
TL;DR: 本文质疑大型语言模型在处理用户主观浏览行为时的有效性，提出了一种基于聚类的异质性感知语言模型训练方法HeTLM，发现小型模型在特定领域表现优于大型模型，且异质性训练能提高平均性能并降低方差。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理用户主观行为和偏好时存在局限性，特别是当用户行为具有高度异质性和主观性时，单一模型难以有效捕捉不同用户的多样化浏览模式。

Method: 提出HeTLM（异质性感知语言模型训练）方法，使用页面级分词器训练小型语言模型，并采用基于聚类的异质性训练策略，为不同用户群体学习特定的参数集。

Result: 研究发现：1）使用页面级分词器的小型模型优于大型预训练或微调模型；2）异质性聚类特定参数集的HeTLM优于同家族单一模型；3）实现了更高的平均性能和更低的方差，表明对齐效果改善。

Conclusion: 针对主观用户行为，采用异质性感知的聚类训练方法比单一大型语言模型更有效，小型专业化模型在特定领域可以超越大型通用模型，并能提供更好的用户级对齐。
...
TL;DR: 本研究提出使用训练数据影响力作为模型中心化的难度指标，替代传统人工定义的难度标准，在语言模型预训练中实现了超过10个百分点的性能提升，证明了课程学习的有效性。


<details>
  <summary>Details</summary>
Motivation: 传统的课程学习在语言模型预训练中效果有限，主要原因是基于人工定义的难度指标（如文档复杂度）不能准确反映模型训练过程中的实际难度。

Method: 使用训练数据影响力作为难度指标对训练样本进行排序，该指标能够估计单个训练样本对模型输出的影响程度，从而构建更符合模型训练需求的课程。

Result: 采用模型中心化难度指标的课程学习方法在基准测试中比随机顺序训练的性能高出10个百分点以上。

Conclusion: 课程学习对语言模型预训练是有益的，关键在于采用更符合模型训练特性的难度定义方式，而非传统的人工中心化指标。
...
Method: 开发了开源基准测试管道，在4种硬件配置下评估15个SLM在9个NLP任务上的表现，使用23个数据集覆盖14个领域，量化11个正确性、计算和能耗指标。

Result: 评估结果显示不同SLM在准确性和能效方面存在显著权衡，部分模型在准确性上表现优异，而其他模型在能效方面更优。

Conclusion: SLM-Bench为SLM评估设立了新标准，弥合了资源效率与实际应用之间的差距，为后续研究提供了可复现的标准化评估协议。
...
TL;DR: HebID是首个希伯来语多标签社会身份检测语料库，包含5536个以色列政客Facebook帖子的句子，标注了12种细粒度社会身份，使用希伯来语调优的LLM获得最佳效果（F1=0.74），可用于研究精英话语与公众身份认同的差异。
...
Method: 构建HebID语料库（5536个以色列政客Facebook帖子句子），手动标注12种细粒度社会身份，基于调查数据，比较多标签/单标签编码器和2B-9B参数生成式LLM的性能。

Result: 希伯来语调优的LLM表现最佳（宏观F1=0.74），应用分类器分析政客帖子和议会演讲，发现身份表达的流行度、时间趋势、聚类模式和性别相关差异。
...
TL;DR: Dream 7B是目前最强大的开源扩散大语言模型，采用离散扩散建模实现并行序列生成，在通用、数学和编程任务上表现优异，具有卓越的规划能力和灵活的推理特性。


<details>
  <summary>Details</summary>
Motivation: 为了解决自回归模型顺序生成的局限性，探索扩散模型在语言生成中的潜力，实现更高效的并行生成和灵活的推理能力。

Method: 采用离散扩散建模方法，通过迭代去噪并行精炼序列；使用自回归LLM初始化和上下文自适应的token级噪声重调度等简单有效的训练技术。

Result: 在通用任务、数学任务和编程任务上持续超越现有扩散语言模型；展示了卓越的规划能力、任意顺序生成、填充能力和可调节的质量-速度权衡。

Conclusion: Dream 7B证明了扩散模型在语言建模中的强大潜力，通过创新的训练技术实现了优异的性能，为基于扩散的语言建模研究提供了重要基础。
...
TL;DR: 本研究首次对政治去合法化话语进行大规模计算分析，构建了希伯来语语料库并开发了两阶段分类管道，发现政治去合法化话语在过去三十年中显著增加，社交媒体上更普遍，右翼政治人物使用更多。


<details>
  <summary>Details</summary>
Motivation: 政治去合法化话语对民主制度构成威胁，但缺乏系统性的大规模计算研究。本研究旨在填补这一空白，通过自动化分析理解政治话语中的去合法化现象。

Method: 构建包含10,410个句子的希伯来语语料库，手动标注1,812个PDD实例；开发两阶段分类管道，结合微调编码器模型和解码器LLM；使用DictaLM 2.0模型进行二元PDD检测和多特征分类。

Result: 最佳模型在二元PDD检测上达到F1分数0.74，多特征分类宏F1分数0.67；分析显示PDD在过去三十年中显著上升，社交媒体比议会辩论更普遍，男性政治人物使用更多，右翼行为者倾向更强，选举期间和重大政治事件中出现峰值。

Conclusion: 自动化PDD分析对于理解民主话语具有可行性和重要价值，为监测和分析政治去合法化现象提供了有效工具。
...
TL;DR: SafetyFlow是一个自动化构建LLM安全评估基准的代理流系统，能在4天内无需人工干预构建包含23,446个查询的全面安全基准，显著降低时间和资源成本。
...
Method: 通过协调7个专门代理构建自动化流水线，集成人类专业知识，确保过程可控和成本可控，最终生成低冗余、强判别力的安全基准数据集。

Result: 构建了SafetyFlowBench数据集，包含23,446个查询，评估了49个先进LLM的安全性，验证了系统的有效性和效率。

Conclusion: SafetyFlow是首个完全自动化的基准构建流水线，提供了全面的安全基准，显著提升了LLM安全评估的效率和覆盖范围。
...
TL;DR: 提出"训练微型模型"概念，通过小型语言模型针对特定高价值应用进行微调，以极低成本生成类似领域特定响应
...
Method: 使用小型语言模型针对特定领域进行微调，创建专门的"训练微型模型"

Result: 能够以极低成本生成与大型模型相似的领域特定响应

Conclusion: 训练微型模型为特定高价值应用提供了经济高效的替代方案，大幅降低了部署成本
...
<details>
  <summary>Details</summary>
Motivation: 发现LLMs作为判别器识别有害请求的能力比作为生成器防御攻击的能力更强，这种安全不一致性促使研究者探索对齐模型的判别和生成能力。

Method: 提出SDGO强化学习框架，使用模型自身的判别能力作为奖励信号，通过迭代自改进来增强生成安全性，训练阶段不需要额外标注数据或外部模型。

Result: SDGO显著提高了模型安全性，在通用基准上保持有用性，对分布外越狱攻击表现出鲁棒性能，且只需少量判别样本就能进一步增强生成能力。

Conclusion: 通过对齐LLMs的判别和生成能力，SDGO实现了两种能力的紧密耦合，为模型安全提供了一种有效且实用的解决方案。
...
TL;DR: 本文介绍了SurGE基准测试，用于评估计算机科学领域的科学综述生成，包含测试实例和大规模学术语料库，并提出了自动化评估框架。


<details>
  <summary>Details</summary>
Motivation: 由于学术文献快速增长，手动撰写科学综述变得不可行，而现有大语言模型在自动生成综述方面缺乏标准化基准和评估协议。

Method: 创建SurGE基准，包含主题描述、专家撰写的综述及其引用文献的测试实例，以及超过100万篇论文的学术语料库；提出自动化评估框架，从信息覆盖度、引用准确性、结构组织和内容质量四个维度评估生成综述。
...
<details>
  <summary>Details</summary>
Motivation: 位置偏差（PB）表现为不同上下文位置的非均匀敏感性，严重损害了长上下文理解能力。虽然之前的工作通过修改架构来缓解PB，但显著的PB仍然存在，需要更有效的解决方案。

Method: 提出了Pos2Distill框架，利用位置诱导的差异来对抗PB本身。针对检索和推理两种范式设计了专门实例：Pos2Distill-R¹和Pos2Distill-R²，通过知识蒸馏将优势位置的能力转移到劣势位置。

Result: 使用Pos2Distill方法，在长上下文检索和推理任务中实现了增强的均匀性和显著的性能提升。两个专门化系统都表现出强大的跨任务泛化能力，同时在各自任务上达到优异性能。

Conclusion: Pos2Distill通过位置到位置的知识蒸馏有效缓解了位置偏差问题，为长上下文处理提供了新的解决方案，在保持任务特异性的同时实现了良好的泛化性能。
...
TL;DR: 本文对孟加拉语词干提取方法进行全面调查，指出该领域存在研究断层、实现稀缺和评估方法不足等问题，并提出了未来发展方向建议。


<details>
  <summary>Details</summary>
Motivation: 孟加拉语作为全球第七大语言面临数字资源不足的问题，词干提取对于这种低资源、高屈折性语言的分析处理至关重要，但现有研究存在明显空白。

Method: 通过对孟加拉语词干提取方法的全面文献调查和分析，批判性地评估现有方法的局限性，并提出改进方向。

Result: 发现孟加拉语词干提取领域存在研究断层、可复现实现稀缺、评估指标不相关等问题，特别是在处理丰富形态和方言多样性方面面临挑战。

Conclusion: 需要开发更强大的孟加拉语词干提取器，采用更相关的评估指标，并持续进行研究以提升语言分析和处理能力。
...
TL;DR: 提出了EcomMMMU电商多模态数据集和SUMEI方法，发现产品图像并不总是提升模型性能，有时反而会降低效果


<details>
  <summary>Details</summary>
Motivation: 电商平台包含丰富的多模态数据，但现有数据集规模有限，难以系统研究图像是否真正增强产品理解能力，以及图像是否会引入冗余或降低性能

Method: 构建了包含406,190个样本和8,989,510张图像的EcomMMMU数据集，包含8个核心任务；提出了SUMEI方法，通过预测视觉效用来策略性地利用多张图像进行下游任务

Result: 分析发现产品图像并不总是提升性能，有时反而会降低效果，表明MLLMs在有效利用丰富视觉内容方面存在困难；SUMEI方法在综合实验中显示出有效性和鲁棒性

Conclusion: 需要更智能的方法来选择和利用多模态数据，SUMEI提供了一种数据驱动的方法来优化多图像在电商任务中的使用
...
TL;DR: Deep-DxSearch是一个基于强化学习训练的智能RAG系统，通过端到端训练提升医疗诊断准确性，在多个数据集上超越现有方法。
...
Result: 在多个数据中心超越提示工程和无训练RAG方法，诊断准确性显著提升，优于GPT-4o、DeepSeek-R1等强基线模型，在常见和罕见疾病诊断中均有优异表现。

Conclusion: 端到端智能RAG强化学习训练框架有效提升医疗诊断性能，奖励设计和检索语料库组件对性能提升至关重要，为临床提供更可靠精确的初步诊断支持。
...
TL;DR: ReasonZoo基准测试显示，工具集成推理(TIR)能显著提升大语言模型在数学和非数学推理任务中的表现，并通过新提出的效率指标证明TIR能减少过度思考、提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在需要精确计算的任务中存在不足，工具集成推理(TIR)虽然被提出作为解决方案，但其对模型推理能力的泛化提升效果和具体作用机制尚不明确。

Method: 提出了ReasonZoo综合基准测试，涵盖9个不同推理类别；设计了两个新指标PAC和AUC-PCC来评估推理效率；通过实证评估比较TIR启用模型与非TIR模型的性能差异。

Result: TIR启用模型在数学和非数学任务中均持续优于非TIR模型；TIR提高了推理效率，表现为改进的PAC和AUC-PCC指标，表明减少了过度思考并实现了更简化的推理过程。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估AI代理在真实动态场景中使用多样化MCP工具解决多步骤任务的基准测试，需要填补这一空白。
...
TL;DR: 本文重新审视了人脸关键点检测中广泛使用的Soft-argmax方法，提出了一种基于结构化预测框架的替代训练目标，在三个基准数据集上实现了最先进性能，训练收敛速度提升2.2倍。


<details>
  <summary>Details</summary>
Motivation: 当前基于热图回归的人脸关键点检测方法普遍使用Soft-argmax作为可微近似，但这种方法并非唯一选择。作者希望探索更有效的替代方案来提升性能。

Method: 提出基于经典结构化预测框架的训练目标，替代传统的Soft-argmax方法，实现端到端训练。

Result: 在WFLW、COFW和300W三个人脸关键点基准数据集上达到最先进性能，训练收敛速度提升2.2倍，同时保持更好/竞争性的准确率。

Conclusion: Soft-argmax并非实现高性能人脸关键点检测的唯一方法，基于结构化预测的替代方案能够提供更快的收敛速度和竞争性的性能表现。
...
TL;DR: 本研究提出了一种将图卷积网络(GCN)与Voronoi图相结合的新方法，通过将图像表示为图结构并利用Delaunay三角剖分优化表示，显著提升了图像分类的预处理效率和分类精度。
...
Result: 在多个基准数据集上取得了显著的预处理效率提升和分类准确率改进，超越了最先进的方法，特别是在复杂场景和细粒度分类任务中表现优异。
...
TL;DR: YOPO是一个单阶段、基于查询的框架，将类别级9自由度姿态估计统一到2D检测中，仅使用RGB图像就达到了最先进性能


<details>
  <summary>Details</summary>
Motivation: 需要一种更简单的RGB-only替代方案，直接从类别级别学习，统一物体检测和9自由度姿态估计，无需额外数据
...
Conclusion: YOPO证明了单阶段RGB-only方法能够有效统一物体检测和9自由度姿态估计，填补了与RGB-D系统的性能差距
...
TL;DR: 提出了一种统一的训练框架Paired-Sampling Contrastive Framework，通过自动匹配的真实和攻击自拍对来学习模态无关的活体检测线索，在物理和数字攻击检测任务上达到2.10%的平均分类错误率。


<details>
  <summary>Details</summary>
Motivation: 传统的人脸反欺诈系统使用单独的模型处理物理呈现攻击和数字伪造攻击，这增加了系统复杂性、推理延迟，并且容易受到组合攻击向量的威胁。

Method: 采用配对采样对比框架，利用自动匹配的真实和攻击自拍照对，学习模态无关的活体检测特征，实现统一的攻击检测。

Result: 在第6届人脸反欺诈挑战赛的统一物理-数字攻击检测基准上取得了2.10%的平均分类错误率，优于现有解决方案，模型轻量（4.46 GFLOPs）且训练时间短于1小时。

Conclusion: 该框架提供了一种高效统一的人脸反欺诈解决方案，具有实际部署的可行性，代码和预训练模型已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有生成对抗攻击方法生成图像质量低且计算资源消耗大，扩散模型虽然能生成高质量图像但需要数百步采样，因此需要开发高效的对抗图像生成方法

Method: 在无条件扩散模型的混合步间隔注入扰动，采用选择性RGB通道策略：对红色通道应用注意力图，对绿色和蓝色通道使用GradCAM引导的扰动

Result: 在ImageNet数据集上，对VGGNet源模型，攻击ResNet成功率达70.6%，MNASNet达80.8%，ShuffleNet达97.8%；PSNR超过30dB，生成速度快10倍

Conclusion: TAIGen是最高效的对抗攻击方法，防御机制对其生成的图像净化效果最差，具有最强的攻击影响力
...
<details>
  <summary>Details</summary>
Motivation: 现有CVP方法主要局限于掩码域的可逆策略，未能充分利用RGB域的潜力，需要一种能够在两个域同时工作并有效处理不确定性的方法。

Method: 将CVP任务公式化为数学优化问题，展开为多阶段深度网络，包含三个模块：CORE（掩码域可逆建模）、CARE（RGB域可逆建模）和FINE（基于噪声的细化扩散模型）。

Result: 提出的方法能够有效识别核心对象区域，改善前景-背景分离，并通过针对性扩散模型细化不确定区域，显著减少误报和漏报。

Conclusion: RUN++提供了一个新的CVP系统构建范式，在真实世界退化条件下保持有效性，并扩展为更广泛的双层优化框架。
...
<details>
  <summary>Details</summary>
Motivation: 牲畜甲烷排放占人为甲烷排放的32%，需要自动化监测来支持气候缓解策略，但缺乏有效的实时监测解决方案。

Method: 提出GasTwinFormer混合视觉变换器，采用新颖的Mix Twin编码器（交替使用空间缩减全局注意力和局部分组注意力机制）和轻量级LR-ASPP解码器，实现甲烷分割和饮食分类的统一框架。

Result: 在11,694个标注帧的数据集上，达到74.47% mIoU和83.63% mF1的分割性能，仅3.348M参数和3.428G FLOPs，推理速度114.9 FPS，饮食分类准确率100%。

Conclusion: GasTwinFormer通过验证的架构组件，成为实时牲畜排放监测的实用解决方案，有效利用饮食-排放相关性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的整流流模型基于数据和噪声分布之间的线性轨迹，这种线性性强制零曲率，可能导致图像生成过程穿过数据流形的低概率区域。需要探索轨迹曲率与生成图像和文本描述语义对齐之间的关系。

Method: 引入CurveFlow框架，直接在流路径中融入曲率指导，采用鲁棒的曲率正则化技术来惩罚轨迹内在动态的突变，学习平滑的非线性轨迹。

Result: 在MS COCO 2014和2017数据集上的实验表明，CurveFlow在文本到图像生成方面达到最先进性能，显著优于标准整流流变体和其他非线性基线方法，在BLEU、METEOR、ROUGE和CLAIR等语义一致性指标上表现尤为突出。

Conclusion: 曲率感知建模显著增强了模型忠实遵循复杂指令的能力，同时保持了高图像质量，证明了曲率指导在提升文本到图像生成语义一致性方面的重要性。
...
Result: 仅在合成失真上训练，HiRQA能有效泛化到真实退化场景（如镜头光晕、雾霾、运动模糊、低光条件）。轻量版HiRQA-S推理时间仅3.5ms/图像，在合成和真实基准测试中达到最先进性能。

Conclusion: HiRQA通过自监督的层次排序和质量对齐方法，解决了无参考图像质量评估的泛化问题，在保持高精度的同时实现了实时部署能力，为实际应用提供了有效解决方案。
...
TL;DR: 提出基于投资组合理论的边缘资源管理策略，用于在动态边缘环境中保证多视角3D重建的可靠性，即使相机存在时空相关中断也能维持重建质量


<details>
  <summary>Details</summary>
Motivation: 多视角3D重建在应急响应等关键应用中需要近实时性能，但边缘环境的动态性和操作困难会导致时空相关中断，影响相机操作和重建质量

Method: 采用投资组合理论启发的方法，通过遗传算法解决优化问题，选择相机组合来对抗可能的系统中断

Result: 使用公开和定制3D数据集验证，相比传统基线策略，该方法能在时空中断下保证可靠的3D重建质量

Conclusion: 提出的相机选择策略能有效应对边缘环境中的时空相关中断，为关键应用提供可靠的多视角3D重建服务
...
Method: 集成专用医学视觉编码器和LVLM核心，采用多任务提示工程和多阶段微调，从眼底图像理解病理特征并生成包含DR严重程度分级、关键病理概念识别和详细解释的诊断报告
...
TL;DR: 提出MeSS方法，利用城市网格模型作为几何先验，通过改进的图像扩散模型生成高质量、风格一致的室外场景纹理，解决了现有方法在3D场景生成中的几何对齐和视角一致性问题。


<details>
  <summary>Details</summary>
Motivation: 城市网格模型缺乏真实纹理限制了其在虚拟城市导航和自动驾驶中的应用，现有图像和视频扩散模型在3D场景生成中存在视角一致性差、几何对齐不准确等问题。
...
Method: 使用学习方法来推导规则智能体的对抗行为，通过对抗智能体来测试各种规则智能体的表现

Result: 评估显示对抗智能体能够有效降低所有规则智能体的累积奖励，证明成功制造了故障场景
...
TL;DR: DyMorph-B2I是一个针对肾脏病理学的动态形态学引导的二元到实例分割管道，通过整合分水岭、骨架化和形态学操作，实现更好的实例分离和形态计量分析。


<details>
  <summary>Details</summary>
Motivation: 现有的肾脏病理学数据集和自动化方法主要提供二元（语义）掩码，限制了后续分析的精确性。传统后处理技术在分离复杂连接的肾脏组织结构时效果有限。

Method: 提出DyMorph-B2I管道，整合分水岭、骨架化和形态学操作，采用自适应几何细化和可定制的超参数调优，针对不同功能单元类别进行系统参数优化。

Result: 实验结果表明该方法优于单个传统方法和简单组合方法，能够稳健地分离二元掩码中的粘连和异质结构，实现更优的实例分离。

Conclusion: DyMorph-B2I为肾脏病理学工作流程提供了更准确的实例分割解决方案，促进了更精确的形态计量分析，代码已公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 现有ADAS系统依赖多种传感器，成本高且部署复杂。仅使用行车记录仪视频是一个更具成本效益和易部署的解决方案，但预测准确性面临挑战

Method: 采用改进的时空特征提取方法，通过循环网络聚合这些特征，基于图神经网络构建STAGNet模型用于行车记录仪视频的事故预测

Result: 在三个公开数据集上的实验表明，STAGNet模型在平均精度和平均碰撞时间指标上均优于现有方法，在跨数据集测试中也表现良好

Conclusion: STAGNet模型证明了仅使用行车记录仪视频就能有效预测交通事故的可行性，为低成本、易部署的ADAS系统提供了有前景的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有3D生成架构大多局限于单一模态或3D结构，无法充分利用多模态数据的互补优势，限制了训练数据集的规模和模型性能。

Method: 1）引入协作多模态编码整合模态特定特征；2）采用辅助2D和3D监督提升鲁棒性；3）基于triplane潜在扩散模型生成高质量3D资产

Result: 在多个知名数据集上的实验表明，TriMM通过有效利用多模态数据，在少量训练数据下实现了与大规模数据集训练模型相竞争的性能，并在RGB-D数据集上验证了整合其他多模态数据的可行性。

Conclusion: TriMM成功证明了多模态数据在3D生成中的价值，为利用多样化数据源进行3D内容创建提供了有效框架。
...
TL;DR: 提出双高度无人机协作视觉语言导航任务(DuAl-VLN)，通过高低空无人机协作解决传统无人机VLN任务中轨迹长、机动复杂的问题，并构建HaL-13k数据集和AeroDuo框架。


<details>
  <summary>Details</summary>
Motivation: 传统无人机视觉语言导航存在轨迹过长、机动复杂的问题，需要人工干预或过于详细的指令。利用无人机高机动性提供多粒度视角的同时，需要保持可控的运动空间。

Method: 提出双高度无人机协作框架：高空无人机负责环境推理，低空无人机负责精确导航。构建HaL-13k数据集（13,838条协作轨迹），开发AeroDuo框架，高空使用多模态大语言模型进行目标推理，低空使用轻量级多阶段策略进行导航和目标定位。

Result: 构建了包含13,838条协作轨迹的HaL-13k数据集，包含未见地图和未见目标验证集，系统评估模型在新环境和陌生目标上的泛化能力。

Conclusion: 双无人机协作VLN框架通过高低空无人机优势互补，仅交换最小坐标信息确保效率，为解决复杂户外环境下的无人机导航问题提供了有效方案。
...
TL;DR: 本文提出了一种跳过步采样机制，通过马尔可夫方式实现扩散模型加速采样，无需依赖非马尔可夫过程，显著减少生成步骤的同时保持高质量输出。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型需要顺序生成过程，步骤繁多效率低。虽然DDIM等方法尝试减少采样步骤，但缺乏对原始扩散过程是否能够以马尔可夫方式实现相同效率的理解。

Method: 提出跳过步采样机制，在迭代生成过程中绕过多个中间去噪步骤，与传统逐步细化方法形成对比。该方法基于与标准扩散模型相同的训练目标推导而来，并与DDIM集成增强生成效果。

Result: 在OpenAI ADM、Stable Diffusion和Open Sora等流行预训练扩散模型上的大量实验表明，该方法能够以显著减少的采样步骤实现高质量生成。

Conclusion: 跳过步采样是预训练扩散模型的内在属性，通过马尔可夫方式实现加速采样，为扩散模型的高效生成提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像编解码器编码模式有限且依赖工程师手动选择，对非专业用户不友好。需要一种更智能、交互式的图像压缩解决方案。
...
Conclusion: 该方法为图像压缩领域的人工通用智能提供了有前景的途径，推动了图像编码范式的演进。
...
TL;DR: Ano-NAViLa是一种基于预训练视觉语言模型的病理图像异常检测方法，通过整合正常和异常病理知识，在淋巴结数据集上实现了最先进的异常检测和定位性能。
...
Conclusion: Ano-NAViLa通过知识增强的视觉语言模型有效解决了病理图像异常检测的挑战，提供了高准确性和可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有车道拓扑推理方法采用先检测后推理的范式，基于一对一分配进行监督，导致监督范围有限和性能次优。
...
TL;DR: 本文提出了DesignCLIP框架，利用CLIP视觉语言模型处理设计专利分析任务，通过类别感知分类和对比学习等方法，在专利分类和检索任务上超越了现有最佳模型。
...
Method: 开发DesignCLIP统一框架，采用类别感知分类和对比学习，利用生成的详细图像描述和多视图图像学习来处理专利数据的独特特征。

Result: DesignCLIP在专利分类和检索等各种下游任务中始终优于基线和最先进模型，展示了多模态方法在推进专利分析方面的潜力。

Conclusion: 多模态方法在专利分析领域具有巨大潜力，DesignCLIP框架通过结合视觉语言模型有效解决了传统专利图像分析的局限性，为设计创新提供了更多样化的灵感来源。
...
<details>
  <summary>Details</summary>
Motivation: 超声视频中的先天性心脏病检测受图像噪声和探头位置变异性的影响，现有机器学习方法常忽略时序信息、仅限于二分类且未考虑预测校准问题

Method: TPA方法使用图像编码器提取视频片段每帧特征，通过可训练时序提取器聚合特征捕捉心脏运动，利用边界铰链对比损失将视频表示与类别特定文本提示对齐，并引入CVAESM模块学习潜在风格向量来调制嵌入并量化分类不确定性

Result: 在CHD检测上达到85.40%的宏F1分数，预期校准误差降低5.38%，自适应ECE降低6.8%；在EchoNet-Dynamic三分类任务上宏F1提升4.73%

Conclusion: TPA是一个整合时序建模、提示感知对比学习和不确定性量化的框架，在胎儿先天性心脏病超声视频分类中表现出色，提高了临床可靠性
...
TL;DR: 本文提出了BasketLiDAR数据集和新型多模态MOT框架，结合LiDAR点云和多视角相机数据，在职业篮球环境中实现了实时3D球员追踪，解决了传统相机方法在遮挡条件下的性能瓶颈。


<details>
  <summary>Details</summary>
Motivation: 传统基于多相机的体育追踪系统受限于2D视频数据和复杂3D重建处理，难以实现实时分析。篮球场景中球员快速移动、频繁遮挡，是MOT领域最具挑战性的场景之一。

Method: 构建首个结合LiDAR点云和同步多视角相机数据的多模态数据集BasketLiDAR，包含4,445帧和3,105个球员ID。提出基于LiDAR高精度3D空间信息的新型MOT算法，包括纯LiDAR实时追踪管道和LiDAR-相机融合的多模态追踪管道。

Result: 实验结果表明，该方法实现了传统纯相机方法难以达到的实时运行，并在遮挡条件下仍保持优异的追踪性能，同时降低了计算成本。

Conclusion: BasketLiDAR数据集和提出的多模态MOT框架为体育分析领域提供了新的解决方案，通过LiDAR技术有效解决了篮球等复杂运动场景中的实时3D追踪难题。
...
TL;DR: RAG-SEG是一种无需训练的两阶段伪装目标检测方法，通过检索增强生成粗掩码作为提示，再使用SAM进行精细分割，在个人笔记本上实现与SOTA相当的性能。


<details>
  <summary>Details</summary>
Motivation: 解决伪装目标检测中目标与背景高度相似、现有方法需要大量训练和计算资源的问题，同时克服基础模型如SAM在COD任务中需要高质量提示的局限性。

Method: 将COD解耦为两个阶段：1）通过无监督聚类构建检索数据库，使用检索增强生成粗掩码作为提示；2）使用SAM2进行精细分割。整个过程无需训练。

Result: 在基准COD数据集上的大量实验表明，RAG-SEG的性能与或超过最先进方法，所有实验均在个人笔记本上完成，证明了计算效率和实用性。

Conclusion: RAG-SEG提供了一种高效、实用的COD解决方案，无需传统训练过程，在保持竞争力的同时显著降低了计算资源需求。
...
TL;DR: VideoEraser是一个无需训练的即插即用框架，通过两阶段处理防止文本到视频扩散模型生成有害内容，在四个任务中平均减少46%的不良内容生成。


<details>
  <summary>Details</summary>
Motivation: 文本到视频扩散模型可能被滥用来生成有害或误导性内容，这些模型通常使用未经授权的个人身份、艺术创作和有害材料进行训练，导致不受控制的内容生产和传播。

Method: 采用两阶段过程：选择性提示嵌入调整（SPEA）和抗干扰噪声引导（ARNG），作为即插即用模块无缝集成到主流文本到视频扩散模型中。

Result: 在对象擦除、艺术风格擦除、名人擦除和显式内容擦除四个任务中，VideoEraser在效能、完整性、保真度、鲁棒性和泛化性方面持续优于现有方法。

Conclusion: VideoEraser在抑制文本到视频生成中的不良内容方面达到了最先进的性能，为防范扩散模型滥用提供了有效的解决方案。
...
TL;DR: 本研究开发了一个端到端的深度学习框架，使用姿态检测和序列建模技术来预测行人过马路意图，比较了GRU、LSTM和1D CNN三种模型的效果。


<details>
  <summary>Details</summary>
Motivation: 随着自动驾驶技术的发展，车辆需要能够从远处预测行人是否要过马路，这对交通安全至关重要。

Method: 结合深度学习姿态检测模型和序列建模技术（GRU、LSTM、1D CNN），通过视频分析构建端到端的预测框架。
...
TL;DR: RCDINO是一种基于transformer的多模态3D目标检测模型，通过融合DINOv2基础模型的语义丰富表示来增强视觉特征，在nuScenes数据集上达到最先进性能


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶和机器人领域中三维目标检测需要有效融合摄像头和雷达多模态数据的问题，提升检测性能

Method: 提出基于transformer的RCDINO模型，将预训练的DINOv2基础模型的语义丰富表示与视觉骨干网络特征进行融合，增强视觉表示能力

Result: 在nuScenes数据集上实现56.4 NDS和48.1 mAP的state-of-the-art性能，在雷达-摄像头模型中表现最佳
...
TL;DR: 本文通过注意力敲除技术系统分析Video-LLMs的内部机制，发现视频信息处理主要在早期层完成，呈现两阶段模式，且时空建模更依赖语言引导而非自注意力计算。


<details>
  <summary>Details</summary>
Motivation: 现有Video-LLMs研究主要关注性能提升，缺乏对其内部工作机制的理解，本文旨在填补这一空白。

Method: 采用注意力敲除技术，设计三种敲除变体（时间、空间、语言到视频），通过全局和细粒度两种设置分析不同层的影响。

Result: 发现视频信息提取主要在早期层完成，中间层存在关键异常点，时空建模更依赖语言引导检索而非自注意力机制。

Conclusion: 研究揭示了Video-LLMs的内部处理机制，为未来研究提供可解释性和效率提升的视角，并能减少注意力计算开销。
...
TL;DR: BioTune是一种进化自适应微调技术，通过进化算法选择性地微调层来提升迁移学习效率，减少可训练参数数量，在多个图像分类数据集上实现了竞争性或改进的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 深度学习在图像分析中取得显著进展，但大型全训练模型的计算需求仍然是一个问题。传统微调方法需要更新所有模型参数，可能导致过拟合和更高的计算成本。

Method: 提出BioTune进化自适应微调技术，使用进化算法识别需要微调的层子集，专注于相关层进行微调，而不是更新所有参数。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS压缩方法只能压缩到兆字节级别，无法处理大规模场景，且缺乏长期归档后场景变化的适应机制

Method: 图像条件化高斯溅射量化器，联合利用高斯间和属性间相关性，使用跨场景共享码书，编码-量化-解码联合训练，支持基于图像的条件解码

Result: 将3DGS存储需求降低到千字节范围，保持视觉保真度，在压缩效率和场景变化适应性方面均优于现有方法

Conclusion: ICGS-Quantizer显著提升了3D高斯溅射的压缩效率，解决了大规模场景压缩和长期归档后场景更新的实际问题
...
TL;DR: DriveSplat提出了基于神经高斯表示的动态-静态解耦方法，通过区域体素初始化、可变形神经高斯和深度法线先验监督，解决了驾驶场景中动态物体和静态背景的3D重建挑战。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的方法在解耦动态静态组件时忽视了背景的几何关系优化，仅通过添加高斯来拟合训练视图，导致新视角渲染鲁棒性有限且几何表示不准确。

Method: 采用区域体素初始化方案将场景分为近、中、远区域；引入可变形神经高斯建模非刚性动态物体；使用预训练模型的深度和法线先验进行监督。

Result: 在Waymo和KITTI数据集上进行了严格评估，在驾驶场景的新视角合成方面达到了最先进的性能。

Conclusion: DriveSplat通过创新的动态-静态解耦框架和几何先验监督，显著提升了驾驶场景3D重建的质量和几何准确性。
...
TL;DR: 本文针对深度学习模型在抽象推理方面的瓶颈，通过Raven渐进矩阵(RPM)问题作为评估基准，提出了基于因果链建模的方法，并逐步改进基线模型DIO以解决互信息最大化方法的局限性。
...
Method: 采用因果链建模视角分析RPM任务的完整因果链，设计了基线模型DIO。针对DIO优化目标（最大化上下文与正确答案间的互信息变分下界）的局限性，逐步提出了三种改进方法。

Result: 实验发现DIO模型的互信息最大化优化目标无法让模型真正获得预定义的人类推理逻辑，主要原因是下界紧致性影响优化效果，且互信息作为统计度量无法捕捉因果关系。

Conclusion: 需要克服互信息最大化方法的局限性，通过渐进式改进方法来提升模型在RPM任务中的抽象推理性能，使模型能够真正学习到人类推理逻辑。
...
TL;DR: 本文分析了MAE如何学习输入图像的空间相关性，推导了线性MAE学习到的特征，并证明掩码比例和补丁大小可用于选择捕捉短程和长程空间相关性的特征。


<details>
  <summary>Details</summary>
Motivation: 尽管MAE在视觉基础模型预训练中表现强大，但在应用于新数据集时需要大量超参数调优，而MAE超参数与下游任务性能之间的关系尚未得到充分探索。

Method: 通过分析推导线性MAE学习到的特征，并扩展到非线性MAE，研究MAE如何适应数据集中的空间相关性。

Result: 研究发现掩码比例和补丁大小可以控制MAE学习短程和长程空间相关性的能力，MAE表示能够适应超越二阶统计量的数据集空间相关性。

Conclusion: 研究为实践中如何选择MAE超参数提供了理论见解，帮助更好地理解MAE学习机制和超参数调优策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于学习的多帧方法采用滑动窗口方式聚合相邻帧信息，但忽略了窗口外的全局时间信息，导致冗余计算和次优性能

Method: 设计双向传播策略：前向和后向分支分别包含局部时间运动融合(LTMF)模块建模相邻帧时空依赖，以及全局时间运动融合(GTMF)模块聚合全局传播特征。最后融合双向特征进行检测，并采用检测损失和时空融合(STF)损失联合优化

Result: 大量实验表明BIRD方法不仅达到最先进性能，而且具有快速推理速度

Conclusion: BIRD方法通过双向时间信息传播有效解决了现有滑动窗口方法的局限性，在红外小目标检测任务中表现出优异性能
...
TL;DR: 该论文提出基于YOLOv8的深度学习解决方案，用于自动检测汽车表面的微小凹陷缺陷，相比传统人工检测方法具有更高的准确性和实时性。


<details>
  <summary>Details</summary>
Motivation: 传统汽车损伤检测方法劳动密集、手动操作且经常忽略微小表面缺陷，需要更快更精确的自动化检测解决方案。

Method: 使用YOLOv8目标检测框架，创建包含各种光照条件、角度和纹理的定制数据集，采用实时数据增强技术训练YOLOv8m及其定制变体YOLOv8m-t4和YOLOv8m-t42模型。

Result: YOLOv8m-t42模型表现最佳，精度0.86、召回率0.84、F1分数0.85，mAP@0.5达到0.60，PR曲线面积0.88，优于YOLOv8m-t4模型。

Conclusion: YOLOv8m-t42模型在微小凹陷检测方面具有更高的准确性和实用性，适合实时应用如自动化保险评估和汽车检测，尽管收敛速度较慢。
...
<details>
  <summary>Details</summary>
Motivation: 视频到视频时刻检索任务需要解决语义帧级对齐和建模查询视频与目标视频间复杂依赖关系的挑战，现有方法在这方面存在不足。

Method: 提出MATR模型，使用Transformer架构捕获语义上下文和时序细节，采用双阶段序列对齐编码相关性，结合前景/背景分类和边界预测头，并通过自监督预训练技术进行任务特定初始化。

Result: 在ActivityNet-VRL数据集上R@1提升13.1%，mIoU提升8.1%；在新提出的SportsMoments数据集上R@1提升14.7%，mIoU提升14.4%。

Conclusion: MATR通过创新的双阶段对齐和自监督预训练方法，有效解决了视频到视频时刻检索的核心挑战，取得了显著的性能提升。
...
TL;DR: 提出了一种无需SfM的3D高斯泼溅方法，能够在极稀疏视角输入下联合估计相机位姿并重建3D场景，显著提升了渲染质量
...
Conclusion: 该方法有效解决了3DGS在极稀疏输入下的性能瓶颈，为真实场景应用提供了可行的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 单视角方法ECON在3D人体重建方面存在局限，需要利用多视角信息来提升几何精度和身体姿态估计的准确性

Method: 提出联合多视角身体优化(JMBO)算法，跨所有输入视角拟合SMPL-X身体模型；使用优化后的身体模型作为低频先验，通过法线图积分添加几何细节；整合前后视角的法线图来捕捉服装褶皱和发型等细节

Result: 实验结果表明MExECON相比单视角基线方法在保真度上持续提升，与现代少样本3D重建方法相比具有竞争力

Conclusion: 该方法在不需重新训练网络的情况下实现了多视角优势，为穿衣人体3D重建提供了有效的多视角解决方案
...
Method: 提出AdaSFFuse框架，包含自适应近似小波变换(AdaWAT)进行频率解耦，以及空间-频率Mamba块进行跨域融合，通过可学习映射动态调整实现稳健融合。

Result: 在红外-可见光、多焦点、多曝光和医学图像融合四个任务上的广泛实验表明，AdaSFFuse具有卓越的融合性能，同时保持低计算成本和紧凑网络结构。

Conclusion: AdaSFFuse通过创新的频率解耦和跨域融合机制，有效提升了多模态特征的对齐和整合，减少了频率损失，在性能和效率之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 从记录的行车日志合成外推视图对自动驾驶模拟至关重要，但现有方法使用生成先验作为伪真值往往导致几何一致性差和渲染过度平滑的问题。
...
Method: 采用两阶段流程：1）运动初始化阶段交互式分割语义组并定义关键帧，通过插值生成粗略动画；2）运动细化阶段使用基于组的位移网络（GDN）预测组特定位移场，并利用文本到视频模型的先验知识，包含上下文条件特征增强（CCFE）等专门模块以提高时间一致性。
...
TL;DR: D3FNet是一个针对高分辨率遥感图像中狭窄道路提取的深度学习网络，通过差分注意力扩张提取、双流解码融合机制和多尺度扩张策略，在复杂场景下实现了优异的狭窄道路分割性能。


<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感图像中的狭窄道路提取面临宽度有限、拓扑结构碎片化和频繁遮挡等挑战，传统模型容易过拟合到一般道路宽度，无法有效处理细粒度、遮挡和低对比度的道路段。
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时自适应方法存在两大问题：1）依赖反向传播或迭代优化，限制了可扩展性和实时部署；2）缺乏对类别条件特征分布的显式建模，导致决策边界不可靠和预测校准不足。

Method: 将测试时自适应重新构建为高斯概率推理任务，使用逐渐更新的类别均值和共享协方差矩阵建模类别条件似然，实现闭式、无训练推理。引入CLIP先验和历史知识库指导的轻量级正则化来纠正可能的似然偏差。

Result: 在多样化基准测试上的广泛实验表明，该方法在广泛的分布偏移下实现了最先进的性能，具有卓越的可扩展性和鲁棒性。

Conclusion: ADAPT方法无需源数据、梯度更新或完整访问目标数据，支持在线和转导设置，为测试时自适应提供了一种高效可靠的解决方案。
...
Method: 采用两阶段训练策略：第一阶段使用邻域感知软掩码为局部变化强烈的像素分配更高权重，鼓励模型早期关注细节；第二阶段进行全图像训练

Result: 实验结果表明该方法能一致提升重建质量，并与现有INR方法形成互补
...
TL;DR: 本文提出了一种新的线性时间凸松弛方法和contractor方法，用于加速分支定界算法，能够以两个数量级的速度提升解决点云配准问题，在0.5秒内实现100个点的全局最优配准。


<details>
  <summary>Details</summary>
Motivation: 现有的半定规划松弛方法虽然能保证全局最优性，但计算速度极慢（数百秒处理100个点），无法满足实际应用需求，需要开发更高效的全局优化算法。

Method: 提出线性时间凸松弛方法和contractor方法，结合分支定界算法，专门针对旋转轴已知的情况进行优化，大幅提升求解效率。

Result: 新方法能在0.5秒内完成100个点的3D点云配准，比现有SDP求解器STRIDE快两个数量级，同时保持全局最优性保证。

Conclusion: 该方法在旋转轴已知的情况下实现了显著的速度提升，虽然目前还不能解决完整的6自由度问题，但在特定场景下提供了高效的全局最优解方案。
...
<details>
  <summary>Details</summary>
Motivation: 了解野生动物和人类活动的空间分布对于评估人兽互动和制定有效保护规划至关重要，需要多视角监测方法来捕捉分布模式。

Method: 使用可见光/近红外相机陷阱和热红外无人机收集图像，构建深度学习模型（YOLOv11s和增强版Faster RCNN）进行自动识别，并进行空间模式分析识别活动热点区。

Result: YOLOv11s模型表现最佳（精度96.2%，召回率92.3%），无人机热成像提供了补充视角，空间分析识别出明确的野生动物和人类活动热点区及潜在冲突区域。

Conclusion: 多视角监测与自动化目标检测相结合可增强野生动物监测和景观管理能力，揭示了保护景观内的人兽冲突问题。
...
TL;DR: Grounded VideoDiT是一个视频大语言模型，通过扩散时间潜在编码器、对象接地表示和混合令牌方案，解决了现有视频LLM在时间感知方面的局限性，在多个基准测试中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有视频LLM在时间感知方面存在不足：时间戳编码隐式、帧级特征连续性差、语言视觉对齐容易漂移。需要开发能够精确定位事件发生时间和实体交互的模型。

Method: 1) 扩散时间潜在(DTL)编码器增强边界敏感性和时间一致性；2) 对象接地表示将查询实体显式绑定到局部视觉证据；3) 混合令牌方案使用离散时间令牌进行显式时间戳建模

Result: 在Charades STA、NExT GQA和多个VideoQA基准测试中取得了最先进的性能表现，验证了模型的强大接地能力

Conclusion: Grounded VideoDiT通过三个关键创新解决了视频LLM的时间感知问题，为细粒度时间推理提供了有效解决方案，在多个视频理解任务上表现出色
...
TL;DR: 提出了一种弱监督方法，通过人工质量评估来改进机载激光扫描数据的树木实例分割，在正确识别树木实例方面提升了34%的性能。


<details>
  <summary>Details</summary>
Motivation: 机载激光扫描数据的树木实例分割对森林监测至关重要，但由于数据变化和精确标注数据获取成本高昂，完全监督方法面临挑战。

Method: 使用非微调模型或闭式算法获得初始分割结果，由人工操作员进行质量评级，然后训练评级模型对分割输出进行分类，最后利用评级模型的反馈微调分割模型。

Result: 原始分割模型在正确识别树木实例方面提升了34%，同时显著减少了非树木实例的预测数量。

Conclusion: 该方法有效解决了树木实例分割的挑战，但在稀疏林区小树木和复杂环境中的性能仍有待提升。
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中难以获取目标模型的任何信息，因此需要研究基于迁移的黑盒攻击方法，生成无需目标模型信息的对抗点云

Method: 提出CFG方法，通过计算提取特征的重要性来正则化对抗点云搜索，优先破坏可能被多种架构采用的关键特征，并在损失函数中约束最大偏差程度以确保不可感知性
...
TL;DR: MapKD是一个新颖的多层次跨模态知识蒸馏框架，通过教师-教练-学生范式将多模态模型的知识转移到轻量级视觉学生模型中，显著提升在线高清地图构建性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有在线高清地图构建方法依赖离线地图先验和多模态传感器，导致推理时计算开销过大的问题，旨在开发高效、低成本的视觉中心模型。

Method: 提出MapKD框架，包含：1）相机-LiDAR融合教师模型；2）带先验知识和模拟LiDAR的视觉教练模型；3）轻量级视觉学生模型。采用TGPD和MSRD两种知识蒸馏策略进行特征对齐和语义学习指导。

Result: 在nuScenes数据集上，学生模型性能提升+6.68 mIoU和+10.94 mAP，同时加速推理速度。

Conclusion: MapKD通过知识蒸馏成功实现了从多模态模型到视觉模型的跨模态知识转移，为自动驾驶系统提供了高效、低成本的在线地图构建解决方案。
...
TL;DR: 提出CM2LoD3方法，通过冲突图和语义分割自动重建LoD3建筑模型，解决了传统手动建模的挑战


<details>
  <summary>Details</summary>
Motivation: 现有LoD1和LoD2建筑模型缺乏详细立面元素，LoD3模型需要手动建模难以大规模应用，需要自动化解决方案

Method: 利用射线到模型先验分析获得冲突图，通过语义冲突图生成器进行语义分割，并结合纹理模型分割结果进行融合

Result: 在建筑开口分割和重建方面表现有效，通过不确定性感知融合达到61%的性能

Conclusion: 该方法推进了自动化LoD3模型重建，为可扩展的高效3D城市建模铺平了道路
...
TL;DR: 提出MDPR框架解决预训练视觉语言模型在类别不平衡场景下微调的偏差问题，通过多维动态提示路由机制实现稳定预测


<details>
  <summary>Details</summary>
Motivation: 现有方法在利用大语言模型增强视觉语言模型微调时，忽略了预训练数据中固有的类别不平衡问题，导致下游任务中的偏差累积
...
Conclusion: MDPR为数据不平衡下的VLM微调提供了一种灵活高效的增强方案，语义库对尾部类别特别有效
...
TL;DR: WorldWeaver是一个用于生成长视频的框架，通过联合建模RGB帧和感知条件来解决长序列中的结构一致性问题，利用深度线索和分段噪声调度来减少漂移并提高视频质量。


<details>
  <summary>Details</summary>
Motivation: 当前生成视频建模方法主要依赖RGB信号，在长序列中会导致物体结构和运动误差累积，需要一种能够确保结构一致性和时间一致性的新方法。

Method: 提出WorldWeaver框架，联合预测感知条件和颜色信息，利用深度线索构建记忆库，采用分段噪声调度训练预测组，在扩散和整流流模型上进行实验。

Result: 实验证明WorldWeaver能有效减少时间漂移，提高生成视频的保真度，在长序列视频生成中表现出更好的质量。

Conclusion: WorldWeaver通过统一的长时程建模方案，显著提升了视频生成的时间一致性和运动动态，为长视频生成提供了有效的解决方案。
...
TL;DR: 提出了分子增强的All-in-SAM模型，通过结合分子信息来改进计算病理学中的细胞分割和分类性能，减少标注工作量并提高在资源受限环境中的可访问性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉基础模型（如SAM）在细粒度语义分割方面存在挑战，特别是在识别特定细胞亚型时表现不佳。需要开发能够利用分子信息增强分割精度的新方法。

Method: 采用全栈方法：1）通过分子增强学习减少像素级标注需求；2）使用SAM适配器调整模型以关注特定语义；3）整合分子导向校正学习（MOCL）提升分割精度。
...
TL;DR: Waver是一个高性能的多模态生成基础模型，能够统一生成图像和视频，支持文本到视频、图像到视频和文本到图像生成，在多个基准测试中排名前三。
...
Result: 能够直接生成5-10秒的720p原生分辨率视频（可上采样至1080p），在Artificial Analysis的T2V和I2V排行榜中均排名前三，超越现有开源模型并媲美商业解决方案。

Conclusion: Waver在复杂运动捕捉、运动幅度和时间一致性方面表现优异，为社区提供了高效训练高质量视频生成模型的方法，有望加速视频生成技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有参数化人体模型在捕捉多样化姿态和形状细节方面存在困难，主要由于训练数据多样性有限和建模假设限制。传统方法先优化外部体表再回归内部骨骼关节，导致内部骨骼与外部软组织之间存在问题依赖关系，限制了直接控制身高和骨长的能力。
...
Conclusion: ATLAS通过骨骼基础的显式解耦实现了增强的形状表达能力、细粒度身体属性定制以及独立于外部软组织特征的关键点拟合，为高保真人体建模提供了更优的解决方案。
...
Method: 提出SceneGen框架，包含特征聚合模块整合视觉和几何编码器的局部和全局信息，配合位置头在单次前向传递中生成3D资产及其相对空间位置。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在指令引导图像编辑中存在全局去噪过程导致编辑区域与整个图像上下文纠缠的问题，造成意外修改和指令遵循度下降。自回归模型因其因果和组合机制，能够自然避免这些问题。

Method: 提出VAREdit框架，将图像编辑重新定义为下一尺度预测问题。通过源图像特征和文本指令条件化，生成多尺度目标特征。引入尺度对齐参考(SAR)模块，在第一个自注意力层注入尺度匹配的条件信息。

Result: 在标准基准测试中，VAREdit比领先的扩散方法高出30%以上的GPT-Balance分数。完成512×512图像编辑仅需1.2秒，比同等规模的UltraEdit快2.2倍。
...
TL;DR: 本文提出了一种可扩展的组推理方法，通过将组推理建模为二次整数分配问题，在提升样本质量的同时最大化组内多样性，解决了独立采样导致的冗余问题。


<details>
  <summary>Details</summary>
Motivation: 在现实应用中，用户通常需要获得一组多样化的输出样本（如4-8个），但传统的独立采样方法往往产生冗余结果，限制了用户选择和创意探索。

Method: 将组推理建模为二次整数分配问题：候选输出作为图节点，通过优化样本质量（一元项）和最大化组多样性（二元项）来选择子集。采用渐进式剪枝策略提高运行效率，可扩展到大型候选集。

Result: 大量实验表明，该方法相比独立采样基线和近期推理算法，显著提升了组的多样性和质量。框架可泛化到文本到图像、图像到图像、图像提示和视频生成等多种任务。
...
TL;DR: CineScale是一种新的推理范式，无需微调即可实现更高分辨率的视觉生成，支持8K图像和4K视频生成


<details>
  <summary>Details</summary>
Motivation: 解决视觉扩散模型因训练分辨率限制导致高分辨率生成时出现重复模式和低质量内容的问题

Method: 针对不同类型视频生成架构提出专用变体，基于开源视频生成框架实现高分辨率I2V和V2V合成

Result: 实验验证了该范式在扩展图像和视频模型高分辨率生成能力方面的优越性，实现8K图像生成和4K视频生成

Conclusion: CineScale成功突破了预训练模型的高分辨率生成限制，为高质量视觉内容生成提供了有效解决方案
